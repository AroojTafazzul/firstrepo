...
(function(){
	var d = dojo,
		...
		theme = false,
		testMode = null,
		defTheme = "claro base demobank",
		packageName = "misys.themes",
		vars={};
	...
	if(theme || testMode || dir){
		...
		if(theme){
			...
			var themeCss = d.moduleUrl(packageName, theme+"/"+theme+".css");
			//var themeCssRtl = d.moduleUrl(packageName, theme+"/"+theme+"_rtl.css");
		...
		(originalOnLoad || dojo.addOnLoad)(function(){
			...
			if(theme){
					dojo.attr(b, "class", "");
					if(!d.hasClass(b, theme)){ d.attr(b, "class", "claro base " + theme); }
