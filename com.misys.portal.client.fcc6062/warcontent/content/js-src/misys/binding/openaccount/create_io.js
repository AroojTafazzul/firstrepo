dojo.provide("misys.binding.openaccount.create_io");
/*
 ----------------------------------------------------------
 Event Binding for
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m._config, {
		xmlTransform : function (/*String*/ xml) {
			...
			if(close && xmlString!=="" && comments !== "" ) {
				...
				var xmlRoot = "<io_tnx_record>",
				transformedXml = [xmlRoot],			
						ioXMLEnd   = "</io_tnx_record>",
						subIOXML	= "" ;
				...
				subIOXML = xmlString.substring(xmlRoot.length,(xmlString.length-ioXMLEnd.length));
				transformedXml.push(subIOXML);
				transformedXml.push(comment);
				if(reauthPass && reauthPass !== "")
				{
					transformedXml.push(reauthPass);
				}
				transformedXml.push(ioXMLEnd);
	...
	d.mixin(m, {
		...
		bind : function() {
		    ...
		    //Don't validate issue date for re-submission and close.
		    if(dj.byId("tnxtype") && !(dj.byId("tnxtype").get("value") === "31" || dj.byId("tnxtype").get("value") === "38")){
...
//			m.connect("transport_type", "onFocus", m.checkRoutingSummaryUnicity);
			...
			m.connect("close_tnx","onClick",function() {
				var fieldsToToggle = ["bill_to_name","bill_to_town_name","bill_to_post_code","bill_to_country",
				                      ...
				                      "seller_account_type_code","seller_account_type_prop","entity","buyer_name","buyer_country",
				                      "seller_name","seller_country","seller_bank_bic","issuing_bank_abbv_name","buyer_bank_bic"];
				...
				if(dj.byId("close_tnx") && (dj.byId("close_tnx").get("checked") === true)) 
				{
					...
					//dj.byId("expiry_place").set("readOnly", true);
					dj.byId("bpo_used_status").set("disabled", true);
					...
					for(var i in fieldsToToggle)
					{
						...
						{
							originalFieldState[fieldsToToggle[i]] = dj.byId(fieldsToToggle[i]).get("required"); 
				...
				{
					var maxPmtOblgtnExpiryDate = m.getMaxBPOExpDate();
					...
					//dj.byId("expiry_place").set("readOnly", false);
					dj.byId("bpo_used_status").set("disabled", false);
					
					for(var j in originalFieldState)
					{
						if(dj.byId(j))
						{
							m.toggleRequired(j,originalFieldState[j]);						
						}
					}
									
					//Validate the re-submission count
			...
			/*m.connect("pmt_term_code","onChange", function() {
				if(d.byId("paymentTermCode") && dj.byId("details_code") && dj.byId("details_code").get("value") !== "") {
					dj.byId("details_code").set("disabled",false);
					dj.byId("details_nb_days").set("disabled",false);
				}
				else if(dj.byId("details_code") && dj.byId("details_nb_days") && dj.byId("pmt_term_code")) {
					dj.byId("details_code").set("disabled",false);
					dj.byId("details_nb_days").set("disabled",false);
				}
			});*/
...
//Including the client specific implementation
       dojo.require('misys.client.binding.openaccount.create_io_client');