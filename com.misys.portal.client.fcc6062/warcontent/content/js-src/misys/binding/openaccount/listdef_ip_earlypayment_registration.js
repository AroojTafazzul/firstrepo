dojo.provide("misys.binding.openaccount.listdef_ip_earlypayment_registration");

/*
-----------------------------------------------------------------------------
Scripts for the list def binding for Early Payment Registration
...
(function(/*Dojo*/d, /*dj*/dj, /*Misys*/m) {
	...
	d.mixin(m._config, {
		saveOpenAccount : function(programcode) {
			...
			//m.grid.processRecords(grids, "/screen/AjaxScreen/action/RunGroupSubmission?s=InvoicePayableScreen&operation=SAVE&option=IP_FOLDER&mode=DRAFT&tnxtype=01&referenceid="+referenceid+"&tnxid="+tnxid+"&programcode="+programcode);
		},
		...
		showTermsAndConditions : function(rowIndex)
		{
			...
			var displayMessage;
				...
				{
					displayMessage	=	this.grid.store._items[rowIndex].i["Counterparty@ObjectDataClob@conditions_01"];
		...
		mouseOverShowTermsAndConditions : function(evt)
		{
			...
			{
				var displayMessage;
				displayMessage	=	this.store._items[evt.rowIndex].i["Counterparty@ObjectDataClob@conditions_01"];
				displayMessage 	= 	m.replaceCarriageReturn(displayMessage,"<br>");
		...
		initReAuthParams : function(){	
			var reAuthParams 		= 	{productCode 		: "EPR",
					         			 subProductCode 	: "",
...
//				reauthURLParameter.push("&reauth_e2ee_pki=",dj.byId("ra_e2ee_pki").get("value"));
	...
	function _setAmountValue() {
       ...
       dojo.require('misys.client.binding.openaccount.listdef_ip_earlypayment_registration_client');