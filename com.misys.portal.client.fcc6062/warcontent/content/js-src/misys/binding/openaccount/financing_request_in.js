 ...
 *//**
 * 
 */dojo.provide("misys.binding.openaccount.financing_request_in");
/*
-----------------------------------------------------------------------------
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m, {
			...
			m.connect("inv_eligible_pct","onChange",function(){
				if(this.get("value") !== null && !isNaN(this.get("value")) && parseFloat(this.get("value")) <= 100 && parseFloat(this.get("value")) > 0)
				{
					...
					var liabTotalNetAmt = parseFloat(dj.byId("invoice_outstanding_amt").get("value").replaceAll(",", ""));
					...
					if(liabTotalNetAmt < invEligibleAmt)
					{   
						liabTotalNetAmt = liabTotalNetAmt-totalDiscountAmt;
						convertedAmt = (parseFloat(this.get("value")) * liabTotalNetAmt) / 100;
					}
					else
					{   
					...
					dj.byId("liab_total_net_amt").set("value", liabTotalNetAmt);
				}
				else
				{
	...
	beforeSubmitValidations : function() {
		...
		if((prodCode && prodCode.get("value") == 'IN') && (freeFormatText && dojo.string.trim(freeFormatText.get("value")) === ""))
	    {
...
//Including the client specific implementation
       dojo.require("misys.client.binding.openaccount.financing_request_in_client");