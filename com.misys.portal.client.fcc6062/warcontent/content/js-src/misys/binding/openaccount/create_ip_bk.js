/*
 * ---------------------------------------------------------- 
 * Event Binding for Bulk in Invoice Payable
 * 
 * Copyright (c) 2000-2011 Misys (http://www.misys.com), All Rights Reserved.
 * 
 * version: 1.0 date: 28/10/16
 * 
 * ----------------------------------------------------------
 */
dojo.provide("misys.binding.openaccount.create_ip_bk");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m._config, {
		...
		xmlTransform : function (/*String*/ xml) {
			...
			if(xml.indexOf(tdXMLStart) !== -1){
				...
				{
					transformedXml.push("<invoice_payable_items>");
					var grid = dj.byId("invoicePayablesGrid");
					var numRows = grid.rowCount;
					
					for (var i = 0; i < numRows; i++){
						var item = grid.getItem(i);
						transformedXml.push("<invoice_payable>");
						transformedXml.push("<invoice_payable_ref_id>",grid.store.getValue(item, "REFERENCEID"),"</invoice_payable_ref_id>");
						transformedXml.push("</invoice_payable>");
					}
					transformedXml.push("</invoice_payable_items>");
				}
		...
		initReAuthParams : function(){	
						console.debug("[misys.binding.openaccount.create_ip_bk] Bulk Method Start: initReAuthParams");
			...
			var reAuthParams = { 	productCode : "BK",
						...
						console.debug("[misys.binding.openaccount.create_ip_bk] Bulk Method End: initReAuthParams");
			return reAuthParams;
		}
		});
	...
	d.mixin(m, {
		...
		bind : function() {
			m.connect("invoice_payable_lookup","onClick", function(){
				...
				if(customerReference && (customerReference.get("value") === "")){
						m.dialog.show("ERROR", m.getLocalization("selectIssuerReferenceForInvoicePayable"));
						return false;
				}
				m.getInvoicePayablesForBulk("IP");
				return true;
			});
			...
			m.connect("entity", "onChange", function(){
				...
				var invoicesGrid = dj.byId("invoicePayablesGrid");
		...
		onFormLoad : function(){						
			...
			if(invoiceItems && invoiceData && invoiceItems.length > 0)
			{
				...
				//add - update
				if(dj.byId("invoicePayablesGrid") && invoiceItems && invoiceItems.length > 0) {
					for(var i=0;i<invoiceItems.length;i++) {
						...
						var actn= "<img src="+misys.getContextualURL('/content/images/delete.png')+" onClick =\"javascript:misys.deleteInvoicePayableRecord('" + item.REFERENCEID + "')\"/>";
						var newItem = {"REFERENCEID" : refId, "ENTITY" : entity, "PROGRAM_NAME" : fscmProgrammeCode, "SELLER_NAME" :sellerName, "CURCODE" :totalNetCurCode, "AMOUNT" :totalNetAmt,"OS_AMOUNT" :liabTotalAmt,"INVOICE_DATE" :issDate,"INVOICE_DUE_DATE" :dueDate,"STATUS" :prodStatCode, "FIN_REQUESTED_PERCENT" : financeReqPercent, "ACTION" : actn};				
			...
			else if(dj.byId("invoicePayablesGrid")){			
			// when from form load
				m.getInvoicePayablesForBulkLinkedFiltered();
			}
...
dojo.require("misys.client.binding.openaccount.create_ip_bk_client");