dojo.provide("misys.binding.openaccount.listdef_ip_common");
...
(function(/*Dojo*/d, /*dj*/dj, /*Misys*/m) {
	...
	d.mixin(m._config, {
		saveOpenAccount : function(programcode) {
			...
			m.grid.processRecords(grids, "/screen/AjaxScreen/action/RunGroupSubmission?s=InvoicePayableScreen&operation=SUBMIT&option=IP_FOLDER&mode=DRAFT&tnxtype=01&referenceid="+referenceid+"&tnxid="+tnxid+"&programcode="+programcode);
		},
		...
		showTermsAndConditions : function(evt)
		{
			...
			{
			var displayMessage=this.store._items[evt.rowIndex].i['Counterparty@ObjectDataString@conditions_01'];
	...
	function _setAmountValue() {
       ...
       dojo.require('misys.client.binding.openaccount.listdef_ip_common_client');