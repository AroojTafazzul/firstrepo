dojo.provide("misys.binding.openaccount.create_in_repayment");
/*
 ----------------------------------------------------------
 Event Binding for

   Invoice (IN) Form, Customer Side.
	...
	d.mixin(m, {
		...
		onFormLoad : function() {
			...
			if(beneAccessOpened && accessOpened){
				console.debug("[create_in_repayment], Setting access_opened value");
			...
			if(beneEmail && cptyEmail){
				...
				console.debug("[create_in_repayment], Setting transaction_counterparty_email value");
			}
			...
			if(beneCompAbbvName && abbvName){
				...
				console.debug("[create_in_repayment], Setting ben_company_abbv_name value");
			}
		}
	});
...
////Including the client specific implementation
       dojo.require("misys.client.binding.openaccount.create_in_repayment_client");