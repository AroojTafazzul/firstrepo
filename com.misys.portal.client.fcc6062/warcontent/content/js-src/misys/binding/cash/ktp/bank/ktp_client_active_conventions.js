dojo.provide("misys.binding.cash.ktp.bank.ktp_client_active_conventions");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m, {
		...
		onFormLoad : function(value, index)
		{
			...
			m._config.gridFirstRowAlreadyHidden = false;
			...
			// Look for query read store
			d.connect(grid.store, "fetch", function(){
				if (!m._config.gridFirstRowAlreadyHidden)
				{
					// Hide empty header row
					d.place("<div id='firstRowHider' style='width=:100%;height:12px;position:relative;top:37px;z-index:255;background-color:white'></div>", d.query("#hugeDiv > .widgetContainer")[0], "first");
					m._config.gridFirstRowAlreadyHidden = true;
				}
			});
		}
	});
