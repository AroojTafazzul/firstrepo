/* ---------------------------------------------------------- 
 * Event Binding for HK domestic transfers
 * Copyright (c) 2000-2019 Finastra (http://www.misys.com), All Rights Reserved.
 * version: 1.0 date: 19/05/2019
 * ----------------------------------------------------------
 */
dojo.provide("misys.binding.cash.create_ft_rtgs");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
		...
		if ( applicant_act_cur_code !== beneficiary_ft_cur_code)
			{
				...
				if(dj.byId("applicant_act_pab").get("value") !== 'Y' && dj.byId("cpty_bank_country_img"))
				{
	...
	CptybicCode.fields = [ "cpty_bank_name", "cpty_bank_address_line_1",
			"cpty_bank_address_line_2", "cpty_bank_dom", "cpty_bank_country" ];
	...
	interBicCode.fields = [ "intermediary_bank_name",
			"intermediary_bank_address_line_1",
			"intermediary_bank_address_line_2", "intermediary_bank_dom",
			"intermediary_bank_country" ];
	...
	//Below function checks for the number of email id provided in the alternative email field should not exceed 10
	...
	{	
		var regex = dj.byId("customerReferenceRtgs");
	    var regexStr = regex ? dj.byId("customerReferenceRtgs").get("value") : '';		 
		...
		var errorMessage = null;
	     ...
	     {			   			   
			   ...
			   if(!isValid){
			   errorMessage =  m.getLocalization("invalidCustRefRTGS");
		...
		if(beneficiaryName !== "")
		{
			...
			if(dj.byId("pre_approved_status") && dj.byId("pre_approved_status").get("value") === "Y")
			{
				...
				if(dj.byId("intermediary_bank_swift_bic_code") && dj.byId("intermediary_bank_swift_bic_code").get("value") !== "" && 
						dj.byId("intermediary_bank_swift_bic_code").get("value") !== null)
				{
			...
			else
			{
				...
				if(dj.byId("intermediary_bank_swift_bic_code") && dj.byId("intermediary_bank_swift_bic_code").get("value") !== "")
				{
					m.toggleFieldsReadOnly(["intermediary_bank_swift_bic_code","intermediary_bank_name","intermediary_bank_address_line_1",
						"intermediary_bank_address_line_2", "intermediary_bank_dom", "intermediary_bank_country"], true);
				}
				if(dj.byId("beneficiary_iso_img"))
				{
					dj.byId("beneficiary_iso_img").set("disabled",false);
				}
				if(dj.byId("beneficiary_country_btn_img") && dj.byId("beneficiary_country").get("value") !== "" && 
						dj.byId("beneficiary_country").get("value") !== null)
				{
					dj.byId("beneficiary_country_btn_img").set("disabled",true);
				}
				if(dj.byId("cpty_bank_swift_bic_code") && dj.byId("cpty_bank_swift_bic_code").get("value").length !==0)
		    	{
					m.toggleFieldsReadOnly( ["cpty_bank_swift_bic_code","cpty_bank_name","cpty_bank_address_line_1","cpty_bank_address_line_2",
						"cpty_bank_dom","cpty_bank_country"], true);
		    	}
				if(dj.byId("intermediary_bank_swift_bic_code") && dj.byId("intermediary_bank_swift_bic_code").get("value").length !==0)
		    	{
					m.toggleFieldsReadOnly( ["intermediary_bank_swift_bic_code","intermediary_bank_name","intermediary_bank_address_line_1",
	...
	function _beneficiaryButtonHandler()
	{
		...
		m.showSearchDialog("beneficiary_accounts", 
							   ...
							   "'pre_approved_status', 'change_counterparty_name_2', 'change_counterparty_name_3', 'cpty_bank_name', 'cpty_bank_address_line_1'," +
							   " 'cpty_bank_address_line_2', " +
							   "'cpty_bank_dom', 'beneficiary_bank_branch_address_line_1', 'beneficiary_bank_branch_address_line_2', 'beneficiary_bank_branch_dom'," +
							   " 'cpty_bank_country', " +
							   ...
							   "'bene_adv_mailing_name_add_2_no_send','bene_adv_mailing_name_add_3_no_send','bene_adv_mailing_name_add_4_no_send'," +
							   "'bene_adv_mailing_name_add_5_no_send'," +
							   "'bene_adv_mailing_name_add_6_no_send','bene_adv_postal_code_no_send','bene_adv_country_no_send','bene_email_1','bene_email_2', " +
							   "'beneficiary_postal_code','beneficiary_country'," +
							   "'bene_adv_fax_no_send', 'bene_adv_ivr_no_send','beneficiary_nickname', 'bene_adv_phone_no_send','intermediary_bank_swift_bic_code'," +
	...
	function _onPreApprovedChangeToggleBranchAdress()
	{
		if(dj.byId("branch_address_flag"))
		{
			...
			{
				m.toggleFieldsReadOnly(["branch_address_flag","beneficiary_bank_branch_address_line_1","beneficiary_bank_branch_address_line_2",
					"beneficiary_bank_branch_dom"], true);
			}
			else
			{
				if(	(dj.byId("pre_approved") && dj.byId("pre_approved").get("value") === "") || (dj.byId("pre_approved") && dj.byId("pre_approved").get("value") === null))
				{
					m.toggleFieldsReadOnly(["branch_address_flag","beneficiary_bank_branch_address_line_1","beneficiary_bank_branch_address_line_2",
						"beneficiary_bank_branch_dom"], false);
				}
				else
				{
					m.toggleFieldsReadOnly(["branch_address_flag","beneficiary_bank_branch_address_line_1","beneficiary_bank_branch_address_line_2",
	...
	function _handleCusomterBankOnChangeFields()
	{
		...
		if(misys && misys._config && misys._config.businessDateForBank && misys._config.businessDateForBank[customer_bank] && 
	...
	function _applicantAccountChangeHandler()
	{
		...
		dojo.forEach(["pre_approved", "beneficiary_account", "beneficiary_act_cur_code", "beneficiary_address", "beneficiary_city", "beneficiary_dom", 
			"pre_approved_status", "cpty_bank_name", 
					"cpty_bank_address_line_1", "cpty_bank_address_line_2", "cpty_bank_dom", "cpty_branch_address_line_1", "cpty_branch_address_line_2",
					"cpty_branch_dom", "cpty_bank_country",
					"cpty_bank_swift_bic_code", "cpty_bank_code", "cpty_branch_code", "cpty_branch_name", "beneficiary_name", 
					"bene_adv_beneficiary_id_no_send", 
					"bene_adv_mailing_name_add_1_no_send","bene_adv_mailing_name_add_2_no_send","bene_adv_mailing_name_add_3_no_send",
					"bene_adv_mailing_name_add_4_no_send", 
					"bene_adv_mailing_name_add_5_no_send","bene_adv_mailing_name_add_6_no_send","bene_adv_postal_code_no_send",
					"bene_adv_country_no_send","bene_email_1", 
					"bene_email_2","beneficiary_postal_code","beneficiary_country", "bene_adv_fax_no_send", "bene_adv_ivr_no_send", "bene_adv_phone_no_send",
					"bene_adv_email_no_send1", 
					"bene_adv_email_no_send", "ft_amt", "ft_cur_code", "intermediary_bank_swift_bic_code","intermediary_bank_name",
					"intermediary_bank_address_line_1","intermediary_bank_address_line_2",
					"intermediary_bank_dom","intermediary_bank_country","beneficiary_bank_branch_address_line_1","beneficiary_bank_branch_address_line_2",
					"beneficiary_bank_branch_dom"], function(currentField) {
	...
	{	
	    ...
	    var isValid = false;
		//var beneName = dj.byId(field);
		var beneNameString = dj.byId("beneficiary_name").get("value") ? dj.byId("beneficiary_name").get("value") : '';
	...
	function _validateAccountRegexBeneficiary(){
		...
		if(regexStr !== null && regexStr !== ''){
		     ...
		     {			   			   
				   ...
				   if(!isValid){
					   errorMessage =  m.getLocalization("invalidBeneficiaryAccountforRtgs");
	...
	function _validateInstructionToBranchRegx(field){
	   var regex = dj.byId("beneAddressRegex");
	   var regexStr = regex ? dj.byId("beneAddressRegex").get("value") : '';
	   var beneficiaryRegExp = new RegExp(regexStr); 
	   ...
	   var regexFor1stChar = new RegExp(regexFor1stCharStr);
		...
		var splitline=instLineStr.split("\n");
		var errorMessage = null;
		if(regexStr !== null && regexStr !== ''){
	         if(instLineStr !== '')
		     {			   			   
				   ...
				   if(!isValid){
				   	...
				   	splitline.forEach(function(splitline){
							  if(regexFor1stChar.test(splitline)){
	...
	function _validateInstructionToBranchRegxSubmit(field){
		   var regex = dj.byId("beneAddressRegex");
		   var regexStr = regex ? dj.byId("beneAddressRegex").get("value") : '';
		   var beneficiaryRegExp = new RegExp(regexStr); 
		   var regexFor1stCharStr = dj.byId("addressStartCharsRegex") ? dj.byId("addressStartCharsRegex").get("value") : '';
    ...
    function _validateBeneficiaryAccount()
	{
		...
		var account_number_field	=	dj.byId("beneficiary_account"),
			company_id= dj.byId("company_id"),
	     valid = false;
		
		m.xhrPost({
			url 		: misys.getServletURL("/screen/AjaxScreen/action/ValidateBeneMasterAccount") ,
			handleAs 	: "json",
			sync 		: true,
			...
			if (swift_code_field.get("value") !== "")
			{
				m.xhrPost(
				{
					...
					load : function(response, args)
					{
						...
						{
							displayMessage = misys.getLocalization("invalidBankCode");
							swift_code_field.focusNode.focus();
							swift_code_field.set("value","");
							swift_code_field.set("state", "Error");
							dj.hideTooltip(swift_code_field.domNode);
							dj.showTooltip(displayMessage, swift_code_field.domNode, 0);
	...
	function _validateBranchAddressRegex(field){
		...
		var regexFor1stCharStr = dj.byId("addressStartCharsRegex") ? dj.byId("addressStartCharsRegex").get("value") : '';
		var regexFor1stChar = new RegExp(regexFor1stCharStr);
		var rtgsisValid = false;
		var isValidAddrs = false;
...
d.mixin(m, {
		...
		bind : function() {
			...
			m.connect("beneficiary_address", "onBlur",function(){ _validateBranchAddressRegex(dj.byId("beneficiary_address"));});
			m.connect("beneficiary_city", "onBlur", function(){_validateBranchAddressRegex(dj.byId("beneficiary_city"));});
			m.connect("beneficiary_dom", "onBlur", function(){_validateBranchAddressRegex(dj.byId("beneficiary_dom"));});
...
//			});
			...
			m.connect("payment_details_to_beneficiary", "onBlur", function(){
				_validateInstructionToBranchRegx(dj.byId("payment_details_to_beneficiary"));
			});
		...
		onFormLoad : function() {
			...
			d.forEach(d.query(".rtgsDisclaimer"),function(node){
		...
		initForm : function()
		{
			...
			if(applicantAcctPAB === 'Y' || (dj.byId("pre_approved_status") && dj.byId("pre_approved_status").get("value") === "Y")) {
			 ...
			 if(_validateBeneficiaryAccount())
				 {
					m.toggleFieldsReadOnly(this.fields, true);

				 }
			 else
				 {
					m.toggleFieldsReadOnly(this.fields, false);

				 }
			 if(dj.byId("intermediary_bank_swift_bic_code").get("value").length !== 0 && dj.byId("has_intermediary"))
				{
		...
		setCustomConfirmMessage : function(){
	     	...
	     	var subPdtCode = dj.byId("sub_product_code") ? dj.byId("sub_product_code").get("value") : "";
	     	 if(modeValue === "DRAFT" && subPdtCode === "RTGS")
    		 {
	     		var  hostResponse ;
	     		var gppFeeConfigFlag = dj.byId("gpp_fee_config_flag") ? dj.byId("gpp_fee_config_flag").get("value") : "false";
	     		if(dj.byId("response_status")) {
		...
		beforeSaveValidations : function(){
			...
			beforeSubmitValidations : function() 
			{
	            ...
	            var val = m._validateCurrencyCode(dj.byId("sub_product_code"),
						dj.byId("ft_cur_code"));
	            if(!val)
	            {
				...
				if(dj.byId("cpty_bank_swift_bic_code") && dj.byId("cpty_bank_swift_bic_code").get('value')!== "" && !_populateBankDetailsRtgs()){
					return false;
				}
				...
				//payment narrative 
				if(dj.byId("payment_details_to_beneficiary") && dj.byId("payment_details_to_beneficiary").get("value")!=="" && !_validateInstructionToBranchRegx(dj.byId("payment_details_to_beneficiary"))){
					return false;
				}
			...
			if((dj.byId("recurring_flag") && dj.byId("recurring_flag").get("checked") === false) && gppFeeConfigFlag === "true"){
				...
				if(dj.byId("payment_details_to_beneficiary"))
				{
					var payment_details_to_beneficiary = dj.byId("payment_details_to_beneficiary");
					if(!_validateInstructionToBranchRegxSubmit(payment_details_to_beneficiary))
					{
			...
			{
	    		misys.showSearchDialog("bankBicCodeRTGS","[ 'cpty_bank_swift_bic_code', 'cpty_bank_name', '', 'cpty_bank_address_line_1', 'cpty_bank_address_line_2', 'cpty_bank_dom', 'cpty_bank_country', '','intermediary_flag']", {swiftcode: false, bankcode: false}, "", "",whStyle, m.getLocalization("ListOfBanksTitleMessage"),"","",true);
			},
			showIntermediaryBicCodesforRTGS : function()
			{
	    		misys.showSearchDialog("intermediaryBicCodeRTGS","[ 'intermediary_bank_swift_bic_code', 'intermediary_bank_name', 'intermediary_bank_address_line_1', 'intermediary_bank_address_line_2', 'intermediary_bank_dom', 'intermediary_bank_country']", {swiftcode: false, bankcode: false}, "", "",whStyle, m.getLocalization("ListOfBanksTitleMessage"),"","",true);
			}
});		
})(dojo, dijit, misys);//Including the client specific implementation
dojo.require('misys.client.binding.cash.create_ft_rtgs_client');
