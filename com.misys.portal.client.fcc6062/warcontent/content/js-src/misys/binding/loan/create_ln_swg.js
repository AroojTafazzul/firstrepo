dojo.provide("misys.binding.loan.create_ln_swg");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	/**
	 * validate the legal text accepted or not
	 */
	function isLegalTextAccepted(){
		var isLegalTextField =dj.byId('accept_legal_text');
		if(isLegalTextField){
			if(isLegalTextField.get('checked')){
				
				return true;
			}else{
				m._config.onSubmitErrorMsg =  misys.getLocalization('legalTextErrorMsg');
				return false;
			}
		}
		return true;
 ...
 */
	...
	function _validateLoanMaturityDate(){
		...
		if(thisObject && loanEffDate && thisObject.displayedValue !== '' && loanEffDate.displayedValue !== '')
		{
	...
	{	
		...
		var repricingDateValue =  dj.byId("repricing_date").get("displayedValue");
		...
		var getSize = function(size, request){
					...
					var repricingDateSplit = repricingDateValue.split("/");    // ["29", "1", "2016"]
					var repricingDateObject = new Date(parseInt(repricingDateSplit[2],10),parseInt(repricingDateSplit[1],10)-1,parseInt(repricingDateSplit[0],10));	
				
					repricingDate.set('value',dateObject);
					
					if(dateObject.valueOf()>repricingDateObject.valueOf() || dateObject.valueOf()<repricingDateObject.valueOf())
					{
						var displayMessage = m.getLocalization("repricingDateBusinessDayValidationErrorMsg", [ repricingDateValue ]);
	...
	{	
		...
		var effectiveDateValue =  dj.byId("effective_date").get("displayedValue");
		...
		var getSize = function(size, request){
					...
					if(storeSize!==0){
					    ...
					    // ["29", "1", "2016"]
						var effectiveDateSplit = effectiveDateValue.split("/");    // ["29", "1", "2016"]
						var effectiveDateObject = new Date(parseInt(effectiveDateSplit[2],10),parseInt(effectiveDateSplit[1],10)-1,parseInt(effectiveDateSplit[0],10));	
						...
						if(!((m.compareDateFields(appDateField, that))&& !(m.compareDateFields(defaultEffectiveDate, that)))){
							...
							{
								var displayMessage1 = m.getLocalization("effectiveDateBusinessDayValidationErrorMsg", [ effectiveDateValue ]);
	...
	function _populateDefaultEffectiveDate(response){
		...
		var dateValue=response.dateValues[0].name;
 ...
 */
	function _checkPastDatesBasedOnIntendNoticeValidationForEffectiveDate()
 	{
		...
		if(dj.byId('screenMode').get('value') === 'UNSIGNED'){
			...
			if (!m.compareDateFields(calculatedEffectiveDate, effectiveDate)) {
				m._config.onSubmitErrorMsg = m.getLocalization("intendNoticeDaysErrorMsgForSwingline",
			...
			if (!m._config.repricingDatesValidationInprocess) {
				...
				if (!m.compareDateFields(calculatedEffectiveDate, this)) {
					...
					this.invalidMessage = m.getLocalization("intendNoticeDaysErrorMsgForSwingline",
							[m.getLocalizeDisplayDate(calculatedEffectiveDate) ]);
					return false;
	...
	function _validateLoanRepricingDate(){
		...
		var loanEffDate = dijit.byId('effective_date');
		if(m.compareDateFields(thisObject, loanEffDate))
		{
	...
	function _validateLoanAmount(){
		...
		if (currency && currency.get('value'))
		{
                ...
                if (loanAmountWidget.get('value') > riskTypeLimitAmt){
                      ...
                      if(riskTypeLimitAmt[0] < limitAmountToDiaplay){
                            ...
                            if(riskTypeLimitAmt[0]<=0){
                                  lnAmtInvalidErrorMsg = misys.getLocalization('noAmountForSwingRiskLimitError');
			...
			if (loanAmountWidget.get('value') > swinglineLimit){
				//validate Swingline sublimit available amount limit
				isLoanAmountLimitExceeded = true;
				if(swinglineLimit[0] < limitAmountToDiaplay){
					limitAmountToDiaplay = swinglineLimit[0];
					if(swinglineLimit[0]<=0){
						lnAmtInvalidErrorMsg = misys.getLocalization('noAmountForSwinglineLimitError');
						lnAmtInvalidWarningMsg = m.getLocalization('noAmountForSwinglineLimitError');	
					}
					else{
						lnAmtInvalidErrorMsg = misys.getLocalization('loanAmountTooBigThanSwinglineLimitAmtError',[ currency.get('value'),d.currency.format((Math.floor(limitAmountToDiaplay * 100) / 100), {round: cldrMonetary.round,places: cldrMonetary.places}) ]);
					    lnAmtInvalidWarningMsg = m.getLocalization('loanSublimitExceeded');
					}
				}				
			}
			
			if (loanAmountWidget.get('value') > sublimitRisklimit){
				//validate Swingline risk limit
				isLoanAmountLimitExceeded = true;
				if(sublimitRisklimit[0] < limitAmountToDiaplay){
					limitAmountToDiaplay = sublimitRisklimit[0];
					if(swinglineLimit[0]<=0){
						lnAmtInvalidErrorMsg = misys.getLocalization('noAmountForSublimitRiskLimitError');
						lnAmtInvalidWarningMsg = m.getLocalization('noAmountForSublimitRiskLimitError');	
					}
					else{
						lnAmtInvalidErrorMsg = misys.getLocalization('loanAmountTooBigThanSublimitRiskAmtError',[ currency.get('value'),d.currency.format((Math.floor(limitAmountToDiaplay * 100) / 100), {round: cldrMonetary.round,places: cldrMonetary.places}) ]);
					    lnAmtInvalidWarningMsg = m.getLocalization('loanSublimitRiskExceeded');
					}
				}				
			}
			...
			if (loanAmountWidget.get('value') > limit){
				...
				if(limit[0] <= limitAmountToDiaplay){
					...
					{
						lnAmtInvalidErrorMsg = misys.getLocalization('swinglineLoanAmountTooBigError',[ currency.get('value'), d.currency.format((Math.floor(limitAmountToDiaplay * 100) / 100), {round: cldrMonetary.round,places: cldrMonetary.places})]);
					}
	 ...
	 */	
		...
		function _pastDateValidationForEffectiveDate(){
			...
			if(dj.byId('screenMode').get('value') === 'UNSIGNED'){
				selectedEffectiveDate=effectiveDateUnsignedField.get("value");
				selectedEffectiveDate = selectedEffectiveDate.split("/");
				selectedEffectiveDate = new Date(selectedEffectiveDate[2], selectedEffectiveDate[1]-1, selectedEffectiveDate[0]);
				pricingOptionValue = dijit.byId('pricing_option_unsigned').get('value');
			}else{
				...
				onComplete: dojo.hitch(this, function(items, request){
					dojo.forEach(items, function(item, index){
						...
						else if(remittanceInstDescriptionHiddenValue.get('value') === item.description[0] && remittanceInstLocationCodeHiddenValue.get('value') === item.locationCode[0] && remittanceInstServicingGrpHiddenValue.get('value') === item.servicingGroupAlias[0])
						{
						 ...
						 if((remittanceFlag === "mandatory" && count > 1 && operation === "NEW_SWINGLINE") || (remittanceFlag === "true" && operation === "NEW_SWINGLINE")){
							 remInst.selection.deselect(index) ;
...
//        		}
        	...
        	if(dj.byId('repricing_frequency_view') && dj.byId('repricing_frequency'))
        	{
      	    ...
      	    if(dijit.byId('operationValue') && dijit.byId('operationValue').get('value') === 'NEW_SWINGLINE')
			{
        	...
        	if(dijit.byId('operationValue') && dijit.byId('operationValue') !== '' && dijit.byId('operationValue').get('value') === 'NEW_SWINGLINE'){
				_setupRemittanceInstructions(true);
        	}else{
        		_setupRemittanceInstructions(false);
        	}
		...
		onCancelNavigation : function()
		{
			...
			var cancelButtonURL = ["/screen/LoanScreen?", "operation=List_Deals_Swingline"];
		 ...
		 setCustomConfirmMessage : function(){
		    	console.log('In custom confirmation');
		    		m._config.globalSubmitConfirmationMsg = m.getLocalization("submitTransactionConfirmationForSwingline", ["Swingline"]);
		},
	...
	d.mixin(m._config, {
       ...
       dojo.require('misys.client.binding.loan.create_ln_swg_client');