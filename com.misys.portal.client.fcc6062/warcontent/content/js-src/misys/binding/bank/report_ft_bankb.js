 ...
 * ----------------------------------------------------------
 */
dojo.provide("misys.binding.bank.report_ft_bankb");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	function _preSubmissionFXValidation(){
		...
		if (boardRateOption.get("checked") && totalUtiliseAmt && !isNaN(totalUtiliseAmt.get("value")) && ftAmt && !isNaN(ftAmt.get("value")) && (ftAmt.get("value") < totalUtiliseAmt.get("value")))
		{
	...
	d.mixin(m, {
					...
					var isDirectConversion = false;
						...
						{	
							...
							if(fromCurrency && fromCurrency !== "" && toCurrency && toCurrency !== "")
							{
								...
								if(dj.byId("fx_rates_type_1") && dj.byId("fx_rates_type_1").get("checked") && (isNaN(dj.byId("fx_exchange_rate").get("value")) || dj.byId("fx_exchange_rate_cur_code").get("value") === "" ||
		...
		bind : function()
		{
			...
			dj.byId("sub_product_code").set("value", "BANKB");		
      ...
      //hide fx section by default
       ...
       dojo.require('misys.client.binding.bank.report_ft_bankb_client');