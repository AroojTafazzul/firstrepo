dojo.provide("misys.binding.bank.report_in");
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m, {
		bind : function() {
			...
			else if (tnx_amt_field)
			{
				...
				if (tnx_amt_field && tnx_amt_field.get("value") && liab_total_amt_field && total_net_amt_field)
				{
		...
		onFormLoad : function() {
		  ...
		  var actionReqCodeField = dj.byId("action_req_code");
			...
			/*
			 * Display the entire form for 07(updated). Display transaction details for Updated status only.
			 */
	...
	function _handleStatusChange()
	{
		...
		{
				 if (prod_stat_code_field.get('value') === "46")
				 {
	...
	function _handleEligibileAmt()
	{
		...
		else 
		{
			...
			{
				liab_total_amt_field.set("value",total_net_amt_field.get("value")-inv_eligible_amt_field.get("value"));
				var oldLiabAmtWidget = dj.byId("old_liab_total_amt");
				var prod_stat_code_field 		=	dj.byId("prod_stat_code");
				var liabAmtWidget = dj.byId("liab_total_amt");
				if(prod_stat_code_field.get('value') === "01" && oldLiabAmtWidget && liabAmtWidget)
				{
					liabAmtWidget.set("value",oldLiabAmtWidget.get("value"));
				}
	...
	function _handleFinanceAmt()
	{
		...
		if (finance_amt_field.get("value") >=0 && finance_ccy_field.get("value")&& finance_ccy_field.get("value") == inv_eligible_ccy_field.get("value"))
		{
			...
			else if(total_net_amt_field.get("value")&& (finance_amt_field.get("value") <= total_net_amt_field.get("value")))
				{
				dj.byId("liab_total_amt").set("value",total_net_amt_field.get("value")-finance_amt_field.get("value"));
				}
		}
	}
	...
	function _checkLineItemsCurrencyTotalCurrency()
	{
       ...
       dojo.require('misys.client.binding.bank.report_in_client');