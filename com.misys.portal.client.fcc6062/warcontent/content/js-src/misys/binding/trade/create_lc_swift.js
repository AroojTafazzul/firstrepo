dojo.provide("misys.binding.trade.create_lc_swift");
...
//
// Summary: 
// Event Binding for Letter of Credit (LC) Form specific for SWIFT 2018 enhancements
//
// version:   1.0
// date:      11/11/2017
//@author :   Avilash
...
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	function _fetchNarrativeContent() {
		var isSingleFieldValid = false, isCombinedValid = false;
		...
		if(dj.byId("narrative_description_goods") && dj.byId("narrative_documents_required") && dj.byId("narrative_additional_instructions") && dj.byId("narrative_special_beneficiary")){
			...
			if(!m._config.isBank){
				...
				var combinedArray1=[descOfGoods,docsRequired,addInstructions,specBeneficiary];	
				_checkSingleCombined(combinedArray1);
				
			}
			else{
				allNarratives = descOfGoods + docsRequired + addInstructions + specBeneficiary + specBeneficiaryBank;
				var combinedArray=[descOfGoods,docsRequired,addInstructions,specBeneficiary,specBeneficiaryBank];
				_checkSingleCombined(combinedArray);				
			}
			if(dj.byId(this.id)) {
				...
				singleLines =  singleNarrative.trim().split(/\r*\n/).length;
				isSingleFieldValid = m.validateRowExtNarrativeSwiftInit2018(singleNarrative, true);
			}
			if(combined){
				isCombinedValid = m.validateRowExtNarrativeSwiftInit2018(allNarratives, false);
				}
			
			
			if(isCombinedValid) {
					combinedLines=allNarratives.trim().split(/\r*\n/).length;
					limit = 1000 * this.cols;
					entered = allNarratives.length;
					if(combinedLines > 1000){
						this.invalidMessage  = m.getLocalization("invalidLineError",[1000, combinedLines]);		
						
					}
					else if(entered > limit) {
						this.invalidMessage  = m.getLocalization("invalidFieldSizeError",[limit, entered]);
					}
				
			}
			else if(isSingleFieldValid) {				 
							
								if(singleLines > 800){
									this.invalidMessage  = m.getLocalization("invalidSinglelineLength",[800, singleLines]);					
								}
								else if(singleEntered > singleLimit) {
									this.invalidMessage  = m.getLocalization("invalidSingleFieldLength",[singleLimit, singleEntered]);
								}				
						}
			
			if(isSingleFieldValid && isCombinedValid)
			  {
			  	isSingleFieldValid=false;
			  }
		  if(!isSingleFieldValid && !isCombinedValid)
		  {
	...
	// function to validate single field row count and character count or combined field  row count and character count
	
	function _checkSingleCombined(combinedArray) {
		for(var i=0;i<combinedArray.length;i++){
			for(var j=i+1;j<combinedArray.length;j++){
					  if((combinedArray[i]!="") && (combinedArray[i]!="undefined") && (combinedArray[j]!="") && (combinedArray[j]!="undefined"))
						  {
						   	combined=true;											  
							   		break;
						 }
		...
		_onFormLoadSwift2018 : function () {
			...
			if(option === "EXISTING" || option === "BACK_TO_BACK"){
				m.dialog.show("CONFIRMATION", m.getLocalization("warningMsgCreateLC"), "", 
						"", "","","",function(){
					var screenURL = "/screen/LetterOfCreditScreen?option="+option+"&tnxtype=01";
...
dojo.require('misys.client.binding.trade.create_lc_swift_client');