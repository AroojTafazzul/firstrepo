...
@Component({
  selector: 'fcc-common-available-amount-export',
  templateUrl: './available-amount-export.component.html',
  styleUrls: ['../../../chart-balance.scss'],
  animations: [OPEN_CLOSE_ANIMATION]
})
export class AvailableAmountExportComponent implements OnInit {
  tabIndex: string;

  constructor(protected fccGlobalConstantService: FccGlobalConstantService,
  ...
  availableAmountExportForGridView: any[] = [];
  availableAmountExport: any[] = [];
  currencyList: any[] = [];
  ...
  availableAmountExportLabel = 'availableBalanceExport';
  ...
  availableAmountExportExists = false;
  showChart = true;
  showPie = true;
  showBar = false;
  ...
  this.dashboardService.getAvailableAmount(this.toCurrency)
    .subscribe(responseData => {
      if (responseData.errorMessage && responseData.errorMessage === 'SESSION_INVALID') {
        ...
        const availableAmountExportProductArray: string[] = ['BR', 'EL', 'SR'];
        this.availableAmountExport =
          this.outstandingBalanceService.arrangeData(responseData.availableAmount, availableAmountExportProductArray, this.toCurrency);
        this.availableAmountExport = this.outstandingBalanceService.sortData(this.availableAmountExport);
        this.availableAmountExportForGridView = this.availableAmountExport;
        ...
        this.availableAmountExport.forEach(
              value => {
        ...
        this.availableAmountExportForGridView.forEach(value => {
          value.totalAmount = this.currencySymbolDisplayEnabled ?
          this.commonService.getCurrencySymbolForDownloads(value.currency, value.totalAmount) : value.totalAmount;
        });
        if (labelArray.length !== 0) {
          this.availableAmountExportExists = true;
        }

        const legendNamePie = `${this.translateService.instant(this.availableAmountExportLabel)} ${
          this.translateService.instant(this.doughnutChart)}`;

        const legendNameBar = `${this.translateService.instant(this.availableAmountExportLabel)} ${
          this.translateService.instant(this.barChart)}`;

        const legendNameLogarithmic = `${this.translateService.instant(this.availableAmountExportLabel)} ${
...
deleteCards() {
    this.hideShowDeleteWidgetsService.availableAmountExportCardHideShow.next(true);
    this.hideShowDeleteWidgetsService.availableAmountExportCardHideShow.subscribe(res => {
