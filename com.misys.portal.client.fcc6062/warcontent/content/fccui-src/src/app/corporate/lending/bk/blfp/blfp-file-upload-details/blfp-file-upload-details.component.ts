...
@Component({
  selector: 'app-blfp-file-upload-details',
  templateUrl: './blfp-file-upload-details.component.html',
  styleUrls: ['./blfp-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: BlfpFileUploadDetailsComponent }]
})
export class BlfpFileUploadDetailsComponent extends BlfpProductComponent implements OnInit, OnDestroy {
  module = `${this.translateService.instant('blfpFileUploadDetails')}`;
  confirmMsg: string;
  ...
  @ViewChild(BlfpFileUploadDialogComponent) fileDialogReference;
  ngOnInit() {
    super.ngOnInit();
      ...
      if (response) {
        ...
        this.allFileExtensions.forEach(element => {
          ...
          if (this.validFileExtensions.indexOf(element.replace(/[\[\]"]/g, '').toUpperCase()) === -1) {
            this.validFileExtensions.push(element.replace(/[\[\]"]/g, '').toUpperCase()); //eslint-disable-line no-useless-escape
          }
        });
  ...
  initializeFormGroup() {
    const sectionName = FccGlobalConstant.BLFP_FILE_UPLOAD_DETAIL;
 ...
 onClickBrowseButton() {
    ...
    if (this.numberOfFile >= this.numberOfFilesRegex) {
      ...
      const dialogRef = this.dialogService.open(BlfpFileUploadDialogComponent, {
  ...
  saveFormObject() {
    this.stateService.setStateSection(
      FccGlobalConstant.BLFP_FILE_UPLOAD_DETAIL,
      this.form
    );
  }
  ...
  ngOnDestroy() {
    this.checkAttachments();
    this.stateService.setStateSection(FccGlobalConstant.BLFP_FILE_UPLOAD_DETAIL, this.form);
  }
