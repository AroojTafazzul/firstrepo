...
@Component({
  selector: 'app-ic-file-upload-details',
  templateUrl: './ic-file-upload-details.component.html',
  styleUrls: ['./ic-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: IcFileUploadDetailsComponent }]
})
export class IcFileUploadDetailsComponent extends IcProductComponent implements OnInit, OnDestroy {
  confirmMsg: string;
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
    ...
    super(emitterService, stateService, commonService, translateService, confirmationService,
      customCommasInCurrenciesPipe, searchLayoutService, utilityService, resolverService, uploadFile, dialogRef,
      currencyConverterPipe, icProductService);
    this.module = this.translateService.instant('icFileUploadDetails');
  }

  @ViewChild(IcFileUploadDialogComponent) public fileDialogReference;
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('icFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('icFileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'icFileUploadDetails';
  ...
  checkValidFileName(file: any) {
    ...
    const dialogRef = this.dialogService.open(IcFileUploadDialogComponent, {
