...
import { SgFileUploadDialogComponent } from '../sg-file-upload-dialog/sg-file-upload-dialog.component';

@Component({
  selector: 'app-sg-file-upload-details',
  templateUrl: './sg-file-upload-details.component.html',
  styleUrls: ['./sg-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: SgFileUploadDetailsComponent }]
})
export class SgFileUploadDetailsComponent extends SgProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
    ...
    this.module = this.translateService.instant('sgFileUploadDetails');
  }

  @ViewChild(SgFileUploadDialogComponent) public fileDialogReference;
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('sgFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('sgFileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'sgFileUploadDetails';
      let bankType;
      this.form = this.stateService.getSectionData(sectionName);
      if (this.tnxTypeCode === FccGlobalConstant.N002_INQUIRE) {
        bankType = this.stateService.getValue(FccGlobalConstant.SG_GENERAL_DETAILS,
  ...
  onClickBrowseButton() {
    ...
    const dialogRef = this.dialogService.open(SgFileUploadDialogComponent, {
  ...
  ngOnDestroy() {
    ...
    // }
    this.stateService.setStateSection('sgFileUploadDetails', this.form);
  }

  onClickNext() {
    this.saveFormObject();
    this.saveDraftService.changeSaveStatus('sgFileUploadDetails',
    this.stateService.getSectionData('sgFileUploadDetails'));
  }
