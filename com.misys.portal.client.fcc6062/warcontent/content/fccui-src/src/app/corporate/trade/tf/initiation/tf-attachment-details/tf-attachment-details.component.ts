...
@Component({
  selector: 'app-tf-attachment-details',
  templateUrl: './tf-attachment-details.component.html',
  styleUrls: ['./tf-attachment-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: TfAttachmentDetailsComponent }]
})
export class TfAttachmentDetailsComponent extends TfProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
    ...
    this.module = this.translateService.instant('tfAttachmentDetails');
  }
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('tfAttachmentDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('tfAttachmentDetails')
  ...
  initializeFormGroup() {
    const sectionName = 'tfAttachmentDetails';
  ...
  checkValidFileName(file: any) {
    ...
    if (this.numberOfFile >= this.numberOfFilesRegex) {
      this.patchFieldParameters(this.form.get(FccGlobalConstant.FILE_UPLOAD_TABLE), { hasError: true });
      this.patchFieldParameters(
        this.form.get(FccGlobalConstant.FILE_UPLOAD_TABLE),
        { message: `${this.translateService.instant("MaxFileReached")}` }
      );
      this.form.updateValueAndValidity();
      return;
    } else {
  ...
  ngOnDestroy() {
    // TODO :: Need to clean up code in this component.
    ...
    //   }
    this.stateService.setStateSection('tfAttachmentDetails', this.form);
  }

  onClickNext() {
    this.saveFormObject();
    this.saveDraftService.changeSaveStatus('tfAttachmentDetails',
    this.stateService.getSectionData('tfAttachmentDetails'));
  }
