...
@Component({
  selector: 'app-ft-trade-file-upload-details',
  templateUrl: './ft-trade-file-upload-details.component.html',
  styleUrls: ['./ft-trade-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: FtTradeFileUploadDetailsComponent }]
})
export class FtTradeFileUploadDetailsComponent extends FtTradeProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              public downloadFile: CommonService, protected prevNextService: PrevNextService,
              ...
              protected currencyConverterPipe: CurrencyConverterPipe, protected ftTradeProductService: FtTradeProductService) {
                super(eventEmitterService, stateService, commonService, translateService, confirmationService, customCommasInCurrenciesPipe,
                  searchLayoutService, utilityService, resolverService, fileListSvc, dialogRef, currencyConverterPipe,
                  ftTradeProductService);
                this.module = this.translateService.instant('ftTradeFileUploadDetails');
               }

  @ViewChild(FtTradeUploadDialogComponent) public fileDialogReference;
  ...
  ngOnInit(): void {
    ...
    if (this.stateService.getSectionData('ftTradeFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('ftTradeFileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'ftTradeFileUploadDetails';
  ...
  checkValidFileName(file: any) {
    ...
    const dialogRef = this.dialogService.open(FtTradeUploadDialogComponent, {
