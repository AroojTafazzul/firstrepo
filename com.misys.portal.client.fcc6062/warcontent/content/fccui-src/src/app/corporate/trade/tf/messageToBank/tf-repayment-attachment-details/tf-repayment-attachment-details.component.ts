...
@Component({
  selector: 'app-tf-repayment-attachment-details',
  templateUrl: './tf-repayment-attachment-details.component.html',
  styleUrls: ['./tf-repayment-attachment-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: TfRepaymentAttachmentDetailsComponent }]
})
export class TfRepaymentAttachmentDetailsComponent extends TfProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              protected dialogRef: DynamicDialogRef, protected fileHandlingService: FileHandlingService,
    ...
    super(emitterService, stateService, commonService, translateService, confirmationService,
      ...
      tfProductService);
    this.module = this.translateService.instant('tfRepayAttachmentDetails');
  }
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('tfRepayAttachmentDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('tfRepayAttachmentDetails')
...
initializeFormGroup() {
  const sectionName = 'tfRepayAttachmentDetails';
...
ngOnDestroy() {
  ...
  //   }
  this.stateService.setStateSection('tfRepayAttachmentDetails', this.form);
}

onClickNext() {
  this.saveFormObject();
  this.saveDraftService.changeSaveStatus('tfRepayAttachmentDetails',
  this.stateService.getSectionData('tfRepayAttachmentDetails'));
}
