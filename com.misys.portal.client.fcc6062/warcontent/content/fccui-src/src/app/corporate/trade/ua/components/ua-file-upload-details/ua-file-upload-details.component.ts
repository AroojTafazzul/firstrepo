...
import { UaFileUploadDailogComponent } from '../ua-file-upload-dailog/ua-file-upload-dailog.component';
...
@Component({
  selector: 'app-ua-file-upload-details',
  templateUrl: './ua-file-upload-details.component.html',
  styleUrls: ['./ua-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: UaFileUploadDetailsComponent }]
})
export class UaFileUploadDetailsComponent extends UaProductComponent implements OnInit {
  confirmMsg: string;
  ...
  this.module = this.translateService.instant(FccGlobalConstant.UA_FILE_UPLOAD_DETAIL);
}

@ViewChild(UaFileUploadDailogComponent) public fileDialogReference;
...
ngOnInit() {
  ...
  if (this.stateService.getSectionData('UaFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
    this.renderBrowseButton(this.stateService.getSectionData('UaFileUploadDetails')
  ...
  initializeFormGroup() {
    const sectionName = FccGlobalConstant.UA_FILE_UPLOAD_DETAIL;
    this.form = this.stateService.getSectionData(sectionName, FccGlobalConstant.PRODUCT_BR);
...
checkValidFileName(file: any) {
  ...
  const dialogRef = this.dialogService.open(UaFileUploadDailogComponent, {
