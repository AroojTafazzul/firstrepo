...
@Component({
  selector: 'fcc-trade-file-upload-details',
  templateUrl: './file-upload-details.component.html',
  styleUrls: ['./file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: FileUploadDetailsComponent }]
})
export class FileUploadDetailsComponent extends LcProductComponent implements OnInit, OnDestroy {
  confirmMsg: string;
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              protected resolverService: ResolverService, protected dialogRef: DynamicDialogRef, protected route: ActivatedRoute,
    ...
    super(emitterService, stateService, commonService, translateService, confirmationService, customCommasInCurrenciesPipe,
          searchLayoutService, utilityService, resolverService, fileListSvc, dialogRef, currencyConverterPipe, lcProductService);
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('fileUploadDetails').get('attachments').value[ FccGlobalConstant.DOC_ID ] !== undefined){
    this.renderBrowseButton(this.stateService.getSectionData('fileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'fileUploadDetails';
      let bankType;
      this.form = this.stateService.getSectionData(sectionName);
      if (this.tnxTypeCode === FccGlobalConstant.N002_INQUIRE) {
        bankType = this.stateService.getValue(FccGlobalConstant.LC_MESSAGE_GENERAL_DETAILS,
        ...
        if (this.isFileSizeValid && this.isFileExtValid) {
          ...
          await this.autoUploadFile.uploadAttachments(this.fileModel.file, this.fileModel.title, lcNumber, eventId).toPromise().then(
            response => {
              ...
              if (this.form.get ('noOfAttachments')){
  ...
  checkValidFileExtensionfn(fileExt): boolean {
    ...
    if (this.commonService.isnonEMptyString(this.stateService.isStateSectionSet('applicantBeneficiaryDetails', false))) {
      entityDetails = this.stateService.getControl('applicantBeneficiaryDetails', 'applicantEntity');
