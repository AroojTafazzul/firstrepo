...
<xsl:stylesheet
		...
		exclude-result-prefixes="localization loanIQ utils loanutils defaultresource stringescapeutils tools jetspeedresources">
...
<xsl:variable name="legal_text_value" select="loanIQ:getLegalText($language,issuing_bank/abbv_name,facility_type,'DRAWDOWN')"/>
		<script>
				dojo.ready(function(){			
...
<xsl:template name="js-imports">
	<xsl:call-template name="common-js-imports">
		<xsl:with-param name="binding">misys.binding.loan.create_ln</xsl:with-param>
		...
		<xsl:with-param name="content">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:call-template name="hidden-field">
						<xsl:with-param name="name">ln_amt_unsigned</xsl:with-param>
										...
										<xsl:call-template name="fieldset-wrapper">
											<xsl:with-param name="content">			
												...
												<xsl:variable name="void"
													select="java:add($arrayList1, concat('', loanutils:convertApiDateToFccDate($interestDetails/today_date,$language)))" />
												<xsl:variable name="void"
    ...
    <xsl:call-template name="common-hidden-fields">
     ...
     <xsl:with-param name="additional-fields">
	    ...
	    <xsl:call-template name="hidden-field">
				...
				<xsl:with-param name="value">LNDR</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="hidden-field">
					...
					<div style="width:100%;height:100%;" class="widgetContainer clear">
						<div dojoType="dojo.data.ItemFileWriteStore" jsId="storeInterestDetails" requestMethod="post">
							<xsl:attribute name="url">								
										...
										<xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/AjaxScreen/action/GetJSONData?listdef=loan/listdef/customer/LN/inquiryLNInterestCycleInfo.xml&amp;borefid=<xsl:value-of select='bo_ref_id'/>&amp;loan_ccy=<xsl:value-of select='ln_cur_code'/>
