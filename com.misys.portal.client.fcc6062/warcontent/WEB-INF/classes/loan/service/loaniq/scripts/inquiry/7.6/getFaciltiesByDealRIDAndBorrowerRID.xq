...
{for $dealobj in LIQ.BO.GetBOByRID("Deal", $dealId) return
       <deal>        
              ...
              <currency>{$dealobj/currency}</currency>
       </deal>
       <borrower>
...
{for $facobj in $dealobj/facilities/Facility[LIQ.XQS.OR(LIQ.XQS.OR(LIQ.XQS.EQUAL(@displayStatus, "Active"),LIQ.XQS.EQUAL(@displayStatus, "Expired")),LIQ.XQS.EQUAL(@displayStatus, "Matured"))] return
...
LIQ.XQ.IFTRUE($facobj/typeObject/isRevolver,"REV","TERM")))}
			...
			<facAvailAmtWithPendLoans>{$facobj/potentialAvailableToDrawAmount}</facAvailAmtWithPendLoans>
