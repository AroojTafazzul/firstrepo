...
<ln_tnx_record
  ...
  <bo_deal_name>{$drawdown/eventOwner/dealName}</bo_deal_name>
  <bo_facility_id>{$drawdown/eventOwner/facilityId}</bo_facility_id>
  <bo_facility_name>{$drawdown/eventOwner/facilityName}</bo_facility_name>
  ...
  <risk_type>{$drawdown/eventOwner/riskType}</risk_type>
  <tnx_amt>{$drawdown/eventOwner/actualAmountAbs}</tnx_amt>
  <tnx_cur_code>{$drawdown/eventOwner/currency}</tnx_cur_code>
  ...
  {for $misCode in $drawdown/eventOwner/facility/misCodes[LIQ.XQS.EQUAL(@type,"PORTL")] return
  ...
  <sublimit_name>{LIQ.XQ.IFTRUE(LIQ.XQS.ISNOTNULL($drawdown/eventOwner/getSublimit), $drawdown/eventOwner/getSublimit/getName, "")}</sublimit_name>
  {LIQ.XQ.UTIL.SET("rid",$drawdown/eventOwner/id)}
   ...
   <transaction_desc>{$tran/transactionDescription}</transaction_desc>
   <transactions> 
		{for $o in $tran/sortedTransactions[LIQ.BO.ContainsCode(@type,"PPYMT")] return 
		 <principal_payment>
			 ...
			 <ln_liab_amt>{$o/currentAmount}</ln_liab_amt>
