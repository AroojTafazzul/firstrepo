declare function getFacilityDetailsByFacilityRIDandBorrowerRID($facility_id,$borrower_id){
  <liqInfo>
{LIQ.XQ.UTIL.SET("idPredicate",LIQ.XQ.UTIL.ADD(LIQ.XQ.UTIL.ADD("FAC_PID_FACILITY = '",$facility_id),"'"))}
{for $f in /Facility[LIQ.SQL.PREDICATE($idPredicate)]
return
...
<isin>{$f/fac/isin}</isin>
  ...
  <globalAvailableAmount>{$f/fac/globalAvailToDrawAmount}</globalAvailableAmount>
	...
	<facAvailAmtWithPendLoans>{$f/fac/potentialAvailableToDrawAmount}</facAvailAmtWithPendLoans>
...
<scheduleItems>
	{for $schdeule in $f/fac/getAmortizationSchedule/getScheduleItems return
				{for $item in $schdeule return
				<schedule>
						<item>{$item/itemNumber}</item>
						<type>{$item/scheduledIndicatorAsString}</type>
						<percent_rate>{LIQ.XQ.UTIL.MUL($item/facilityScheduleItemPercentage,100)}</percent_rate>
						<amount>{$item/amount}</amount>
		             	<date>{$item/effectiveDate}</date>
		             	<remainingAmount>{$item/remainingPrincipalBalance}</remainingAmount>
		             	<hostBankAmount>{$item/hostNetAmount}</hostBankAmount>
						<hostBankRemainingAmount>{$item/hostRemainingNetAmount}</hostBankRemainingAmount>
		             	<billedAmount>{$item/getPrincipalBilledAmount}</billedAmount>
	            </schedule>
				}
		}
</scheduleItems>
