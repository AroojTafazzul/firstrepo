...
<xsl:stylesheet 
	...
	<!--
	   ...
	   All Rights Reserved. 
	-->
	<!-- Transform MT798<755> (Notification of Advice of Reimbursement or Payment) into lc_tnx_record or el_tnx_record -->
	...
	<xsl:template match="/">
		...
		<xsl:variable name="productCode" select="service:detectProductCodeFromBoRefId(MeridianMessages/MeridianMessage[ExternalMessageType = 'MT755']/DocCreditNo, 'LC,EL')"/>
		<xsl:choose>
			<xsl:when test="$productCode='LC'"><xsl:apply-templates select="MeridianMessages/MeridianMessage[ExternalMessageType = 'MT755']" mode="LC"/></xsl:when>
			<xsl:when test="$productCode='EL'"><xsl:apply-templates select="MeridianMessages/MeridianMessage[ExternalMessageType = 'MT755']" mode="EL"/></xsl:when>
	    </xsl:choose>
	</xsl:template>
    ...
    <xsl:template match="MeridianMessage" mode="LC">
		...
		<lc_tnx_record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.neomalogic.com/gtp/interfaces/xsd/lc.xsd">
			...
			<xsl:if test="../MeridianMessage[ExternalMessageType = 'MT755']/PrincipalAmount != ''">
				<tnx_cur_code><xsl:value-of select="substring(../MeridianMessage[ExternalMessageType = 'MT755']/PrincipalAmount,1,3)"/></tnx_cur_code>
				<xsl:variable name="amount">
					<xsl:call-template name="format_amount"><xsl:with-param name="amount"><xsl:value-of select="substring(../MeridianMessage[ExternalMessageType = 'MT755']/PrincipalAmount,4)"/></xsl:with-param>
					</xsl:call-template>		
				</xsl:variable>
				<tnx_amt><xsl:value-of select="$amount"/></tnx_amt>
			</xsl:if>
			
			<xsl:if test="../MeridianMessage[ExternalMessageType = 'MT755']/DateOfIssue != ''">
				<iss_date><xsl:call-template name="format_date"><xsl:with-param name="input_date" select="substring(../MeridianMessage[ExternalMessageType = 'MT755']/DateOfIssue,1,6)"/></xsl:call-template></iss_date>
			</xsl:if>
			...
			<bo_comment>
				<xsl:text>
				This is a notification of advice of Reimbursement or Payment.
				See the attached SWIFT message for all details.
				</xsl:text>
				
				<xsl:text>

				DETAILS COMMISSION AND CHARGES:
				</xsl:text>
				<xsl:if test="DetailsCommissionAndCharges != ''">
				    <xsl:call-template name="backslashn_replace">
						<xsl:with-param name="input_text">
							<xsl:value-of select="DetailsCommissionAndCharges"/>
						</xsl:with-param>
					 </xsl:call-template>	
				</xsl:if>
				<xsl:text>
			...
			<!-- <xsl:apply-templates select="DetailsCommissionAndCharges"/> -->
	...
	<xsl:template match="MeridianMessage" mode="EL">
		<xsl:variable name="cust-reference-enabled">
   		...
   		<xsl:variable name="cust-reference" select="service:retrieveCustomerReferenceFromBoRefId(DocCreditNo, 'EL')"/>
			<el_tnx_record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.neomalogic.com/gtp/interfaces/xsd/lc.xsd">	
			<brch_code>00001</brch_code>
			<bo_ref_id><xsl:value-of select="DocCreditNo"/></bo_ref_id>
			<tnx_type_code>15</tnx_type_code>
			<sub_tnx_type_code/>
