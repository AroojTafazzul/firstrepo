 ...
 <div id ="fx-section"> 
    ...
    <xsl:with-param name="content">
	    ...
	    <div id="fx-board-rate-type">
		    ...
		    <!-- Added condition here for finance repayment (A4) -->
		    <xsl:if test="sub_tnx_type_code[.!='A4'] or (product_code[.!='IP'] and tnx_type_code[.!='85'])">
	    ...
	    <div>
	    	<!-- From create_fx js, contract number label, edit field and Utilize amount label and edit fields created and pushed in to this div -->
		    <div id="fx-contracts-type"/>
		    <!-- From create_fx js, one static label creates and push in to this div -->		  
			<div id="contractMsgDiv"/>	 
			<!-- From create_fx js, Total Utilize amount label and edit fields creates and push in to this div -->
	    ...
	    <script>
	    	...
	    	dojo.subscribe("ready",function(){
				...
				dojo.mixin(misys._config, {
						fxParamData:{
							<xsl:for-each select="banks_fx_rates/fx_bank_params/fx_product">
									"<xsl:value-of select="sub_product_name"/>" : {
										fxParametersData :{
   ...
   <xsl:template name="fx_types">
    <xsl:call-template name="multioption-inline-wrapper">
	      ...
	      <xsl:with-param name="content">
			   ...
			   <xsl:if test="sub_product_code [.!='BANKB']">
        	...
        	<xsl:with-param name="content">
            	<xsl:call-template name="column-wrapper">
			     ...
			     <!-- Exchange Rate Message -->
			   	...
			   	<xsl:choose>
			   	    <xsl:when test="sub_tnx_type_code[.='A4'] or (product_code[.='IP'] and tnx_type_code[.='85'])">		        
		             	<xsl:call-template name="column-wrapper">
							<xsl:with-param name="content">
		               			<xsl:call-template name="currency-field">
						    		<xsl:with-param name="label">XSL_FX_EQUIVALENT_AMT</xsl:with-param>
						    		<xsl:with-param name="product-code">fx_exchange_rate</xsl:with-param>
