...
<xsl:stylesheet 
    ...
    exclude-result-prefixes="localization security securityCheck utils">
 ...
 <xsl:param name="companyCaseSensitive">false</xsl:param>
  ...
  <script>
   dojo.ready(function(){
	   ...
	   dojo.mixin(misys._config, {
	   	...
	   	bankBusinessDate: '<xsl:value-of select="utils:getBankBusinessDate($rundata)"/>',
	   	onlineHelpUrl: misys.getServletURL('/screen/OnlineHelpScreen?helplanguage=<xsl:value-of select="$language"/>&amp;accesskey=<xsl:value-of select="$override-help-access-key"/>'),
 ...
 <xsl:template name="currency-template">
 	<xsl:param name="field_value"><xsl:value-of select="./base_cur_code"/></xsl:param>
 	<xsl:call-template name="input-field">
    	...
    	<xsl:with-param name="value"><xsl:value-of select="$field_value"/></xsl:with-param>
 ...
 <xsl:param name="password_autogenerated">true</xsl:param>
 <xsl:param name="password_display_disabled">false</xsl:param>
  <xsl:if test="company_id[.=''] or (tnx_type_code[.=01] and (tnx_stat_code[.=01] or tnx_stat_code[.=02] or tnx_stat_code[.=54]))">
	  ...
	  <xsl:when test="$displaymode = 'edit'">
		   <xsl:call-template name="checkbox-field">
			   	...
			   	<xsl:with-param name="checked">
			   			<xsl:choose>
							<xsl:when test="create_admin[.='Y']">Y</xsl:when>
...
<xsl:template name="main-details">
	...
	<xsl:param name="addressFieldExtended">false</xsl:param>
  <xsl:call-template name="fieldset-wrapper">
   ...
   <xsl:with-param name="content">
		...
		<xsl:when test="company_id[.='']">
	    	<xsl:call-template name="input-field">
	     		<xsl:with-param name="label">XSL_JURISDICTION_ABREVIATED_NAME</xsl:with-param>
	      		...
	      		<xsl:with-param name="regular-expression"><xsl:value-of select="defaultresource:getResource('COMPANYNAME_VALIDATION_REGEX')"/></xsl:with-param>
	      		<!-- "required" indicates that this attribute is a mandatory one -->
	      		<xsl:with-param name="required">Y</xsl:with-param>
	      		<xsl:with-param name="uppercase">
		      		<xsl:choose>
						<xsl:when test="$companyCaseSensitive = 'true'">N</xsl:when>
	    ...
	    <xsl:otherwise>
 		...
 		<!-- Name -->    
	   	<xsl:call-template name="input-field">
     		<xsl:with-param name="label">XSL_JURISDICTION_NAME_COMPANY_MC</xsl:with-param>
      		<xsl:with-param name="name">name</xsl:with-param>
      		<xsl:with-param name="maxsize"><xsl:value-of select="defaultresource:getResource('MAXIMUM_ALLOWED_COMP_NAME')"/></xsl:with-param>
      		<xsl:with-param name="required">Y</xsl:with-param>
     	</xsl:call-template>
     	...
     	<xsl:call-template name="address-details">
     		...
     		<xsl:with-param name="addressFieldExtended"><xsl:value-of select="$addressFieldExtended"/></xsl:with-param>
     	</xsl:call-template>
...
<!-- Treasury Branch Reference -->
		<xsl:if test="type[.='01' or .='02']">
		<xsl:choose>
			<xsl:when test="$displaymode='edit'">
				<xsl:if test="count(treasury_branch_references/branch_reference)>0">
					<xsl:call-template name="select-field">
				      <xsl:with-param name="label">XSL_TREASURY_BRANCH_REFERENCE</xsl:with-param>
				      <xsl:with-param name="name">treasury_branch_reference</xsl:with-param>
				      <xsl:with-param name="value"><xsl:value-of select="treasury_branch_reference"/></xsl:with-param>
				      <xsl:with-param name="options">
				      <xsl:call-template name="treasury_branch_reference_options"/>
				      </xsl:with-param>
				    </xsl:call-template>
			    </xsl:if>	
			</xsl:when>
			<xsl:otherwise>
			<xsl:variable name="treasury_branch_reference"><xsl:value-of select="treasury_branch_reference"></xsl:value-of></xsl:variable>
			  ...
			  <xsl:variable name="parameterId">C027</xsl:variable>
			  <xsl:if test="$treasury_branch_reference !=''">
			  <xsl:call-template name="input-field">
			      <xsl:with-param name="label">XSL_TREASURY_BRANCH_REFERENCE</xsl:with-param>
			      <xsl:with-param name="name">treasury_branch_reference</xsl:with-param>
			      <xsl:with-param name="value">
		      		<xsl:value-of select="treasury_branch_reference"/> - <xsl:value-of select="localization:getCodeData($language,'*',$productCode,$parameterId,$treasury_branch_reference)"/>
				  </xsl:with-param>
		 	  </xsl:call-template>
		 	  </xsl:if>
			</xsl:otherwise>
	...
	<xsl:if test="defaultresource:getResource('LIQUIDITY_BACK_OFFICE_KTP') = 'false' and type[.='01' or .='02']">
	<xsl:choose>
		<xsl:when test="$displaymode='edit'">
			<xsl:if test="count(liquidity_branch_reference/fi_reference)>=0">
				<xsl:call-template name="select-field">
					<xsl:with-param name="label">XSL_LIQUIDITY_FINANCIAL_INSTITUTION</xsl:with-param>
					<xsl:with-param name="name">liquidity_branch_reference</xsl:with-param>
					<xsl:with-param name="value"><xsl:value-of select="liquidity_branch_reference"/></xsl:with-param>
					<xsl:with-param name="options">
						<xsl:call-template name="liquidity_branch_reference_options"/>
					</xsl:with-param>
				</xsl:call-template>
	           <script>  
			    	dojo.ready(function(){
			    		misys._config = misys._config || {};
			    		dojo.mixin(misys._config, {
		    				liquidityBranchReferences : [
		    				<xsl:if test="count(liquidity_branch_reference/fi_reference)>0">
								<xsl:for-each select="liquidity_branch_reference/fi_reference">
								{ 
									fi_code: "<xsl:value-of select="fi_code"/>",
									in_use: "<xsl:value-of select="in_use"/>"
								},
								</xsl:for-each>
							</xsl:if>
							]		
		...
		<xsl:otherwise>
		<xsl:variable name="liquidity_branch_reference"><xsl:value-of select="liquidity_branch_reference"/></xsl:variable>
		  <xsl:if test="$liquidity_branch_reference !=''">
			  <xsl:call-template name="input-field">
			      <xsl:with-param name="label">XSL_LIQUIDITY_FINANCIAL_INSTITUTION</xsl:with-param>
			      <xsl:with-param name="name">liquidity_branch_reference</xsl:with-param>
			      <xsl:with-param name="value">
		      		<xsl:value-of select="liquidity_branch_reference"/>-<xsl:value-of select="utils:retrieveParameter('P327', $liquidity_branch_reference)"/>
				  </xsl:with-param>
		 	  </xsl:call-template>
	 	  </xsl:if>
		</xsl:otherwise>
 ...
 <xsl:template name="address-details">
 	...
 	<xsl:param name="addressFieldExtended"/>
	...
	<xsl:choose>
     ...
     <xsl:call-template name="tabgroup-wrapper">
      ...
      <xsl:with-param name="tabgroup-height">165px</xsl:with-param>
      ...
      <xsl:with-param name="tab0-content">
         ...
         <xsl:with-param name="label">
	         <xsl:choose>
	         	<xsl:when test="$addressFieldExtended = 'false'">XSL_JURISDICTION_ADDRESS_STREET_NAME</xsl:when>
	         	<xsl:otherwise>XSL_JURISDICTION_ADDRESS</xsl:otherwise>
	         </xsl:choose>
         </xsl:with-param>
         ...
         <xsl:with-param name="maxsize">35</xsl:with-param>
       	...
       	<xsl:call-template name="input-field">
       	<xsl:with-param name="label">
	       	<xsl:if test="$addressFieldExtended = 'false'">XSL_JURISDICTION_ADDRESS_TOWN_NAME</xsl:if>
       	</xsl:with-param>
         ...
         <xsl:with-param name="maxsize">35</xsl:with-param>
       	...
       	<xsl:call-template name="input-field">
       	<xsl:with-param name="label">
       	 <xsl:if test="$addressFieldExtended = 'false'">XSL_JURISDICTION_ADDRESS_COUNTRY_SUB_DIVISION</xsl:if>
         </xsl:with-param>
         ...
         <xsl:with-param name="maxsize">35</xsl:with-param>
       	...
       	<xsl:if test="$addressFieldExtended = 'true'">
	       	<xsl:call-template name="input-field">
	         <xsl:with-param name="name">county</xsl:with-param>
	         <xsl:with-param name="id"><xsl:value-of select="$prefix"/>_county</xsl:with-param>
	         ...
	         <xsl:with-param name="id"><xsl:value-of select="$prefix"/>_country_name</xsl:with-param>
	         <xsl:with-param name="size">40</xsl:with-param>
	         <xsl:with-param name="maxsize">40</xsl:with-param>
	       	</xsl:call-template>
       	</xsl:if>
       	 ...
       	 <xsl:call-template name="input-field">
       	 	<xsl:with-param name="label">
       	 	<xsl:if test="$addressFieldExtended = 'false'">XSL_JURISDICTION_ADDRESS_POST_CODE</xsl:if>
            </xsl:with-param>
...
<!-- 
 ...
 -->
  <xsl:template name="charge-account-address">
 	...
 	<xsl:param name="ownerId">02</xsl:param>
 	<xsl:param name="email_mandatory">false</xsl:param> 
 	<xsl:variable name="customerEntitiesChargeAccountEnabled" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_ENABLED')"></xsl:variable>
 	<xsl:variable name="customerEntitiesChargeAccountEnabledAndMandatory" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_MANDATORY')"></xsl:variable>
    <xsl:variable name ="chargeAccountAddressRequired">
       <xsl:choose>
        <xsl:when test="$customerEntitiesChargeAccountEnabledAndMandatory = 'true' and $isCounterparty != 'true'">
     ...
     <!-- Tabgroup #0 -->
     <xsl:choose>
     <xsl:when test="$customerEntitiesChargeAccountEnabled = 'true' and not($companyType = '02' or $ownerId ='02')">
     <xsl:call-template name="tabgroup-wrapper">
      <!-- Tab 0_0 - SWIFT Address  -->
      <xsl:with-param name="legend-type">indented-header</xsl:with-param>
    ...
    <xsl:otherwise>
     <xsl:call-template name="tabgroup-wrapper">
      ...
      <!-- Tab 0_1 - Postal Address  -->
      <xsl:with-param name="tab0-label">XSL_ENTITY_ALTERNATIVE_ADDRESS</xsl:with-param>
      <xsl:with-param name="tab0-content">
       	<xsl:call-template name="input-field">
         <xsl:with-param name="label">XSL_JURISDICTION_ADDRESS</xsl:with-param>
         <xsl:with-param name="name">alternative_address_line_1</xsl:with-param>
      ...
      <!-- Tab 0_2 - Postal Address  -->
      <xsl:with-param name="tab1-label">XSL_JURISDICTION_TAB_POSTAL_ADDRESS</xsl:with-param>
      <xsl:with-param name="tab1-content">
       	<xsl:call-template name="input-field">
         <xsl:with-param name="label">XSL_JURISDICTION_ADDRESS_STREET_NAME</xsl:with-param>
         <xsl:with-param name="name">street_name</xsl:with-param>
   ...
   <xsl:call-template name="country-field">
	...
	<xsl:call-template name="input-field">
	  ...
	  <xsl:with-param name="required">
		<xsl:choose>
			<xsl:when test="$email_mandatory = 'true'">Y</xsl:when>
			<xsl:otherwise>N</xsl:otherwise>
		</xsl:choose>
		</xsl:with-param>
		<xsl:with-param name="value">
	...
	<xsl:if test="$displaymode='view' and contact_email[.!='']">
		<xsl:call-template name="fieldset-wrapper">
	 	 <xsl:with-param name="content">
			<div class="label">
				<span class="field">
		...
		<div class="field">
			...
			<span class="required-field-symbol">*</span>
				<xsl:choose>
					    ...
					    <xsl:when test="starts-with(name(),'access_opened_prog_04')">
						<xsl:value-of select="localization:getGTPString($language, 'XSL_JURISDICTION_COUNTERPARTY_PAYMENT_CONDITIONS_04')"/>
					</xsl:when>
					<xsl:when test="starts-with(name(),'access_opened_prog_05')">
						<xsl:value-of select="localization:getGTPString($language, 'XSL_JURISDICTION_COUNTERPARTY_PAYMENT_CONDITIONS_05')"/>
					</xsl:when>
					<xsl:when test="starts-with(name(),'access_opened_prog_06')">
						<xsl:value-of select="localization:getGTPString($language, 'XSL_JURISDICTION_COUNTERPARTY_PAYMENT_CONDITIONS_06')"/>
					</xsl:when>
				</xsl:choose>
			</span>
			<div class="content">
				<xsl:call-template name="textarea-field">
			         <xsl:with-param name="name">
			         	<xsl:choose>
				         		...
				         		<xsl:when test="starts-with(name(),'access_opened_prog_04')">conditions_04</xsl:when>
				         		<xsl:when test="starts-with(name(),'access_opened_prog_05')">conditions_05</xsl:when>
				         		<xsl:when test="starts-with(name(),'access_opened_prog_06')">conditions_06</xsl:when>
			         	</xsl:choose>
			         </xsl:with-param>
...
<xsl:template name="linking-fields">
		<div class="widgetContainer">
			<xsl:call-template name="hidden-field">
  			...
  			<xsl:call-template name="hidden-field">
   				<xsl:with-param name="name">redirect</xsl:with-param>
   				<xsl:with-param name="value" select="$redirect"/>
  			</xsl:call-template>
  			<xsl:call-template name="hidden-field">
   				<xsl:with-param name="name">queryString</xsl:with-param>
   				<xsl:with-param name="value" select="$queryString"/>
  			</xsl:call-template>
		</div>
</xsl:template>

<!-- Dropdown list to select psml templates for a company -->
<xsl:template name="psml-template-field">
	<xsl:choose>
		<xsl:when test="$displaymode='edit'">
			<xsl:call-template name="select-field">
				<xsl:with-param name="label">XSL_JURISDICTION_PSML_TEMPLATE</xsl:with-param>
				<xsl:with-param name="name">psml_template</xsl:with-param>
				<xsl:with-param name="fieldsize">large</xsl:with-param>
				<xsl:with-param name="required">Y</xsl:with-param>
				<xsl:with-param name="options">
					<xsl:for-each select="psml_templates/template">
						<option>
		...
		<xsl:otherwise>
			<xsl:if test = "psml_template[.!='']">
				<xsl:call-template name="input-field">
					<xsl:with-param name="label">XSL_JURISDICTION_PSML_TEMPLATE</xsl:with-param>
					<xsl:with-param name="value"><xsl:value-of select="psml_template"/></xsl:with-param>
				</xsl:call-template>
			</xsl:if>
		</xsl:otherwise>
	</xsl:choose>
