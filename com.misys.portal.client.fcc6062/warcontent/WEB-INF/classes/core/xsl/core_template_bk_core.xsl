...
<!-- 
########################################################## 
Templates for Modify BK Template 
...
<xsl:stylesheet version="1.0"
	...
	<!-- Global Parameters. These are used in the imported XSL, and to set global params in the JS -->
	...
	<xsl:template name="realform">
		<xsl:call-template name="form-wrapper">
			...
			<xsl:with-param name="action">
				<xsl:choose>
					<xsl:when test="$nextscreen != ''">
						<xsl:value-of select="$nextscreen" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$realform-action" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:with-param>
			<xsl:with-param name="content">
				<div class="widgetContainer">
				 ...
				 <xsl:call-template name="hidden-field">
					...
					<xsl:call-template name="hidden-field">
						...
						<xsl:with-param name="value">TEMPLATE</xsl:with-param>
					</xsl:call-template>
					<xsl:call-template name="hidden-field">
