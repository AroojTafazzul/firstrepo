...
<!--  <xsl:param name="realform-action"><xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/CustomerSystemFeaturesScreen</xsl:param>-->
  ...
  <xsl:template match="/">
	<xsl:apply-templates select="bill_payee"/>
  </xsl:template>
  
  <xsl:template match="bill_payee">
  <xsl:variable name="_option"><xsl:value-of select="$option"/></xsl:variable>
   <!-- Loading message  -->
 ...
 <xsl:template name="account-details">
    ...
    <xsl:call-template name="fieldset-wrapper">
    	<xsl:with-param name="legend">XSL_SYSTEMFEATURES_BILL_PAYEE_DETAILS</xsl:with-param>
    	<xsl:with-param name="content">
			...
			<!-- Reference -->
			<xsl:call-template name="input-field">
				<xsl:with-param name="label">XSL_SYSTEMFEATURES_BILL_REFERENCE</xsl:with-param>
				<xsl:with-param name="name">client_reference</xsl:with-param>
				<xsl:with-param name="size">34</xsl:with-param>
