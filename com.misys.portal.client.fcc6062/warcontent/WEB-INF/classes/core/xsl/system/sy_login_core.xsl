...
<xsl:stylesheet 
    ...
    xmlns:version="xalan://com.misys.portal.Version"
	...
	<xsl:template match="/">
		...
		<xsl:call-template name="form-wrapper">
			...
			<xsl:with-param name="onsubmit">
    			<xsl:if test="defaultresource:getResource('ENABLE_CLIENT_SIDE_ENCRYPTION') = 'true'">return misys.encryptBeforeSubmit();</xsl:if>
			</xsl:with-param> 
	...
	<xsl:template name="js-imports">
		...
		dojo.ready(function(){
			...
			setTimeout(function(){
					var languageSelection = dijit.byId("languageSelection");
					if(languageSelection)
					{
						dojo.attr( dojo.byId('languageSelection'), 'aria-describedby', 'dropdown-instructions');
					}					
				},100);				
		});
			...
			<div id="loginLanguages">
				...
				<xsl:when test="defaultresource:getResource('LANGUAGE_FLAGS_DISPLAY_ENABLED') = 'true' ">
					<div id="languageSelectionContainer" class="widgetContainer">
							...
							<div id="dropdown-instructions" class="sr-only"><xsl:value-of select="localization:getGTPString($language, 'A11Y_DROPDOWN_INST')"/></div>
					<div id="languageSelection" class="dijit dijitReset dijitInlineTable dijitLeft small dijitTextBox dijitComboBox" dojoType="dijit.form.Select">
				        	...
				        	<span tab-index="-1">
								...
								<img>
									<xsl:attribute name="src"><xsl:value-of select="$contextPath"/>/content/images/flags/login/<xsl:value-of select="."/>.png?v=<xsl:value-of select="version:getVersion()"/></xsl:attribute>
