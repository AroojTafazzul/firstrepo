...
<xsl:stylesheet 
 ...
 <xsl:template name="js-imports">
  <xsl:call-template name="system-common-js-imports">
   ...
   <xsl:with-param name="binding">misys.binding.system.entity_mc_interface</xsl:with-param>
	...
	<xsl:variable name="current"><xsl:value-of select="static_company/language"/></xsl:variable>
  	<xsl:call-template name="fieldset-wrapper">
   		...
   		<xsl:with-param name="content">
			...
			<xsl:with-param name="id">retrieve_bwcif</xsl:with-param>
			<xsl:with-param name="content">
		       (<a>
		         <xsl:attribute name="href">javascript:void(0)</xsl:attribute>
		         <xsl:attribute name="onclick">javascript:misys.getCIFDetails('cif');</xsl:attribute>
		         <xsl:attribute name="title"><xsl:value-of select="localization:getGTPString($language, 'XSL_RETRIEVE_BWCIF')"/></xsl:attribute>
		         <xsl:value-of select="localization:getGTPString($language, 'XSL_RETRIEVE_BWCIF')"/>
		        </a>)
        	</xsl:with-param>
       	 </xsl:call-template>
       	 </xsl:if>
	      ...
	      <xsl:call-template name="input-field">
		    <xsl:with-param name="name">second_entity_name</xsl:with-param>
		     <!--<xsl:with-param name="readonly">Y</xsl:witfoh-param>
		    --><xsl:with-param name="maxsize">35</xsl:with-param>
	      </xsl:call-template>
	   </xsl:with-param>
	 </xsl:call-template>
 ...
 <xsl:template name="realform">
   ...
   <xsl:call-template name="fieldset-wrapper">
	...
	<xsl:template name="account-dialog-template">
		...
		<div id="accounts-template" style="display:none">
			<div class="clear">
				<div dojoAttachPoint="noItemLabelNode">
  ...
  <!-- ************************************************************************** -->
	...
	<xsl:param name="items"/>
		 <div dojoType="misys.system.widget.MultipleAccountGrid" dialogId="account-dialog-template" gridId="accounts-grid" id="accounts">
