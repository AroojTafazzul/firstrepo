...
<xsl:stylesheet 
  ...
  <xsl:param name="entity_email_required"/>
  ...
  <xsl:param name="isBankViewingBankGroupProfile">N</xsl:param>
  ...
  <xsl:include href="../../../core/xsl/system/sy_entity_customer_references_widget.xsl"></xsl:include>
   ...
   <xsl:variable name="customerEntitiesChargeAccountEnabled" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_ENABLED')"></xsl:variable>
    ...
    <xsl:call-template name="form-wrapper">
     ...
     <!--  <xsl:apply-templates select="customer_reference/references" /> -->
      ...
      <xsl:call-template name="entity_customer_references-component"/>
          
	   		<xsl:if test="$companyType != ''">
	   			<xsl:choose>
				    <xsl:when test="$currentmode='VIEW'">
				     <xsl:call-template name="account-details_view"/>
				     </xsl:when>
				     <xsl:otherwise>
			...
			<div class="widgetContainer">
			    <xsl:call-template name="charge-account-address">
			     ...
			     <xsl:with-param name="companyType" select ="$companyType"/>
			      <xsl:with-param name="ownerId" select ="$ownerId"/>
			     <xsl:with-param name="email_mandatory"><xsl:value-of select="$entity_email_required"/></xsl:with-param>
			    </xsl:call-template>			    
			  </div> 
	  ...
	  <div class="widgetContainer">  	
	   <xsl:call-template name="system-settings"/>
		...
		<xsl:if test="securityCheck:hasPermission(utils:getUserACL($rundata),'access_subscription_package_mc')">
 ...
 <xsl:template name="js-imports">
  ...
  <xsl:with-param name="override-home-url">'/screen/<xsl:value-of select="$nextscreen"/>?option=<xsl:value-of select="$option"/><xsl:if test="$companyType != '03'">&amp;company=<xsl:value-of select="$company"/></xsl:if>'</xsl:with-param>
  </xsl:call-template>
 </xsl:template>
 ...
 <xsl:template name="hidden-fields">
  <div class="widgetContainer">
   ...
   <xsl:call-template name="hidden-field">
    <xsl:with-param name="name">charging_account_hidden</xsl:with-param>
   		...
   		<xsl:with-param name="content">
        	...
        	<xsl:if test="$option='CUSTOMER_ENTITY_MAINTENANCE_MC'">
        		<xsl:call-template name="input-field">
      				<xsl:with-param name="label">XSL_JURISDICTION_COMPANY_SYSTEM</xsl:with-param>
      				<xsl:with-param name="name">company_abbv_name</xsl:with-param>
			...
			<!-- TODO -->
     	  <xsl:call-template name="input-field">
		    <xsl:with-param name="label">XSL_JURISDICTION_NAME_ENTITY</xsl:with-param>
		    <xsl:with-param name="name">name</xsl:with-param>
		    <xsl:with-param name="required">Y</xsl:with-param>
		     <xsl:with-param name="maxsize"><xsl:value-of select="defaultresource:getResource('MAXIMUM_ALLOWED_ENTITY_NAME')"/></xsl:with-param>
	      </xsl:call-template>
	      <xsl:call-template name="input-field">
   ...
   <xsl:with-param name="action"><xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/<xsl:value-of select="$nextscreen"/></xsl:with-param>
    <xsl:with-param name="content">
     <div class="widgetContainer">
      ...
      <xsl:if test="not(security:isCustomer($rundata))">
      	<xsl:call-template name="hidden-field">
 ...
 <!--<xsl:template match="customer_reference/reference">
   <xsl:call-template name="fieldset-wrapper">
   	   <xsl:with-param name="legend">XSL_ENTITY_CIF_DETAILS</xsl:with-param>
   	   <xsl:with-param name="content">
   	   <xsl:call-template name="entity-customer-references">
   	   </xsl:call-template>
  ...
  <!-- Account Details -->
   ...
   <xsl:variable name="customerEntitiesChargeAccountEnabled" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_ENABLED')"></xsl:variable>
     ...
     <xsl:variable name ="chargeAccountRequired">
       <xsl:choose>
        <xsl:when test="$customerEntitiesChargeAccountEnabledAndMandatory = 'true' ">
   ...
   <xsl:call-template name="entity-fieldset-wrapper">
   	   <xsl:with-param name="legend">XSL_ENTITY_ACCOUNT_DETAILS</xsl:with-param>
   	   <xsl:with-param name="content">
   	   <div id="accounts_div">		
	
		</div>
		 <xsl:if test="$customerEntitiesChargeAccountEnabled = 'true'">
		<div class="widgetContainer">
		 <xsl:call-template name="select-field">
     ...
     <xsl:template name="account-details_view">
       <xsl:variable name="customerEntitiesChargeAccountEnabled" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_ENABLED')"></xsl:variable>
     <xsl:variable name="customerEntitiesChargeAccountEnabledAndMandatory" select="defaultresource:getResource('CUSTOMER_ENTITIES_CHARGE_ACCOUNT_MANDATORY')"></xsl:variable>
     <xsl:variable name ="chargeAccountRequired">
       <xsl:choose>
   ...
   <xsl:call-template name="fieldset-wrapper">
		...
		<xsl:with-param name="items" select="accounts/account"/>
		 ...
		 <xsl:if test="$customerEntitiesChargeAccountEnabled = 'true' and $isBankViewingBankGroupProfile = 'N'">
		<div class="widgetContainer">
		 <xsl:call-template name="select-field">
     			<xsl:with-param name="label">XSL_CHARGING_ACCOUNT</xsl:with-param>
     			<xsl:with-param name="name">charging_account</xsl:with-param>
		...
		<div id="account-dialog-template" style="display:none" class="widgetContainer">
			<xsl:attribute name="title">Confirmation</xsl:attribute>
				...
				<xsl:call-template name="input-field">
        	 	...
        	 	<xsl:call-template name="input-field">
       	  			<xsl:with-param name="label">XSL_ACCOUNT_NICK_NAME</xsl:with-param>
            		<xsl:with-param name="name">nickname</xsl:with-param>
            		<xsl:with-param name="maxsize">75</xsl:with-param>
        	 	</xsl:call-template>
				<xsl:call-template name="multichoice-field">
    			 		<xsl:with-param name="type">checkbox</xsl:with-param>
		 ...
		 <div dojoType="misys.system.widget.Accounts" dialogId="account-dialog-template" gridId="accounts-grid" id="accounts">
			...
			<xsl:for-each select="$items">
				...
				<div dojoType="misys.system.widget.Account">
					...
					<xsl:attribute name="displayed_owner_type"><xsl:value-of select="localization:getDecode($language, 'N081', $account/displayed_owner_type)"/></xsl:attribute>
 	...
 	<xsl:param name="label-2">XSL_ENTITY_BULK_BFT_CNAPS</xsl:param>
  	...
  	<xsl:call-template name="fieldset-wrapper">
   		...
   		<xsl:with-param name="content">
     		...
     		<!-- At Customer side - System Features -> Entity maintenance - only the above one property should be enabled -->
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="valueaddedservice">
     <thead>
  		  <tr>
  		     ...
  		     <th class="dojoxGridCell" scope="col"><xsl:value-of select="localization:getGTPString($language, 'XSL_SUBSCRIPTION_APPLY_LOCAL_TAX')"/></th>
  		  </tr>
  	 </thead>
  	 <tr>
  	   <td>
       ...
       <td>
        <xsl:call-template name="multichoice-field">
     	   <xsl:with-param name="type">checkbox</xsl:with-param>
  	  	  	<xsl:with-param name="name">apply_local_tax</xsl:with-param>
         </xsl:call-template>
       </td>
  	 </tr>
  </table>
