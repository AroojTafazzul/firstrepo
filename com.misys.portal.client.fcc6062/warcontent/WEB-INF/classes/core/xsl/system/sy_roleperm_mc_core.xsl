...
<xsl:stylesheet 
 ...
 <xsl:variable name="help_access_key">
  	<xsl:choose>
 		<xsl:when test="security:isBank($rundata)"><xsl:value-of select="'SY_JURIS_B'"></xsl:value-of></xsl:when>
 		<xsl:otherwise><xsl:value-of select="'JM_ROLES'"></xsl:value-of></xsl:otherwise>
 	</xsl:choose>
 	</xsl:variable>
  <xsl:call-template name="system-common-js-imports">
   <xsl:with-param name="xml-tag-name">role_record</xsl:with-param>
   <xsl:with-param name="binding">misys.binding.system.role_perm_mc</xsl:with-param>
   		...
   		<xsl:call-template name="fieldset-wrapper">
   			...
   			<xsl:with-param name="content">
    			...
    			<xsl:call-template name="input-field">
                    ...
                    <xsl:choose>
                    	<xsl:when test="$draftMode='EXISTING' or $makerCheckerState='02' or $makerCheckerState='12' or $makerCheckerState='01'">view</xsl:when>
    			...
    			<xsl:choose>
                    	<xsl:when test="$draftMode='EXISTING' or $makerCheckerState='02'">
			    			...
			    			<xsl:call-template name="hidden-field">
					  	  		<xsl:with-param name="name">roledest</xsl:with-param>
					  	  		<xsl:with-param name="value"><xsl:value-of select="roledest"/></xsl:with-param>
					   		</xsl:call-template>
					   		
					   		<xsl:call-template name="hidden-field">
					  	  		<xsl:with-param name="name">roletype</xsl:with-param>
					  	  		<xsl:with-param name="value"><xsl:value-of select="roletype"/></xsl:with-param>
					   		</xsl:call-template>
                    	</xsl:when>
                    	<xsl:otherwise>
	                    	<!-- Role Destination  -->    
