...
<xsl:stylesheet 
 ...
 <xsl:template name="js-imports">
  <xsl:call-template name="system-common-js-imports">
   ...
   <xsl:with-param name="override-home-url">'/screen/BankSystemFeaturesScreen?option=BANK_ACCOUNT_MAINTENANCE_MC'</xsl:with-param>
	<xsl:with-param name="override-help-access-key">BGM_01</xsl:with-param>  
  </xsl:call-template>
 </xsl:template>
...
<!--  	<xsl:variable name="current"><xsl:value-of select="language"/></xsl:variable>-->
    ...
    <xsl:call-template name="select-field">
	     			...
	     			<xsl:with-param name="value"><xsl:value-of select="channel[@selected='true' and type = '01' and news_destination = '01']/id"/></xsl:with-param>	     			
	     			<xsl:with-param name="options">
	     			<xsl:if test="$displaymode = 'edit'">
	     				<option></option>
	     				<xsl:apply-templates select="channel[type = '01' and news_destination = '01']"/>
	     			</xsl:if>
	     			<xsl:if test="$displaymode = 'view'">
	     				<xsl:value-of select="channel[@selected='true' and type = '01' and news_destination = '01']/name"/>
	     			</xsl:if>
				    </xsl:with-param> 
	 </xsl:call-template>
	 </xsl:if>
