...
<xsl:param name="company_id"><xsl:value-of select="alert_records/static_company/company_id"/></xsl:param>
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="parseFormOnLoad">N</xsl:with-param>
		   		<xsl:with-param name="content">
					...
					<!-- Sms Field -->
<!-- 					<xsl:if test="gtpsecurity:isBank($rundata) and security:hasPermission(utils:getUserACL($rundata),'sy_alert_sms',utils:getUserEntities($rundata))">
						<xsl:call-template name="fieldset-wrapper">
...
<xsl:template name="build-alerts-dojo-items">
	...
	<div dojoType="misys.admin.widget.AlertsBalance">
		<xsl:attribute name="headers">
			...
			<xsl:value-of select="localization:getGTPString($language, 'XSL_ALERT_THRESHOLD_COL')"/>,
			<xsl:choose>
		...
		<xsl:for-each select="$items">
			...
			<div>
				<xsl:attribute name="dojoType">misys.admin.widget.AlertBalance</xsl:attribute>
				...
				<xsl:attribute name="threshold_cur"><xsl:value-of select="$alert/threshold_cur"/></xsl:attribute>
				<xsl:attribute name="threshold_amt"><xsl:value-of select="$alert/threshold_amt"/></xsl:attribute>
				<xsl:attribute name="threshold_sign"><xsl:value-of select="$alert/threshold_sign"/></xsl:attribute>
			</div>
		</xsl:for-each>
	</div>
</xsl:template>
...
<div style="display:none" class="widgetContainer">
	...
	<xsl:attribute name="id">alert<xsl:value-of select="$type_alert"/>-dialog-template</xsl:attribute>
			...
			<xsl:call-template name="input-field">
				...
				<xsl:with-param name="button-type">balances-alert-account</xsl:with-param>
				<xsl:with-param name="required">Y</xsl:with-param>
				<xsl:with-param name="disabled">Y</xsl:with-param>
				<xsl:with-param name="override-product-code">BA</xsl:with-param>
			...
			<xsl:call-template name="radio-field">
				<xsl:with-param name="label">XSL_ALERT_BALANCE_AMT_LESS</xsl:with-param>
				<xsl:with-param name="name">threshold_sign<xsl:value-of select="$type_alert"/></xsl:with-param>
				<xsl:with-param name="id">threshold_sign<xsl:value-of select="$type_alert"/>_0</xsl:with-param>
				<xsl:with-param name="value">0</xsl:with-param>
			</xsl:call-template>
            <xsl:call-template name="radio-field">
				<xsl:with-param name="label">XSL_ALERT_BALANCE_AMT_GREATER</xsl:with-param>
				<xsl:with-param name="name">threshold_sign<xsl:value-of select="$type_alert"/></xsl:with-param>
				<xsl:with-param name="id">threshold_sign<xsl:value-of select="$type_alert"/>_1</xsl:with-param>
				<xsl:with-param name="value">1</xsl:with-param>
			</xsl:call-template>
			<xsl:call-template name="currency-field">
				<xsl:with-param name="label">XSL_ALERT_BALANCE_THRESHOLD_AMT</xsl:with-param>
				<xsl:with-param name="product-code">threshold<xsl:value-of select="$type_alert"/></xsl:with-param>
				...
				<xsl:call-template name="label-wrapper">
					<xsl:with-param name="content">
						<button dojoType="dijit.form.Button" type="button">
							<xsl:attribute name="onClick">misys.dialog.submitBalanceAlert('alert<xsl:value-of select="$type_alert"/>-dialog-template', '<xsl:value-of select="$type_alert"/>')</xsl:attribute>
							<xsl:value-of select="localization:getGTPString($language, 'XSL_SYSTEMFEATURES_OK')"/>
						</button>
						<button dojoType="dijit.form.Button" type="button">
							<xsl:attribute name="onClick">misys.dialog.closeBalanceAlert('alert<xsl:value-of select="$type_alert"/>-dialog-template')</xsl:attribute>
...
<xsl:template name="js-imports">
	<xsl:call-template name="system-common-js-imports">
		...
		<xsl:with-param name="binding">misys.binding.system.alert_balance</xsl:with-param>
