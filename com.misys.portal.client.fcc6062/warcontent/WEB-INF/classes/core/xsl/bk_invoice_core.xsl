...
<xsl:stylesheet 
 ...
 <xsl:param name="deleteImage"><xsl:value-of select="$images_path"/>delete.png</xsl:param>
 	...
 	<xsl:template name="bulk_invoices">
		<script type="text/javascript">
			var invoiceItems =[];
			<xsl:for-each select="product_file_set/in_tnx_record">
				var refInv = "<xsl:value-of select="./ref_id"/>";
				invoiceItems.push({ "REFERENCEID" :"<xsl:value-of select="./ref_id"/>","INVOICE_REFERENCE_LABEL" :"<xsl:value-of select="./issuer_ref_id"/>","ENTITY":"<xsl:value-of select="./entity"/>","PROGRAM_NAME" :"<xsl:value-of select="./fscm_program/program_name"/>","BUYER_NAME" :"<xsl:value-of select="./buyer_name"/>","CURCODE" :"<xsl:value-of select="./total_net_cur_code"/>","AMOUNT" :"<xsl:value-of select="./total_net_amt"/>","OS_AMOUNT" :"<xsl:value-of select="./liab_total_net_amt"/>","INVOICE_DATE" :"<xsl:value-of select="./iss_date"/>","INVOICE_DUE_DATE" :"<xsl:value-of select="./due_date"/>","STATUS" :"<xsl:value-of select="localization:getDecode($language, 'N005', ./prod_stat_code[.])"/>","FIN_REQUESTED_PERCENT" :"<xsl:value-of select="./inv_eligible_pct"/>", "ACTION" : "<![CDATA[<img src=\"/content/images/delete.png\" onClick =\"javascript:misys.deleteInvoiceRecord(refInv)\"/>]]>"});
			</xsl:for-each>	
		</script>
	</xsl:template>
	...
	<xsl:template name="bulk-invoice-grid">
		...
		<xsl:call-template name="fieldset-wrapper">
			<xsl:with-param name="legend">XSL_HEADER_INVOICE</xsl:with-param>
						...
						dojo.ready(function(){
					    	gridLayoutBulkTransaction = {"cells" : [ 
					                  [{ "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'REFERENCEID')"/>", "field": "REFERENCEID", "width": "10em", "styles":"white-space:nowrap;text-align: center;", "headerStyles":"white-space:nowrap;"},
					                   ...
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'PROGRAM_NAME')"/>", "field": "PROGRAM_NAME", "width": "20%", "styles":"white-space:nowrap;text-align: center;", "headerStyles":"white-space:nowrap;"},
					                   ...
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'AMOUNT')"/>", "field": "AMOUNT", "width": "15%", "styles":"text-align: right;", "headerStyles":"white-space:nowrap;text-align: center"},
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'OS_AMOUNT')"/>", "field": "OS_AMOUNT", "width": "15%", "styles":"text-align: right;", "headerStyles":"white-space:nowrap;text-align: center"},
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'INVOICE_DATE')"/>", "field": "INVOICE_DATE", "width": "15%", "styles":"text-align: right;", "headerStyles":"white-space:nowrap;text-align: center"},
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'INVOICE_DUE_DATE')"/>", "field": "INVOICE_DUE_DATE", "width": "15%", "styles":"text-align: right;", "headerStyles":"white-space:nowrap;text-align: center"},
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'STATUS')"/>", "field": "STATUS", "width": "22%", "styles":"text-align: center;", "headerStyles":"white-space:nowrap;text-align: center"},
<!-- 					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'FIN_REQUESTED_PERCENT')"/>", "field":"FIN_REQUESTED_PERCENT", "width": "15%", "styles":"white-space:nowrap;text-align: center;"}
 -->					                    <xsl:if test="$displaymode='edit'">
						...
						<xsl:otherwise>
							<table border="0" cellpadding="0" cellspacing="0" class="attachments">
						     <xsl:attribute name="id">in_table</xsl:attribute>
						      <xsl:choose>
						      	<xsl:when test="invoices/invoice">
							      <thead>
							       <tr>
						...
						<div id="invoice-items-add" class="widgetContainer">
							<div id="invoice_lookup" type="button" dojoType="dijit.form.Button">
								<xsl:value-of select="localization:getGTPString($language, 'XSL_ADD_INVOICES')" />
							</div>
						</div>
						</xsl:if>	
					</div>
