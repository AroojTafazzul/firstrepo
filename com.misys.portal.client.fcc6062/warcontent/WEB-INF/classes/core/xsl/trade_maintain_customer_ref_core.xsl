...
<xsl:stylesheet 
  ...
  <xsl:template name="hidden-fields">
   ...
   <xsl:when test="product_code[.='LN']">
	    ...
	    <xsl:with-param name="show-tnx-id">N</xsl:with-param>
	   </xsl:call-template>
   </xsl:when>
   <xsl:otherwise>
	   	<xsl:call-template name="common-hidden-fields">
    ...
    <xsl:with-param name="content">
     ...
     <xsl:if test="prod_stat_code[.!='']">
      <xsl:call-template name="input-field">
       ...
       <xsl:with-param name="id">general_prod_stat_code_view</xsl:with-param>
     ...
     <xsl:choose>
      <xsl:when test="applicant_abbv_name[. != '']">
       <xsl:call-template name="fieldset-wrapper">
        ...
        <xsl:with-param name="content">
         ...
         <!-- Customer Reference -->
         <xsl:call-template name="set-customer-ref-field">
      ...
      <xsl:when test ="drawee_abbv_name[. != '']">
       <xsl:call-template name="fieldset-wrapper">
      ...
      <xsl:when test ="drawer_abbv_name[. != '']">
       <xsl:call-template name="fieldset-wrapper">
        ...
        <xsl:with-param name="content">
         ...
         <xsl:if test="borrower_address_line_1[.!='']">
          <xsl:call-template name="input-field">
           ...
           <xsl:with-param name="value" select="borrower_address_line_1"/>
           <xsl:with-param name="override-displaymode">view</xsl:with-param>
          </xsl:call-template>
         </xsl:if>
    ...
    <xsl:with-param name="content">
     <div class="widgetContainer">
      ...
      <xsl:call-template name="hidden-field">
       ...
       <xsl:with-param name="value">UPDATE_CUSTOMER_REF</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="hidden-field">
  ...
  <xsl:template name="set-customer-ref-field">
   <!-- Optional -->
   <xsl:param name="label">XSL_GENERALDETAILS_CUST_REF_ID</xsl:param>
   	...
   	<xsl:choose>
   		<xsl:when test="product_code[.='FX'] or (product_code[.='TD'] and sub_product_code[.='TRTD']) or (product_code[.='FT'] and (sub_product_code[.='TRINT'] or sub_product_code[.='TRTPT']))">
   			<xsl:call-template name="input-field">
			     <xsl:with-param name="label">XSL_GENERALDETAILS_CUST_REF_ID</xsl:with-param>
			     <xsl:with-param name="name">cust_ref_id</xsl:with-param>
			     <xsl:with-param name="value" select="cust_ref_id_new" />
		   ...
		   <xsl:call-template name="input-field">
			...
			<xsl:with-param name="regular-expression">
		      ...
		      <xsl:when test="adv_send_mode[. = '01']">
					<xsl:value-of select="defaultresource:getResource('SWIFT_VALIDATION_REGEX')"/>
			  </xsl:when>
			  <xsl:otherwise>
