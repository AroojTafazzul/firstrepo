...
<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:datetools="xalan://com.misys.portal.common.tools.ConvertTools" exclude-result-prefixes="datetools">
	<!-- Copyright (c) 2000-2008 Misys (http://www.misysbanking.com), All Rights 
		Reserved. -->
	<xsl:template match="/">
		<xsl:apply-templates select="MeridianMessage[ExternalMessageType = 'MT950']" />
	</xsl:template>

	<xsl:template match="MeridianMessage[ExternalMessageType = 'MT950']">
		<accountStatements>
			<account>
				<account_no>
				...
				<cur_code>
					<xsl:value-of select="$cur_code"></xsl:value-of>
				</cur_code>
					...
					<statement>
						<xsl:choose>
							<xsl:when test="contains(NumberSeq,'/')">
								<idx>
									<xsl:value-of select="substring-before(NumberSeq,'/')" />
								</idx>
								<seq_idx>
									<xsl:value-of select="substring-after(NumberSeq,'/')" />
								</seq_idx>
							</xsl:when>
							<xsl:otherwise>
								<idx>
									<xsl:value-of select="NumberSeq" />
								</idx>
								<seq_idx />
							</xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template match="OpBalF|CloBalF|OpBalM|CloBalM|CloAvailBal|FwdAvailBal">
		...
		<balance>
			...
			<xsl:variable name="sign">
	        	<xsl:if test="substring(.,6,2) = D">-</xsl:if>
			</xsl:variable>
			<value_date><xsl:value-of select="substring(.,6,2)" />/<xsl:value-of select="substring(.,4,2)" />/20<xsl:value-of select="substring(.,2,2)" /> 00:00:00 AM</value_date>
	...
	<xsl:template match="StatementLine">
		...
		<line>			
	        ...
	        <xsl:if test="string-length(translate(substring(.,1,10), '0123456789',''))=0">
	            <value_date><xsl:value-of select="substring(.,5,2)" />/<xsl:value-of select="substring(.,3,2)" />/20<xsl:value-of select="substring(.,1,2)" />  00:00:00 AM</value_date>
	            <post_date><xsl:value-of select="substring(.,9,2)" />/<xsl:value-of select="substring(.,7,2)" />/20<xsl:value-of select="substring(.,1,2)" />  00:00:00 AM</post_date>
	        </xsl:if>
