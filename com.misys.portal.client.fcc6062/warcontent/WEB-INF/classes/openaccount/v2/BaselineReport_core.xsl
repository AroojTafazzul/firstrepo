...
<xsl:stylesheet version="1.0" 
	...
	<!-- Process TSU Message -->
	<xsl:template match="BaselnRpt">
		
		<!-- Process only if the baseline is not precalculated -->
		<!-- <xsl:if test="RptTp/Tp != 'PREC'">-->
		
			<!-- Detect if the baseline is related to a PO or a SO -->
			<xsl:variable name="poRefId"><xsl:value-of select="tools:retrievePOfromTID(TxId/Id, $BIC)"/></xsl:variable>
			<xsl:variable name="soRefId"><xsl:value-of select="tools:retrieveSOfromTID(TxId/Id, $BIC)"/></xsl:variable>
