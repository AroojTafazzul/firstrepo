...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="DataSetMtchRpt">
		...
		<xsl:variable name="inRefId"><xsl:value-of select="UsrTxRef/Id"/></xsl:variable>
		
		<xsl:if test="$inRefId != '' and tools:doesInvoiceExist($inRefId, $BIC)">
			<xsl:variable name="invoiceIssuer"><xsl:value-of select="UsrTxRef/IdIssr/BIC"/></xsl:variable>
			...
			<in_tnx_record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.neomalogic.com/gtp/interfaces/xsd/in.xsd">
				...
				<company_name><xsl:value-of select="tools:retrieveFieldValueFromProduct('IN', $inRefId, 'company_name')"/></company_name>
				...
				<!-- Link DataSetMatchReport with Invoice -->
				<cross_references>
					<cross_reference>
						<ref_id><xsl:value-of select="$inRefId"/></ref_id>
						<product_code>IN</product_code>
