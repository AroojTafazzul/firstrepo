...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="Doc:TxRltdRefs/Doc:TxId">
	...
	<xsl:template match="Doc:tsmt.017.001.02">
		...
		<!-- <xsl:for-each select="Doc:TxRltdRefs/Doc:TxId">-->
			<xsl:variable name="tid"><xsl:value-of select="."/></xsl:variable>
			<xsl:variable name="poRefId"><xsl:value-of select="tsutools:retrievePOfromTID(Doc:TxRltdRefs/Doc:TxId, $BIC)"/></xsl:variable>
			...
			<po_tnx_record
				...
				<tnx_cur_code><xsl:value-of select="//Doc:ComrclDataSet/Doc:Goods/Doc:TtlNetAmt/@Ccy"/></tnx_cur_code>
				<tnx_amt>
					<xsl:call-template name="TP_amount">
						<xsl:with-param name="amount"><xsl:value-of select="//Doc:ComrclDataSet/Doc:Goods/Doc:TtlNetAmt"/></xsl:with-param>
					</xsl:call-template>
				</tnx_amt>
