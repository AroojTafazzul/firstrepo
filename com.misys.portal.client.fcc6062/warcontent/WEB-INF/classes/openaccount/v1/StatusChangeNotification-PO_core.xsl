...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="Doc:tsmt.025.001.02">
		...
		<xsl:variable name="poRefId"><xsl:value-of select="tools:retrievePOfromTID($tid)"/></xsl:variable>
		
		<xsl:if test="$poRefId != '' and Doc:UsrTxRef/Doc:IdIssr/Doc:BIC != $BIC">
			<po_tnx_record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.neomalogic.com/gtp/interfaces/xsd/po.xsd">
				...
				<bo_comment>
					<xsl:value-of select="localization:getGTPString($language, 'INTERFACE_PO_REJECT')"/>
				</bo_comment>
			</po_tnx_record>
		</xsl:if>
	</xsl:template>
