...
<xsl:stylesheet xmlns:convertTools="xalan://com.misys.portal.common.tools.ConvertTools" xmlns:defaultresource="xalan://com.misys.portal.common.resources.DefaultResourceProvider" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:localization="xalan://com.misys.portal.common.localization.Localization" xmlns:security="xalan://com.misys.portal.security.GTPSecurity" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc">
	...
	<xd:doc>
	<xd:short>IO transaction Record</xd:short>
			<xd:detail>This calls three templates for header,footer and body to generate pdf.
	</xd:detail>
	</xd:doc>
 ...
 <xd:doc>
 	...
 	<xd:detail>This template calls header issuing bank template,It forms header of PDF which contains bank details,product code
...
<xsl:template name="header">
  ...
  <xd:detail>
  		...
  		Dates(issueDate,ExpiryDate etc). It contains all the feild information of the Import Open Account Form(Seller Details,buyer Details,Line Items,Routing summary
  		DataSet etc)
  </xd:detail>
  </xd:doc>
  <xsl:template name="body">
    <fo:flow flow-name="xsl-region-body" font-size="{$pdfFontSize}">
			...
			<xsl:call-template name="table_template">
				<xsl:with-param name="text">
					...
					<xsl:if test="po_issue_date[. != '']">
						<xsl:call-template name="table_cell">
							<xsl:with-param name="left_text">
								<xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_PO_ISS_DATE')"/>
							</xsl:with-param>
							<xsl:with-param name="right_text">
					...
					<xsl:if test="intent_to_pay_flag[.='Y']">
						<xsl:call-template name="table_cell">
							<xsl:with-param name="left_text"/>
							<xsl:with-param name="right_text">
									<xsl:value-of select="localization:getGTPString($language, 'XSL_INTENT_TO_PAY_FLAG')"/>
							</xsl:with-param>
						</xsl:call-template>
					</xsl:if>
		...
		<xsl:if test="tnx_type_code and tnx_type_code[.='38']">
		...
		<xsl:if test="not(tnx_type_code and tnx_type_code[.='38'])">
			<!-- Applicant Details-->
...
<!-- 			<xsl:if test="seller_account_type != '' or fin_inst_name != '' or fin_inst_bic != ''"> -->
  ...
  <xd:doc>
  	...
  	<xd:detail>This template contains footer section of PDF which contains System Date and page no.
  	</xd:detail>
  </xd:doc>
