...
-->
  ...
  <xsl:import href="../com_attachment.xsl"/>
	...
	<xsl:template match="po_tnx_record">
		...
		<script type="text/javascript" src="/content/OLD/javascript/trade_message_discrepant.js"/>
		...
		<td align="left">
			...
			<!-- Return the original tnx id -->
			<input type="hidden" name="sub_tnx_type_code" value="15"/>
			<input type="hidden" name="prod_stat_code" value="10"/>
			...
			<table border="0" width="570" cellpadding="0" cellspacing="0">
				...
				<!--xsl:if test="exp_date[.!='']">
					<tr>
						...
						<td>
							<font class="REPORTDATA"><xsl:value-of select="exp_date"/></font>
							<input type="hidden" name="exp_date">
								<xsl:attribute name="value"><xsl:value-of select="exp_date"/></xsl:attribute>
							</input>
						</td>
					</tr>
				</xsl:if-->
				...
				<!--tr>
					<td width="40">&nbsp;</td>
					<td width="150">
						<font class="FORMMANDATORY">Close</font>
					</td>
					<td>
