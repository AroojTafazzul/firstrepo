...
<xd:doc>
	...
	<xd:param name="section_payment_transport_dataset">Details of Payment Transport Dataset</xd:param>
	<xd:param name="section_payment_insurance_dataset">Details of Payment Insurance Dataset</xd:param>
	<xd:param name="section_payment_certificate_dataset">Details of Payment Certificate Dataset</xd:param>
	<xd:param name="section_payment_other_certificate_dataset">Details of Other Certificate Dataset</xd:param>
	 </xd:doc>
	...
	<xsl:param name="section_line_item_shipment_details">N</xsl:param>
	<xsl:param name="section_line_item_inco_terms_details">N</xsl:param>
	...
	<xsl:param name="section_payment_transport_dataset">
		<xsl:if test="$displaymode = 'view'">
			<xsl:choose>
				<xsl:when test="io_tnx_record/payment_transport_dataset/TrnsprtDataSet">Y</xsl:when>
				<xsl:otherwise>N</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:param>
	<xsl:param name="section_payment_insurance_dataset">
		<xsl:if test="$displaymode = 'view'">
			<xsl:choose>
				<xsl:when test="io_tnx_record/payment_insurance_dataset/InsrncDataSet">Y</xsl:when>
				<xsl:otherwise>N</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:param>
	<xsl:param name="section_payment_certificate_dataset">
		<xsl:if test="$displaymode = 'view'">
			<xsl:choose>
				<xsl:when test="io_tnx_record/payment_certificate_dataset/CertDataSet">Y</xsl:when>
				<xsl:otherwise>N</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:param>
	<xsl:param name="section_payment_other_certificate_dataset">
		<xsl:if test="$displaymode = 'view'">
			<xsl:choose>
				<xsl:when test="io_tnx_record/payment_other_certificate_dataset/OthrCertDataSet">Y</xsl:when>
				<xsl:otherwise>N</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:param>
	...
	<xsl:template match="io_tnx_record">
		...
		<!-- Payment Transport Routing Summary -->
		<xsl:call-template name="payment-individual-routing-summary-declaration" />
		
		<xsl:call-template name="transport-document-ref-declaration" />
		
		<xsl:call-template name="transported-goods-declaration" />
		
		<xsl:call-template name="payment-transport-freight-charges-declaration" />
		
		<xsl:call-template name="payment-insurance-required-clause-declaration"/>
		
		<xsl:call-template name="payment-insurance-required-conditions-declaration"/> 
		<!-- Payment Other Certificate Declaration -->
		<xsl:call-template name="payment-other-cds-declaration" />
		
		<xsl:call-template name="pmt-other-cds-info-declaration"/>
		
		<!-- Payment Certificate declaration -->
		<xsl:call-template name="payment-certificate-ds-details-declaration" /> 
		
		<!-- Payment Certificate Additional Information declaration -->
		<xsl:call-template name="payment-certificate-addtl-inf-declaration" />
		
		<!--  Payment Certificate LineItems declaration  -->
		<xsl:call-template name="payment-certificate-line-item-declaration" />
		
		<!--  Payment Certificate LineItems Identification declaration  -->
		<xsl:call-template name="payment-certificate-line-item-id-declaration" />	
		...
		<div>
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="content">						
					<!--  Display common menu.  -->
					<!-- This conditions cater to Scenarios of accept/reject from bank and from return from B user in AtoB Scenario,Hence controlled by prod stat code and tnx stat code -->
					<xsl:choose>
						<xsl:when test="prod_stat_code and (prod_stat_code = '65' or prod_stat_code = 'D1' or prod_stat_code = 'D2' or prod_stat_code = 'A0' and $mode='ACCEPT') and (tnx_stat_code and (tnx_stat_code='04' or tnx_stat_code='01' ))">
					...
					<xsl:call-template name="general-details" />
						...
						<!-- TBD : All four datasets to be added with templates in line with forwward dataset message  -->
						<!-- Transport Dataset -->
						<xsl:if test="$section_payment_transport_dataset!='N'">
							<xsl:call-template name="payment-transport-dataset"/>
						</xsl:if>
						
						<!-- Payment Insurance dataset -->
						<xsl:if test="$section_payment_insurance_dataset!='N'">
							<xsl:call-template name="payment-insurance-dataset-details"/>
						</xsl:if>
						
						<xsl:if test="$section_payment_certificate_dataset!='N'">
							<xsl:call-template name="payment-certificate-ds-details"/>
						</xsl:if>
						
						<!-- Other Certificate Dataset -->
						<xsl:if test="$section_payment_other_certificate_dataset!='N'">
							<xsl:call-template name="fieldset-wrapper">
								<xsl:with-param name="legend">XSL_HEADER_OTHER_CERTIFICATE_DATASET_DETAILS</xsl:with-param>
								<xsl:with-param name="content">
									&nbsp;
									<xsl:call-template name="build-payment-other-cds-dojo-items">
										<xsl:with-param name="items"
											select="payment_other_certificate_dataset/OthrCertDataSet" />
										<xsl:with-param name="id">payment_other_cds_dataset_details</xsl:with-param>
									</xsl:call-template>
								</xsl:with-param>
							</xsl:call-template>
						</xsl:if>
						...
						<xsl:call-template name="fieldset-wrapper">
			          	...
			          	<xsl:if test="dataset_match_report/DataSetMtchRpt/Rpt/MisMtchInf[.!='']">
			          	<xsl:call-template name="dataset-mismatch-info">
				   			<xsl:with-param name="items" select="dataset_match_report/DataSetMtchRpt/Rpt"/>
				   		</xsl:call-template>
				   		</xsl:if>
						...
						<xsl:choose>
						<xsl:when test = "tnx_stat_code!='02'">
						<xsl:call-template name="comments-for-return">
					   	...
					   	<xsl:call-template name="comments-for-return">
					   	<xsl:with-param name="value"><xsl:value-of select="return_comments"/></xsl:with-param>
						 	<xsl:with-param name="mode">UNSIGNED</xsl:with-param>
					   	</xsl:call-template>
					   	</xsl:otherwise>
					   	</xsl:choose>
						
						<xsl:if test="prod_stat_code or (prod_stat_code = '65' or prod_stat_code = 'D1' or prod_stat_code = 'D2' and $mode='ACCEPT')">
					   	<xsl:call-template name="instructions-to-bank">
						 	...
						 	<xsl:with-param name="required"><xsl:if test = "$mode = 'ACCEPT'">N</xsl:if></xsl:with-param>
						 	<xsl:with-param name="mode"><xsl:if test = "$mode= 'ACCEPT'"><xsl:value-of select="$mode"/></xsl:if></xsl:with-param>
					   	</xsl:call-template>
					   	</xsl:if>
				</xsl:with-param>										
			</xsl:call-template>						
		...
		<!-- Form #1 : Attach Files -->
		<xsl:choose>
			<xsl:when test="(sub_tnx_type_code and sub_tnx_type_code = '83') or ($displaymode = 'edit' or ($displaymode != 'edit' and $mode = 'UNSIGNED'))">
				   <xsl:choose>
					  	 <xsl:when test="(sub_tnx_type_code and sub_tnx_type_code = '83')">
		...
		<xsl:choose>
			<xsl:when test="prod_stat_code and (prod_stat_code = '65' or prod_stat_code = 'D1' or prod_stat_code = 'D2' or prod_stat_code = 'A0' and $mode='ACCEPT') and (tnx_stat_code and (tnx_stat_code='04' or tnx_stat_code='01' or tnx_stat_code='06' ))">
					<xsl:call-template name="menu">
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.openaccount.payment_oa</xsl:with-param>
		...
		<xsl:call-template name="fieldset-wrapper">
			...
			<xsl:with-param name="content">
				...
				<xsl:call-template name="input-field">
					...
					<xsl:with-param name="readonly"><xsl:if test="tnx_type_code[.='31' or .='38'] or (cross_references and cross_references[.!= ''] and tnx_type_code='01')" >Y</xsl:if></xsl:with-param>
				</xsl:call-template>
				<xsl:call-template name="input-field">
	...
	<xsl:template name="po-general-details">
		...
		<xsl:if test="$displaymode='view' and tma_used_status = 'Y'">
		 	<xsl:call-template name="input-field">
		...
		<xsl:if test="$displaymode='view' and bpo_used_status = 'Y'">
		 	<xsl:call-template name="input-field">
		...
		<xsl:if test="$displaymode='view' and intent_to_pay_flag = 'Y'">
		 	<xsl:call-template name="input-field">
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="content">
					<div class="widgetContainer">
						...
						<xsl:call-template name="hidden-field">
							<xsl:with-param name="name">tnxstatcode</xsl:with-param>
