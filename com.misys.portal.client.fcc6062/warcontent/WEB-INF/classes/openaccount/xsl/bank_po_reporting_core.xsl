...
<xsl:output method="html" indent="no" />
	...
	<script type="text/javascript" src="/content/OLD/javascript/openaccount/bank_po_reporting.js"></script>
	...
	<!-- Edit PO Transaction Details - BEGIN -->

	<tr>
	<td align="left">
	
	<xsl:choose>
		<!-- The details of the PO are only shown if the beneficiary is not defined (it means that the transaction
		...
		<!-- Uncomment line to enable PO detaile editing 
		...
		<xsl:when test="tnx_type_code[.='15' or .='13']">
			...
			<div id='divTransactionDetails' style='display: none'>
		...
		<!--Buyer Details-->	
   			...
   			<tr>
   				...
   				<xsl:if test="tnx_type_code[.='01'] and release_dttm[.='']">
   					<td class="FORMH2" align="right">
   						...
   						<xsl:attribute name="onclick">fncSearchPopup('bankbeneficiary', 'fakeform1',"['company_id','buyer_abbv_name','entity','buyer_name','buyer_post_code','buyer_town_name','buyer_country']",'<xsl:value-of select="product_code"/>','','','CUSTOMER');return false;</xsl:attribute>
   						<img border="0" src="/content/images/search.png" name="img_search_buyer">
   						<xsl:attribute name="alt"><xsl:value-of select="localization:getGTPString($language, 'XSL_ALT_APPLICANT')"/></xsl:attribute>
   						</img>
   						</a>
   					</td>	
   				</xsl:if>
   		...
   		<table border="0" cellspacing="0" width="570">
   			<!-- Show applicant abbv name for initiations on bank side --> 
   			<!--  --> 
         <xsl:choose>
   			  <xsl:when test="tnx_type_code[.='01'] and release_dttm[.='']">
      			<tr>
   				...
   				<tr>
   					...
   					<td>
   						<input type="text" size="35" maxlength="35" name="buyer_abbv_name" onfocus="this.blur();" onblur="fncRestoreInputStyle('fakeform1','buyer_abbv_name');">
           ...
           <xsl:otherwise>
			...
			<!--tr>
				...
				<td width="150">
					<font class="FORMMANDATORY">
						<xsl:value-of select="localization:getGTPString($language, 'XSL_PARTIESDETAILS_BUYER_BANK_BIC')"/>
					</font>
				</td>
				<td>
					<input type="text" size="11" maxlength="11" name="buyer_bank_bic" onblur="fncRestoreInputStyle('fakeform1','buyer_bank_bic');fncCheckBICFormat(this);">
			...
			<tr>
				...
				<td>
					<input type="text" size="35" maxlength="35" name="buyer_country_sub_div" onblur="fncRestoreInputStyle('fakeform1','buyer_country_sub_div');">
			...
			<tr>
				...
				<td class="FORMH2" align="right">
					<a name="anchor_search_seller" href="javascript:void(0)">
						<xsl:attribute name="onclick">fncSearchPopup('beneficiary', 'fakeform1',"['seller_name', 'seller_bei', 'seller_street_name', 'seller_post_code', 'seller_town_name', 'seller_country_sub_div', 'seller_country']",'', 'BaseLineListPopup', '<xsl:value-of select="product_code"/>');return false;</xsl:attribute>
		...
		<table border="0" cellspacing="0" width="570">
			<input type="hidden" name="seller_abbv_name">
			...
			<tr>
				...
				<td><xsl:value-of select="localization:getGTPString($language, 'XSL_PO_PARTIESDETAILS_COUNTRY_SUB_DIVISION')"/></td>
				<td>
					<input type="text" size="35" maxlength="34" name="seller_country_sub_div" onblur="fncRestoreInputStyle('fakeform1','seller_country_sub_div');">
		...
		<form name="form_issuing_bank">
   		<div id='divIssuingBank' style='display: none'> 
   			<input name="issuing_bank_name" type="hidden">
   				<xsl:attribute name="value"><xsl:value-of select="issuing_bank/name"/></xsl:attribute>
   			</input>
   			<input name="issuing_bank_iso_code" type="hidden">
		...
		<div id='divBank0' style='position:relative'>
			...
			<table cellspacing="0" cellpadding="0" border="0">
			<tr>
				<td id="cell_advising_bank" width="175" height="25" align="center">
					<xsl:attribute name="style">background-image: url(/content/images/tab_on<xsl:if test="advising_bank/name[.!='']">_sel</xsl:if>.gif)</xsl:attribute>
					<a href="javascript:void(0)" onclick="fncShowTab(1,'advising_bank','','_name');return false;" style="text-decoration: none">
						<font class="FORMTABMANDATORY">
							<xsl:value-of select="localization:getGTPString($language, 'XSL_BANKDETAILS_TAB_ADVISING_BANK')"/>
						</font>
					</a>
				</td>
				...
				<td id="cell_seller_bank" width="175" height="25" align="center">
					<xsl:attribute name="style">background-image: url(/content/images/tab_off<xsl:if test="seller_bank/name[.!='']">_sel</xsl:if>.gif)</xsl:attribute>
			...
			<table border="0" width="570" height="175">
				<tr>
					<td valign="top">
						...
						<div id='div_advising_bank' style='display: block'>
		...
		<xsl:otherwise>
			...
			<form name="fakeform1">
				<input type="hidden" name="company_id"><xsl:attribute name="value"><xsl:value-of select="company_id"/></xsl:attribute></input>
				<input type="hidden" name="exp_date"><xsl:attribute name="value"><xsl:value-of select="exp_date"/></xsl:attribute></input>
				<xsl:if test="tnx_type_code[.='03']">
					<input type="hidden" name="iss_date"><xsl:attribute name="value"><xsl:value-of select="iss_date"/></xsl:attribute></input>
				</xsl:if>
				<!-- total_type must be passed and saved for PO initiated on bank side -->
 				<input type="hidden" name="brch_code"><xsl:attribute name="value"><xsl:value-of select="brch_code"/></xsl:attribute></input>
				<!-- old_ctl_date used to check the transaction has not been modified in between -->
				<input type="hidden" name="old_ctl_dttm"><xsl:attribute name="value"><xsl:value-of select="bo_ctl_dttm"/></xsl:attribute></input>
			</form>
	...
	<!-- TEMPLATE Advising Bank -->

	<xsl:template match="advising_bank">
				...
				<tr>
					...
					<td colspan="2">
						<select>
							...
							<xsl:attribute name="onchange">fncResetAdvisingBank();fncRestoreInputStyle('form_<xsl:value-of select="$theNodeName"/>','<xsl:value-of select="$theNodeName"/>_type_code');</xsl:attribute>
							<option value="">&nbsp;</option>
							<option value="01">
								<xsl:if test="name = ../issuing_bank/name">
									<xsl:attribute name="selected"/>
								</xsl:if>
								<xsl:value-of select="localization:getGTPString($language, 'XSL_BANKDETAILS_TYPE_ISSUING')"/>
							</option>
							<option value="*">
								<xsl:if test="name != ../issuing_bank/name and name[.!= '']">
									<xsl:attribute name="selected"/>
								</xsl:if>
				...
				<tr>
					...
					<td>
						<a href="javascript:void(0)">
							<xsl:attribute name="onclick">fncSearchAdvisingBankPopup("bank", "form_<xsl:value-of select="$theNodeName"/>","['<xsl:value-of select="$theNodeName"/>_name', '<xsl:value-of select="$theNodeName"/>_address_line_1', '<xsl:value-of select="$theNodeName"/>_address_line_2', '<xsl:value-of select="$theNodeName"/>_dom', '<xsl:value-of select="$theNodeName"/>_iso_code', 'submission_type']");return false;</xsl:attribute>
