		...
		<result>
			<com.misys.portal.openaccount.product.in.common.Invoice>
				...
				<xsl:if test="total_adjustments">
					<total_adjustments>
						<xsl:value-of select="total_adjustments"/>
					</total_adjustments>
				</xsl:if>
				...
				<xsl:if test="discount_exp_date">
					<discount_exp_date>
						<xsl:value-of select="discount_exp_date"/>
					</discount_exp_date>
				</xsl:if>
				...
				<!-- Submitter BIC (used especially for the invoice) -->
				<xsl:if test="submitr_bic">
					<submitr_bic>
						<xsl:value-of select="submitr_bic"/>
					</submitr_bic>
				</xsl:if>
				<!-- Eligibility Flag -->
				<eligibility_flag>
		...
		<xsl:if test="narrative_description_goods">
			...
			<xsl:for-each select="//*[starts-with(name(), 'charge_details_chrg_details_')]">
				<xsl:call-template name="CHARGE">
					<xsl:with-param name="brchCode"><xsl:value-of select="//in_tnx_record/brch_code"/></xsl:with-param>
					<xsl:with-param name="companyId"><xsl:value-of select="//in_tnx_record/company_id"/></xsl:with-param>
					<xsl:with-param name="refId"><xsl:value-of select="//in_tnx_record/ref_id"/></xsl:with-param>
					<xsl:with-param name="tnxId"><xsl:value-of select="//in_tnx_record/tnx_id"/></xsl:with-param>
