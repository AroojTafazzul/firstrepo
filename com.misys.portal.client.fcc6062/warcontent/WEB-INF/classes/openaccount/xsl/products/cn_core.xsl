     ...
     <xsl:call-template name="product-additional-fields"/>
			...
			<xsl:for-each select="//*[starts-with(name(), 'charge_details_chrg_details_')]">
				<xsl:call-template name="CHARGE">
					...
					<xsl:with-param name="companyId"><xsl:value-of select="//cn_tnx_record/company_id"/></xsl:with-param>
					<xsl:with-param name="refId"><xsl:value-of select="//cn_tnx_record/ref_id"/></xsl:with-param>
					<xsl:with-param name="tnxId"><xsl:value-of select="//cn_tnx_record/tnx_id"/></xsl:with-param>
