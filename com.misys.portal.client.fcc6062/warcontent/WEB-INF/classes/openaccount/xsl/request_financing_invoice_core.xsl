...
<xsl:stylesheet 
	...
	<xsl:template name="invoice-grid">
		<xsl:call-template name="fieldset-wrapper">
			<xsl:with-param name="content">
				<div id="in-items-template">
					<script type="text/javascript">
						var gridLayoutINTransaction, pluginsData;
						dojo.ready(function(){
					    	gridLayoutINTransaction = {"cells" : [ 
					                  [{ "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'REFERENCEID')"/>", "field": "reference_id", "width": "8em", "styles":"white-space:nowrap;text-align: center;", "headerStyles":"white-space:nowrap;"},
					                  ...
					                  <xsl:if test="entities and entities > '0'">
					                   ...
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'BUYER_NAME')"/>", "field": "buyer_name", "width": "20%", "styles":"white-space:nowrap;text-align: center;", "headerStyles":"white-space:nowrap;"},
					                   ...
					                   { "noresize":"true", "name": "<xsl:value-of select="localization:getGTPString($language, 'INVOICE_DATE')"/>", "field": "iss_date", "width": "10%", "styles":"text-align: center;", "headerStyles":"white-space:nowrap;text-align: center"},
					...
					<div style="width:100%;height:100%;" class="widgetContainer clear">
						<div dojoType="dojox.data.QueryReadStore" jsId="storeINDetails" requestMethod="post">
							<xsl:attribute name="url">								
								<xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/AjaxScreen/action/BaseLineAction?ajaxoption=MULTI_IN_REQUEST
					    ...
					    <table rowsPerPage="50" 
							plugins="pluginsData" 
							store="storeINDetails" structure="gridLayoutINTransaction" class="grid" 
		...
		<xsl:call-template name="invoice-requested-amt"/>
	</xsl:template>
	<xsl:template name="open-selected-details">
		<xsl:call-template name="fieldset-wrapper">
			<xsl:with-param name="content">
				...
				<xsl:call-template name="invoice-grid"/>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<xsl:template name="invoice-requested-amt">
		<xsl:call-template name="currency-field">
