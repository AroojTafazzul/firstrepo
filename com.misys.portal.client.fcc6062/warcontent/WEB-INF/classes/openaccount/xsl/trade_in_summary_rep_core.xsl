...
<xsl:stylesheet version="1.0"
   ...
   <div id="event-summary">
    ...
    <xsl:with-param name="parse-widgets">N</xsl:with-param>
     ...
     <xsl:with-param name="content">
      ...
      <xsl:choose>
       <!-- NEW -->
       <xsl:when test="tnx_type_code[.= '01'] or tnx_type_code[.= '13'] or tnx_type_code[.='15'] or tnx_type_code[. = '63'] or tnx_type_code[. = '85']"> 
		<xsl:call-template name="input-field">
        ...
        <xsl:if test="tnx_cur_code[.!=''] and tnx_amt[.!='']">
	        <xsl:if test="product_code[.='IP']">
		        <xsl:choose>
	     			<xsl:when test="sub_tnx_type_code[.='74']">
	     				<div class="field ">
							<span class="label"></span>
							<div class="content"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_IN_CHOICE_YES_EARLY_PAYMENT_NOTE')"/></div>
						</div>
	     			</xsl:when>
	     			<xsl:otherwise>
	     			<div class="field ">
							<span class="label"></span>
							<div class="content"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_IN_CHOICE_NO_REQUEST_EARLY_PAYMENT')"/></div>
					</div>
	     			</xsl:otherwise>
	     		</xsl:choose>
     		</xsl:if>
      ...
      <xsl:if test="tnx_stat_code[.='04'] or security:isBank($rundata) or prod_stat_code = '46' or (finance_requested_flag[.='Y' or .='P'] and tnx_type_code[.='63']) or (prod_stat_code ='E2' and tnx_type_code ='85')">
      	<xsl:call-template name="fieldset-wrapper">
           ...
           <xsl:if test="eligibility_flag[.!='']">
      	  ...
      	  <!-- To add the amended outstanding amt details in case of option = FULL and DETAILS -->
			...
			<xsl:if test="inv_eligible_amt[.!=''] and sub_tnx_type_code[.!='A4'] and tnx_type_code[.!='85']">
          	...
          	<xsl:if test="finance_requested_flag[.='Y' or .='P'] and tnx_type_code[.='63']">
	           ...
	           <xsl:call-template name="currency-field">
					...
					<xsl:with-param name="override-amt-value"><xsl:value-of select="finance_requested_amt"/></xsl:with-param>
					<xsl:with-param name="override-displaymode">view</xsl:with-param>
          	...
          	<xsl:if test="finance_amt[.!='']">
	           	<xsl:call-template name="currency-field">
					<xsl:with-param name="label">
						<xsl:choose>
							<xsl:when test="fscm_programme_code[.='01']">XSL_GENERALDETAILS_IN_PAYMENT_AMOUNT</xsl:when>
			...
			<xsl:if test="settlement_amt[.!=''] and tnx_type_code[.='85']">				
     		...
     		<xsl:if test="liab_total_net_amt[.!=''] and sub_tnx_type_code[.!='A4']">
     			<xsl:call-template name="currency-field">
					...
					<xsl:with-param name="override-currency-name">liab_total_net_cur_code</xsl:with-param>
					<xsl:with-param name="override-amt-name">liab_total_net_amt</xsl:with-param>
      ...
      <xsl:if test="tnx_type_code[.= '15'] and $option != 'FULL'">
			<xsl:call-template name="fieldset-wrapper">
				...
				<xsl:call-template name="currency-field">
					...
					<xsl:with-param name="override-currency-value" select="total_net_cur_code"></xsl:with-param>
					<xsl:with-param name="override-amt-value" select="total_net_amt"></xsl:with-param>
					<xsl:with-param name="override-displaymode">view</xsl:with-param>
				</xsl:call-template>
				<xsl:call-template name="currency-field">
					<xsl:with-param name="label">XSL_AMOUNTDETAILS_OS_AMT_LABEL</xsl:with-param>
 ...
 <xsl:template name="disclaimer-column-wrapper">
  	...
  	<div class="column-container">
		<div style="float:left;width:380px;"><xsl:value-of select="$leftContent" /></div>
