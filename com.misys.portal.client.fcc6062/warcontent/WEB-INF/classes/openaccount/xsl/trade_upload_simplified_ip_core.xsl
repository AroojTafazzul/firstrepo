...
<xsl:stylesheet 
  ...
  <script>
	dojo.ready(function(){
	   ...
	   {
	   <xsl:for-each select="upload_templates/upload_template[./product_code='IP']">
  ...
  <xsl:template name="js-imports">
   <xsl:call-template name="common-js-imports">
    <xsl:with-param name="binding">misys.binding.openaccount.upload_simplified_ip</xsl:with-param>
    <xsl:with-param name="override-help-access-key">IP_01_UPLOAD</xsl:with-param>
   </xsl:call-template>
  </xsl:template>
			...
			<xsl:with-param name="content">
				...
				<xsl:with-param name="label">XSL_INVOICE_UPLOAD_TEMPLATE</xsl:with-param>
		     	...
		     	<xsl:with-param name="options">
		     		<xsl:apply-templates select="upload_templates/upload_template[./product_code='IP' and ./invoice_type='SMP']"/>
		     	</xsl:with-param>
		     	<xsl:with-param name="required">Y</xsl:with-param>
				</xsl:call-template>
    ...
    <xsl:with-param name="content">
     <div class="widgetContainer">
      ...
      <xsl:call-template name="hidden-field">
       ...
       <xsl:with-param name="value">UPLOAD_SIMPLE</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="hidden-field">
