...
-->
	...
	<xsl:include href="../../../openaccount/xsl/products/rs.xsl"/>
	...
	<!-- Retrieve references from DB -->
		...
		<xsl:variable name="references" select="service:manageReferences(//product_code, //ref_id, //tnx_id, //bo_ref_id, //cust_ref_id, //company_id, //company_name, $reference, //issuing_bank/abbv_name, '01')"/>
		...
		<result>
			<com.misys.portal.openaccount.product.cn.common.CreditNote>
				...
				<xsl:if test="seller_reference">
					<seller_reference>
						<xsl:value-of select="utils:decryptApplicantReference($customer_bank_reference)"/>
					</seller_reference>
				</xsl:if>
				<!-- Buyer -->
			...
			<xsl:for-each select="//*[starts-with(name(), 'charge_details_chrg_details_')]">
				<xsl:call-template name="CHARGE">
					<xsl:with-param name="brchCode"><xsl:value-of select="//cn_tnx_record/brch_code"/></xsl:with-param>
	...
	<xsl:template name="invoice-details">
		...
		<xsl:variable name="invoiceReference" select="service:manageInvoiceReferences(invoice_ref_id)"/>
		<com.misys.portal.openaccount.product.baselinecn.util.CreditNoteInvoice>
			<xsl:attribute name="ref_id"><xsl:value-of select="$ref_id"/></xsl:attribute>
			<xsl:attribute name="tnx_id"><xsl:value-of select="$tnx_id"/></xsl:attribute>
			<xsl:if test="invoice_ref_id">
				<invoice_ref_id>
					<xsl:value-of select="$invoiceReference/references/invoice_ref_id"/>
				</invoice_ref_id>
			</xsl:if>
