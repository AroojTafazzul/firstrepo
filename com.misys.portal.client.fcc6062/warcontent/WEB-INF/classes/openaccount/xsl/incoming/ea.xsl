...
<!-- Retrieve references from DB -->
		...
		<xsl:variable name="references" select="service:manageReferences(//product_code, //ref_id, //tnx_id, //bo_ref_id, //cust_ref_id, //company_id, //company_name, //seller_reference, //issuing_bank/abbv_name, '01')"/>
		...
		<result>
			<com.misys.portal.openaccount.product.ea.common.ExportOpenAccount>
				...
				<xsl:if test="tnx_type_code">
					...
					<xsl:value-of select="tnx_type_code"/>
						<!-- <xsl:choose>
							<xsl:when test="tnx_type_code = '38' and sub_tnx_stat_code = '08' and tnx_stat_code = '04'">31</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="tnx_type_code"/>
							</xsl:otherwise>
						</xsl:choose> -->
				...
				<xsl:if test="prod_stat_code">
					<prod_stat_code>
						<xsl:choose>
							<xsl:when test="tnx_type_code = '38' and tnx_stat_code = '04' and prod_stat_code='98' and sub_tnx_stat_code = '05'">77</xsl:when>
							<xsl:otherwise><xsl:value-of select="prod_stat_code"/></xsl:otherwise>
						</xsl:choose>
					</prod_stat_code>
				</xsl:if>
				...
				<xsl:if test="(routing_summaries/rs_tnx_record/final_dest_place or routing_summaries/rs_tnx_record/taking_in_charge) and (routing_summaries/rs_tnx_record/final_dest_place != '' or routing_summaries/rs_tnx_record/taking_in_charge != '')">
			...
			<com.misys.portal.product.common.Narrative type_code="26">
				...
				<xsl:if test="bank_payment_obligation">
					<text>
						<xsl:value-of select="bank_payment_obligation" />
					</text>
				</xsl:if>
			</com.misys.portal.product.common.Narrative>
			...
			<!-- Buyer information and Seller Information -->
