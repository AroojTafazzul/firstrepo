...
-->
	<!-- Get the parameters -->
	<xsl:import href="../../../core/xsl/products/save_common.xsl"/>
	<!-- Common elements to save among all products -->
	<xsl:import href="../../../openaccount/xsl/products/po_save_common.xsl"/>
	...
	<!-- Retrieve references from DB -->
		...
		<xsl:variable name="reference"><xsl:value-of select="buyer_reference"/></xsl:variable>
		...
		<result>
			<com.misys.portal.openaccount.product.cr.common.CreditNoteCR>
				...
				<xsl:if test="fscm_program_code">
					<fscm_program_code>
						<xsl:value-of select="fscm_program_code"/>
					</fscm_program_code>
				</xsl:if>
			...
			<xsl:if test="invoice-items">
				<xsl:apply-templates select="invoice-items/invoice">
