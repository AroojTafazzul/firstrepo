				   		...
				   		<xsl:call-template name="fieldset-wrapper">
							...
							<xsl:with-param name="content">
					           ...
					           <xsl:if test="payment_amt !=''">
						            <xsl:call-template name="input-field">
						      	     <xsl:with-param name="label">XSL_PDF_PAYMENT_OBLIGATION_AMOUNT</xsl:with-param>
						             <xsl:with-param name="value"><xsl:value-of select="payment_amt"/></xsl:with-param>
						             <xsl:with-param name="value"><xsl:value-of select="total_net_cur_code"/>&nbsp;<xsl:value-of select="payment_amt"/></xsl:with-param>
						            </xsl:call-template>
					           </xsl:if>
					           <xsl:if test="payment_due_date !=''">
						            <xsl:call-template name="input-field">
						      	     <xsl:with-param name="label">XSL_PAYMENT_DUE_DATE</xsl:with-param>
						             <xsl:with-param name="value"><xsl:value-of select="payment_due_date"/></xsl:with-param>
						            </xsl:call-template>
					           </xsl:if>
			          		</xsl:with-param>
			          	</xsl:call-template>
	...
	<xsl:template name="general-details">
	 <xsl:variable name="changeoption"><xsl:value-of select="utils:getActionRequiredOption(product_code,'*',tnx_type_code,prod_stat_code)"/></xsl:variable>
		<xsl:variable name="lookup-type">
			<xsl:choose>
				<xsl:when test="tnx_type_code ='31'">crossref-full-resubmission</xsl:when>
			...
			<xsl:with-param name="content">
				...
				<xsl:if test="$show-cust-ref-id='Y'">
					<xsl:call-template name="input-field">
						...
						<xsl:with-param name="readonly"><xsl:if test="tnx_type_code[.='38']" >Y</xsl:if></xsl:with-param>
					</xsl:call-template>
				</xsl:if>
