...
-->
	...
	<xsl:choose>
		<!-- Uncomment the following for PO details editing
				...
				<xsl:if test="total_net_amt[.!='']">
					...
					<table border="0" cellpadding="0" cellspacing="0" width="570">		
						...
						<tr>
							...
							<td >
								<font class="REPORTBLUE"><xsl:value-of select="liab_total_net_cur_code"/>&nbsp;<xsl:value-of select="liab_total_net_amt"/></font>
							</td>
						</tr>
					</table>
		...
		<xsl:if test="count(/po_tnx_record/routing_summaries/routing_summary) != 0">
