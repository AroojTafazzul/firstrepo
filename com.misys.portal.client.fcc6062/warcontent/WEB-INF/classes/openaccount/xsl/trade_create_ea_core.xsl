...
<xsl:stylesheet version="1.0"
				...
				<xsl:with-param name="content">						
					...
					<xsl:when test="prod_stat_code != 'A6'"> -->
						...
						<div id="transactionDetails">
							...
							<!-- Bank Payment Obligation Details(BPO)  -->
							<xsl:if test="prod_stat_code[.='98' or .='75' or .='' or .='A6'] and($displaymode='edit' or ($displaymode='view' and ./bank_payment_obligation/PmtOblgtn))">
					...
					<xsl:call-template name="fieldset-wrapper">
		          	...
		          	<!-- Bank Instructions -->
		          	<xsl:if test="prod_stat_code[.='A6'] or (mismatches != '' and mismatches/Rpt != '')">
				      <xsl:call-template name="bank-instructions">
	...
	<xsl:template name="payment-terms">
		<xsl:call-template name="fieldset-wrapper">
			...
			<xsl:with-param name="content">
			<xsl:if test="not(tnx_type_code) or tnx_type_code[.='49' or .='63']">
			&nbsp;
				<xsl:if test="$displaymode='edit'">
					<xsl:choose>
	...
	<xsl:template name="po-general-details">
		...
		<xsl:if test="prod_stat_code ='98' or prod_stat_code ='75' or prod_stat_code ='A6'">
			<xsl:if test="prod_stat_code ='98' or prod_stat_code ='75'">
				<xsl:call-template name="checkbox-field">
					     <xsl:with-param name="label">XSL_PROVISIONAL</xsl:with-param>
					     <xsl:with-param name="name">provisional_status</xsl:with-param>
					     <xsl:with-param name="readonly">Y</xsl:with-param>
		...
		<!-- Commercial Document Reference Block -->
		<xsl:if test="prod_stat_code[.='A6' or .='AA']">
	...
	<div id ="buyerSellerDetails">
		...
		<xsl:call-template name="checkbox-field">
			...
			<xsl:with-param name="checked"><xsl:if test="bill_to_name[. != ''] or ship_to_name[. != ''] or consgn_to_name[. != ''] or consgnor_name[. != ''] or prod_stat_code[.='A6']">Y</xsl:if></xsl:with-param>
			<xsl:with-param name="readonly"><xsl:if test="prod_stat_code[.='A6']">Y</xsl:if></xsl:with-param>
		</xsl:call-template>
		</xsl:if>
