...
Templates for
 ...
 Credit Note (CR) Upload Form, COunterparty Side.
...
<xsl:stylesheet 
  ...
  <xsl:template name="js-imports">
   <xsl:call-template name="common-js-imports">
    <xsl:with-param name="binding">misys.binding.openaccount.upload_cr</xsl:with-param>
    <xsl:with-param name="override-help-access-key">CR_01_UPL</xsl:with-param>
   </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template name="hidden-fields">
   ...
   <xsl:call-template name="hidden-field">
				...
				<xsl:if test="$show-cust-ref-id='Y'">
					<xsl:call-template name="input-field">
						<xsl:with-param name="label">
							<xsl:value-of select="$override-cust-ref-id-label" />
						</xsl:with-param>
	...
	<xsl:template name="template-details">
		<xsl:call-template name="fieldset-wrapper">
			<xsl:with-param name="legend">XSL_HEADER_CR_TEMPLATE_DETAILS</xsl:with-param>
			<xsl:with-param name="content">

				<xsl:call-template name="select-field">
				<xsl:with-param name="label">XSL_CR_UPLOAD_TEMPLATE</xsl:with-param>
		     	...
		     	<xsl:with-param name="options">
		     		<xsl:apply-templates select="upload_templates/upload_template[./product_code='CR']"/>
		     	</xsl:with-param>
		     	<xsl:with-param name="required">Y</xsl:with-param>
				</xsl:call-template>
