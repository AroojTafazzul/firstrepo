...
<xsl:stylesheet 
  ...
  <xsl:template name="js-imports">
   <xsl:call-template name="common-js-imports">
    <xsl:with-param name="binding">misys.binding.openaccount.upload_ip</xsl:with-param>
    <xsl:with-param name="override-help-access-key">IP_01_UPL</xsl:with-param>
   </xsl:call-template>
  </xsl:template>
			...
			<xsl:with-param name="content">
				...
				<xsl:with-param name="label">XSL_INVOICE_UPLOAD_TEMPLATE</xsl:with-param>
		     	...
		     	<xsl:with-param name="options">
		     		<xsl:apply-templates select="upload_templates/upload_template[./product_code='IP' and ./invoice_type='ISO']"/>
		     	</xsl:with-param>
		     	<xsl:with-param name="required">Y</xsl:with-param>
				</xsl:call-template>
