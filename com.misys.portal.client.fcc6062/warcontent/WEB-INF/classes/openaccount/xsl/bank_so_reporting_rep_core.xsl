...
-->
	...
	<!-- View SO Transaction Details - BEGIN -->
	...
	<td align="left">
		...
		<!--Insert the Company Id as hidden field-->
		<input type="hidden" name="company_id"><xsl:attribute name="value"><xsl:value-of select="company_id"/></xsl:attribute></input>
		<input type="hidden" name="lc_type"><xsl:attribute name="value"><xsl:value-of select="lc_type"/></xsl:attribute></input>
		...
		<table border="0" width="570" cellpadding="0" cellspacing="0">
			...
			<xsl:if test="issuer_ref_id[.!='']">
				<tr>
					...
					<td>
						<font class="REPORTDATA"><xsl:value-of select="issuer_ref_id"/></font>
					</td>
				</tr>	
			</xsl:if>
			<tr><td>&nbsp;</td></tr>
			...
			<tr>
				<td width="40">&nbsp;</td>
				<td width="150"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_TID')"/></td>
				<td>
					<font class="REPORTDATA"><xsl:value-of select="tid"/></font>
				</td>
			</tr>
		</table>
		...
		<xsl:if test="count(/so_tnx_record/routing_summaries/routing_summary/transport_type) != 0">
		...
		<table border="0" width="570" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td>
					<font class="REPORTDATA">
						<xsl:choose>
							<xsl:when test="count(/so_tnx_record/routing_summaries/routing_summary/transport_type[. = '01']) != 0">
								<xsl:value-of select="localization:getDecode($language, 'N213', '01')"/>
							</xsl:when>
							<xsl:when test="count(/so_tnx_record/routing_summaries/routing_summary/transport_type[. = '02']) != 0">
