...
<xd:doc>
		...
		<div>
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="content">						
					...
					<xsl:choose>
						<xsl:when test="tnx_type_code and tnx_type_code = '32' and $mode='ACCEPT'">
					...
					<xsl:call-template name="general-details" />
						...
						<div id="transactionDetails">
							...
							<xsl:if test="tnx_type_code ='03' and ($displaymode='edit' or ($displaymode='view' and amd_details/text !=''))">
								<xsl:call-template name="amend-narrative"/>
							</xsl:if> 
							
							<xsl:if test="tnx_type_code[.='32'] and delta_report != '' and delta_report/DltaRpt != ''">
								<xsl:call-template name="delta-report-declaration">
						   			<xsl:with-param name="node" select="delta_report/DltaRpt" />
						   		</xsl:call-template>
					   		</xsl:if>
						</div>
						<!-- comments for return -->
						<xsl:call-template name="comments-for-return">
					   	...
					   	<!-- comments for reject -->
					   <!-- 	<xsl:if test="tnx_type_code and tnx_type_code = '32' and $mode='ACCEPT'">
							<xsl:call-template name="comments-for-reject">
							 	<xsl:with-param name="value"><xsl:value-of select="reject_comments"/></xsl:with-param>
							 	<xsl:with-param name="mode"><xsl:value-of select="$mode"/></xsl:with-param>
						   	</xsl:call-template>
					   	</xsl:if> --> 
					   	
					   		<!-- reject comments are part of instructions to bank, its mandatory for rejection and optional for amendment -->
					   	<xsl:if test="tnx_type_code and (tnx_type_code = '32' or tnx_type_code='03') or (tnx_type_code = '32' and $mode='ACCEPT')">
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.openaccount.amend_io</xsl:with-param>
	...
	<xsl:template name="hidden-fields">
		...
		<div class="widgetContainer">
			...
			<xsl:if test="sub_tnx_type_code = '69'">
				<xsl:call-template name="hidden-field">
	...
	<xsl:template name="general-details">
	 <xsl:variable name="changeoption"><xsl:value-of select="utils:getActionRequiredOption(product_code,'*',tnx_type_code,prod_stat_code)"/></xsl:variable>
		<xsl:variable name="lookup-type">
			<xsl:choose>
				<xsl:when test="tnx_type_code ='32'">crossref-full-amendment</xsl:when>
			...
			<xsl:with-param name="content">
				...
				<xsl:if test="$show-cust-ref-id='Y'">
					<xsl:call-template name="input-field">
						...
						<xsl:with-param name="readonly"><xsl:if test="tnx_type_code[.='03']" >Y</xsl:if></xsl:with-param>
					</xsl:call-template>
				</xsl:if>
	...
	<xd:doc>
		...
		<xd:detail>
			This template contains PO Purchase Order General Details Section which contains issue date,expiry date,Provisional Status,TMA used status,buyer details,seller details etc. 
 		</xd:detail>
	</xd:doc>
				...
				<xsl:call-template name="fieldset-wrapper">
					...
					<xsl:with-param name="content">
						<xsl:call-template name="party-details">
							...
							<xsl:with-param name="readonly-bank-bic">
								<xsl:choose>
									<xsl:when test="tnx_type_code[.='03']">Y</xsl:when>
									<xsl:otherwise>N</xsl:otherwise>
								</xsl:choose>
