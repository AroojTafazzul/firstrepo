...
<!-- Additional JS imports for this form are -->
 ...
 <xsl:template name="js-imports">
  <xsl:call-template name="system-common-js-imports">
   ...
   <xsl:with-param name="binding">misys.binding.openaccount.simplified_upload_in_template</xsl:with-param>
...
<xsl:template name="avail_column">
 ...
 <xsl:template name="avail_columns">
		...
		<xsl:call-template name="avail_column">
			...
			<xsl:with-param name="column_name_label">
				<xsl:choose>
					<xsl:when test="$productcode = 'IN'">invoice_ref_id</xsl:when>
		...
		<xsl:with-param name="column_type">Date</xsl:with-param>
			<xsl:with-param name="column_name_label">
				<xsl:choose>
					<xsl:when test="$productcode = 'IN'">invoice_date</xsl:when>
		...
		<xsl:call-template name="avail_column">
			...
			<xsl:with-param name="column_type">Date</xsl:with-param>
			<xsl:with-param name="column_name_label">simplified_discount_exp_date</xsl:with-param>
		</xsl:call-template>
		</xsl:if>
 </xsl:template>
 ...
 dojo.ready(function()
 {
    ...
    misys._config.arrColumn["discount_exp_date"] = new Array("Date", false);
...
<xsl:template match="column" mode="list">
	<xsl:variable name="codeval">
		<xsl:choose>
			<xsl:when test="name='iss_date' and $productcode='IN'">XSL_REPORT_COL_invoice_date</xsl:when>
			<xsl:when test="name='issuer_ref_id' and  $productcode='IN'">XSL_REPORT_COL_invoice_ref_id</xsl:when>
			<xsl:when test="name='discount_exp_date' and  $productcode='IN'">XSL_REPORT_COL_simplified_discount_exp_date</xsl:when>
