...
<xsl:stylesheet 
  ...
  <xsl:template match="bg_tnx_record">
   ...
   <xsl:variable name="screen-name">BankerGuaranteeScreen</xsl:variable>
  ...
  <xsl:template name="js-imports">
   ...
   <xsl:call-template name="common-js-imports">
    ...
    <xsl:choose>
    	<xsl:when test="(org_previous_file/bg_tnx_record/prod_stat_code[.='86'] or prod_stat_code[.='84'] or sub_tnx_type_code[.='62' or .='63' or .='25']) and sub_tnx_type_code != '24'">misys.binding.trade.message_bg</xsl:when>
    ...
    <xsl:with-param name="override-help-access-key">BG_02</xsl:with-param>
   </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template name="hidden-fields">
   <xsl:param name="lowercase-product-code"/>  
  	<xsl:variable name="show-tnx-amt">
  		<xsl:choose>
  			<xsl:when test="org_previous_file/bg_tnx_record/prod_stat_code[.='86']">N</xsl:when>
   ...
   <xsl:call-template name="localization-dialog"/>
    <xsl:if test="$displaymode='edit'">
     <xsl:call-template name="hidden-field">
      ...
      <xsl:with-param name="value"><xsl:value-of select="org_previous_file/bg_tnx_record/prod_stat_code"/></xsl:with-param>
     </xsl:call-template>
       <xsl:if test="entity[. != '']">
     <xsl:call-template name="hidden-field">
    ...
    <xsl:call-template name="hidden-field">
     ...
     <xsl:with-param name="value"><xsl:value-of select="//bg_cur_code" /></xsl:with-param>
    </xsl:call-template>
    <xsl:call-template name="hidden-field">
     <xsl:with-param name="name">bg_amt</xsl:with-param>
     <xsl:with-param name="value"><xsl:value-of select="//bg_amt" /></xsl:with-param>
    </xsl:call-template>
    
    <!-- Original Expiry Date -->
    <xsl:call-template name="hidden-field">
     <xsl:with-param name="name">org_exp_date</xsl:with-param>
     <xsl:with-param name="value"><xsl:value-of select="org_previous_file/bg_tnx_record/exp_date" /></xsl:with-param>
    </xsl:call-template>
  ...
  <xsl:template name="general-details">
   <xsl:call-template name="message-general-details">
	...
	<!-- Claim Presentation -->
	<xsl:choose>
 	<xsl:when test="(prod_stat_code ='84' or sub_tnx_type_code[.='62' or .='63' or .='25']) and sub_tnx_type_code != '24' and securitycheck:hasPermission($rundata,'bg_claim_access') = 'true'">
        ...
        <xsl:if test="org_previous_file/bg_tnx_record/claim_cur_code[.!=''] and org_previous_file/bg_tnx_record/claim_amt[.!='']">
        <xsl:call-template name="input-field">
         <xsl:with-param name="label">XSL_CLAIM_AMOUNT_LABEL</xsl:with-param>
         <xsl:with-param name="id">claim_amt_view</xsl:with-param>
         <xsl:with-param name="value"><xsl:value-of select="claim_cur_code"/>&nbsp;<xsl:value-of select="org_previous_file/bg_tnx_record/claim_amt"/></xsl:with-param>
         <xsl:with-param name="override-displaymode">view</xsl:with-param>
        </xsl:call-template>
        <xsl:if test="$displaymode='edit'">
	        <xsl:call-template name="hidden-field">
			  	<xsl:with-param name="name">claim_amt</xsl:with-param>
			  	<xsl:with-param name="value"><xsl:value-of select="org_previous_file/bg_tnx_record/claim_amt"/></xsl:with-param>
			</xsl:call-template>
        </xsl:if>
        </xsl:if>
 	...
 	<xsl:choose>
        ...
        <xsl:if test="($displaymode='view' and sub_tnx_type_code[.='25' or .='62']) or $displaymode='edit'">
	...
	<xsl:when test="org_previous_file/bg_tnx_record/prod_stat_code[.='86'] and sub_tnx_type_code != '24'">
	 	<xsl:call-template name="select-field">
   	 ...
   	 <xsl:when test="org_previous_file/bg_tnx_record/prod_stat_code[.='78' or .='79'] or sub_tnx_type_code[.='88' or .='89']">
	  <xsl:call-template name="select-field">
	     ...
	     <xsl:with-param name="options">
	      <xsl:choose>
		    <xsl:when test="$displaymode='edit'"> 		        
		       <xsl:choose>
		       <xsl:when test="org_previous_file/bg_tnx_record/prod_stat_code[.='78']">
			       <option value="88">
		    ...
		    <xsl:otherwise>
		     <xsl:choose>
		         <xsl:when test="sub_tnx_type_code[.='88'] and org_previous_file/bg_tnx_record/prod_stat_code[.='78']"><xsl:value-of select="localization:getGTPString($language, 'XSL_WUR_YES')"/></xsl:when>
		         <xsl:when test="sub_tnx_type_code[.='88'] and org_previous_file/bg_tnx_record/prod_stat_code[.='79']"><xsl:value-of select="localization:getGTPString($language, 'XSL_FW_YES')"/></xsl:when>
		         <xsl:when test="sub_tnx_type_code[.='89'] and org_previous_file/bg_tnx_record/prod_stat_code[.='78']"><xsl:value-of select="localization:getGTPString($language, 'XSL_WUR_NO')"/></xsl:when>
		         <xsl:when test="sub_tnx_type_code[.='89'] and org_previous_file/bg_tnx_record/prod_stat_code[.='79']"><xsl:value-of select="localization:getGTPString($language, 'XSL_FW_NO')"/></xsl:when>
		        </xsl:choose>
		    </xsl:otherwise>
	   	  </xsl:choose>
	     </xsl:with-param>
	...
	<xsl:otherwise>
 	...
 	<xsl:choose>
     	<xsl:when test="org_previous_file/bg_tnx_record/prod_stat_code[.='86'] and sub_tnx_type_code != '24'">
			<xsl:call-template name="input-field">
		      ...
		      <xsl:with-param name="id">product_status_view</xsl:with-param>
		      <xsl:with-param name="value">
		       <xsl:choose>
     ...
     <xsl:if test="org_previous_file/bg_tnx_record/prod_stat_code[.='86'] and ($displaymode = 'edit' or tnx_amt[.!='']) and sub_tnx_type_code != '24'">    
