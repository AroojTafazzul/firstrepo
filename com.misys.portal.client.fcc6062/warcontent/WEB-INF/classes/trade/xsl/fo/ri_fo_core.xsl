...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="ri_content_fo.xsl" />
	<xsl:template match="/">
			...
			<xsl:call-template name="general-layout-master" />
			    ...
			    <fo:bookmark-tree>
			        ...
			        <fo:bookmark internal-destination="lidetails">
			            <fo:bookmark-title><xsl:value-of
								select="localization:getGTPString($language, 'XSL_HEADER_RI_DETAILS')" />
						</fo:bookmark-title>
			        </fo:bookmark>
			...
			<fo:page-sequence initial-page-number="1"
				...
				<fo:static-content text-align="center" flow-name="xsl-region-start">
					<fo:block font-size="8pt" text-align="center" color="#FFFFFF"
						...
						<xsl:value-of select="localization:getDecode($language, 'N001',ri_tnx_record/product_code[.])"/>
					</fo:block>
				</fo:static-content>
				<!--Apply ri_content_fo.xsl on ri_tnx_record-->
