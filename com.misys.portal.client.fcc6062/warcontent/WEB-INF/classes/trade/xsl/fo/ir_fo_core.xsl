...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="ir_content_fo.xsl" />
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="{$pdfFont}" writing-mode="{$writingMode}">
		    ...
		    <fo:bookmark-tree>
		        ...
		        <fo:bookmark internal-destination="remittancebankdetail">
		            <fo:bookmark-title><xsl:value-of
								select="localization:getGTPString($language, 'XSL_BANKDETAILS_INWARD_REMITTANCE_TAB_REMITTANCE_BANK')" />
					</fo:bookmark-title>
		        </fo:bookmark>
		        <fo:bookmark internal-destination="issuingbankdetail">
		            <fo:bookmark-title><xsl:value-of
								select="localization:getGTPString($language, 'XSL_BANKDETAILS_INWARD_REMITTANCE_TAB_ISSUING_BANK')" />
					</fo:bookmark-title>
		        </fo:bookmark>
		        <fo:bookmark internal-destination="payamentdetails">
		            <fo:bookmark-title><xsl:value-of
			...
			<fo:page-sequence initial-page-number="1"
				...
				<fo:static-content text-align="center" flow-name="xsl-region-start">
					<fo:block font-size="8pt" text-align="center" color="#FFFFFF"
						...
						<xsl:value-of select="localization:getDecode($language, 'N001', ir_tnx_record/product_code[.])"/>
					</fo:block>
				</fo:static-content>
				<!--Apply ir_content_fo.xsl on ir_tnx_record-->
