...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="ls_content_fo.xsl" />
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="{$pdfFont}" writing-mode="{$writingMode}">
			...
			<xsl:call-template name="general-layout-master" />
			    ...
			    <fo:bookmark-tree>
			        ...
			        <fo:bookmark internal-destination="validitydetails">
		            <fo:bookmark-title><xsl:value-of
									select="localization:getGTPString($language, 'XSL_VALIDITY_DETAILS_LABEL')" />
					</fo:bookmark-title>
			        </fo:bookmark>
			...
			<fo:page-sequence initial-page-number="1"
				...
				<fo:static-content text-align="center" flow-name="xsl-region-start">
					<fo:block font-size="8pt" text-align="center" color="#FFFFFF"
						...
						<xsl:value-of select="localization:getDecode($language, 'N001', ls_tnx_record/product_code[.])"/>
					</fo:block>
				</fo:static-content>
				<!--Apply ls_content_fo.xsl on ls_tnx_record-->
