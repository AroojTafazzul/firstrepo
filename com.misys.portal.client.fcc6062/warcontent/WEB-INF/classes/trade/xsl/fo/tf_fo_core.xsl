...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="tf_content_fo.xsl" />
	...
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="{$pdfFont}" writing-mode="{$writingMode}">
			...
			<fo:bookmark-tree>
			        ...
			        <fo:bookmark internal-destination="additionnaldetails">
			            <fo:bookmark-title><xsl:value-of
										select="localization:getGTPString($language, 'XSL_HEADER_ADDITIONAL_DETAILS')" />
						</fo:bookmark-title>
			        </fo:bookmark>
			    </fo:bookmark-tree>
			...
			<fo:page-sequence initial-page-number="1"
				...
				<fo:static-content text-align="center" flow-name="xsl-region-start">
					<fo:block font-size="8pt" text-align="center" color="#FFFFFF"
						...
						<xsl:value-of select="localization:getDecode($language, 'N001', tf_tnx_record/product_code[.])"/>
					</fo:block>
				</fo:static-content>
				<!--Apply tf_content_fo.xsl on tf_tnx_record-->
