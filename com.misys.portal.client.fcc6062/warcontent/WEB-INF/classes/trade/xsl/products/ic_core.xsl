	...
	<xsl:template match="ic_tnx_record">
		<result>
			<com.misys.portal.product.ic.common.ImportCollection>
				...
				<xsl:if test="drawee_reference">
					<drawee_reference>
                        <xsl:value-of select="utils:decryptApplicantReference(drawee_reference)"/>
					</drawee_reference>
				</xsl:if>
     ...
     <xsl:call-template name="product-additional-fields"/>
			...
			<com.misys.portal.product.common.Bank role_code="08">
				...
				<xsl:if test="presenting_abbv_name">
					<abbv_name>
						<xsl:value-of select="presenting_abbv_name"/>
					</abbv_name>
				</xsl:if>
				<xsl:if test="presenting_name">
					<name>
						<xsl:value-of select="presenting_name"/>
					</name>
				</xsl:if>
				<xsl:if test="presenting_address_line_1">
					<address_line_1>
						<xsl:value-of select="presenting_address_line_1"/>
					</address_line_1>
				</xsl:if>
				<xsl:if test="presenting_address_line_2">
					<address_line_2>
						<xsl:value-of select="presenting_address_line_2"/>
					</address_line_2>
				</xsl:if>
				<xsl:if test="presenting_address_line_4">
					<address_line_4>
						<xsl:value-of select="presenting_address_line_4"/>
					</address_line_4>
				</xsl:if>
				<xsl:if test="presenting_dom">
					<dom>
						<xsl:value-of select="presenting_dom"/>
					</dom>
				</xsl:if>
				<xsl:if test="presenting_iso_code">
					<iso_code>
						<xsl:value-of select="presenting_iso_code"/>
					</iso_code>
				</xsl:if>
				<xsl:if test="presenting_reference">
					<reference>
						<xsl:value-of select="presenting_reference"/>
					</reference>
				</xsl:if>
			</com.misys.portal.product.common.Bank>
			...
			<xsl:for-each select="//*[starts-with(name(), 'documents_details_code_')]">
				<xsl:call-template name="DOCUMENT">
					<xsl:with-param name="brchCode"><xsl:value-of select="//ic_tnx_record/brch_code"/></xsl:with-param>
					<xsl:with-param name="companyId"><xsl:value-of select="//ic_tnx_record/company_id"/></xsl:with-param>
					<xsl:with-param name="refId"><xsl:value-of select="//ic_tnx_record/ref_id"/></xsl:with-param>
					<xsl:with-param name="tnxId"><xsl:value-of select="//ic_tnx_record/tnx_id"/></xsl:with-param>
