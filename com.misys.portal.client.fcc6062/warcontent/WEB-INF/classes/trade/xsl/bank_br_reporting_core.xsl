...
<xsl:stylesheet 
    ...
    exclude-result-prefixes="localization utils securitycheck security">
  ...
  <xsl:param name="product-code">BR</xsl:param> 
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="br_tnx_record"/>
  </xsl:template>
      ...
      <div id="transactionDetails">
       ...
       <xsl:call-template name="form-wrapper">
        ...
        <xsl:with-param name="content">
         ...
         <xsl:call-template name="br-renewal-details"/>
         ...
         <xsl:call-template name="br-guarantee-details"/>
         <xsl:call-template name="br-contract-details"/>
 ...
 <xsl:template name="js-imports">
  <xsl:call-template name="common-js-imports">
   <xsl:with-param name="binding">misys.binding.bank.report_br</xsl:with-param>
    <xsl:with-param name="override-help-access-key">
	   <xsl:choose>
	   	<xsl:when test="$option ='EXISTING'">ER_01</xsl:when>
	   	<xsl:otherwise>BR_BANK</xsl:otherwise>
	   </xsl:choose>
	</xsl:with-param>
  </xsl:call-template>
 </xsl:template>
 ...
 <xsl:template name="hidden-fields">
  <xsl:call-template name="common-hidden-fields">
   <xsl:with-param name="show-type">N</xsl:with-param>
   <xsl:with-param name="override-product-code">br</xsl:with-param>
  </xsl:call-template>
 ...
 <xsl:template name="general-details">
  <xsl:call-template name="fieldset-wrapper">
   ...
   <xsl:value-of select="ref_id"/></div>
    ...
    <xsl:call-template name="input-field">
     ...
     <xsl:if test="$isMT798 = 'Y'">
	    <xsl:call-template name="input-field">
			 ...
			 <xsl:with-param name="name">related_ref</xsl:with-param>
	...
	<xsl:if test="bo_ref_id[.!=''] and $displaymode='view' and (not(tnx_id) or tnx_type_code[.!='01'] or preallocated_flag[.='Y'])">
	     <xsl:call-template name="input-field">
    ...
    <xsl:call-template name="fieldset-wrapper">
     ...
     <xsl:with-param name="content">
      <xsl:call-template name="address">
       ...
       <xsl:with-param name="beneficiary-reference">Y</xsl:with-param>
    ...
    <xsl:call-template name="fieldset-wrapper">
     ...
     <xsl:with-param name="content">
      <xsl:call-template name="address">
       ...
       <xsl:with-param name="search-button-type"><xsl:if test="tnx_type_code[.='01']">applicant</xsl:if></xsl:with-param>
       <xsl:with-param name="show-reference">Y</xsl:with-param>
       <xsl:with-param name="required">Y</xsl:with-param>
 ...
 <xsl:template name="br-renewal-details">
    <xsl:call-template name="fieldset-wrapper">
    ...
    <xsl:with-param name="content">
     ...
     <xsl:choose>
      ...
      <xsl:call-template name="select-field">
      <xsl:with-param name="label">XSL_RENEWAL_RENEWAL_ON</xsl:with-param>
       ...
       <xsl:choose>
    		<xsl:when test="$displaymode='edit'">
		        <xsl:call-template name="radio-field">
		         ...
		         <xsl:with-param name="readonly">Y</xsl:with-param>
		         <xsl:with-param name="checked"><xsl:if test="renew_amt_code[. = '01']">Y</xsl:if></xsl:with-param>
		        </xsl:call-template>
		        <xsl:call-template name="radio-field">
		         ...
		         <xsl:with-param name="readonly">Y</xsl:with-param>
		         <xsl:with-param name="checked"><xsl:if test="renew_amt_code[. = '02'] or renew_amt_code[. = '']">Y</xsl:if></xsl:with-param>
		        </xsl:call-template>
       		</xsl:when>
    		<xsl:otherwise>
     			<xsl:choose>
 ...
 <xsl:choose>
	 <xsl:when test="$displaymode != 'edit'"> 
		  <xsl:call-template name="tabgroup-wrapper">
		   ...
		   <!-- Tab 1_1 - Issuing Bank  -->
		   <xsl:with-param name="tab1-label">XSL_BANKDETAILS_TAB_ISSUING_BANK</xsl:with-param>
 ...
 <xsl:template match=" confirming_bank">
 ...
 <xsl:template match="issuing_bank">
  <xsl:param name="prefix"/>
   ...
   <div>
	...
	<xsl:if test="$displaymode = 'view' and reference[.!='']">
		<xsl:call-template name="row-wrapper">
			<xsl:with-param name="label">XSL_PARTIESDETAILS_REFERENCE</xsl:with-param>
			<xsl:with-param name="content"><div class="content">
			<xsl:value-of select="reference"/></div>
		</xsl:with-param>
		</xsl:call-template>
	</xsl:if>   
 </xsl:template>
