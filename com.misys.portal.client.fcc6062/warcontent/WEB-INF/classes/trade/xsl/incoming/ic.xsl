...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="ic_tnx_record">
		...
		<xsl:variable name="references" select="service:manageTradeReferences(//product_code, //ref_id, //tnx_id, //bo_ref_id, //bo_tnx_id, //cust_ref_id, //company_id, //company_name, //drawee_reference, //presenting_bank/abbv_name, '08', //prod_stat_code, //tnx_type_code)"/>
		...
		<result>
			<com.misys.portal.product.ic.common.ImportCollection>
				...
				<xsl:if test="ic_cur_code">
					<ic_cur_code>
						<xsl:value-of select="ic_cur_code"/>
					</ic_cur_code>
				</xsl:if>
				<xsl:if test="ic_amt">
					<ic_amt>
				...
				<xsl:choose>
					<xsl:when test="ic_liab_amt">
						<ic_liab_amt>
							<xsl:value-of select="ic_liab_amt"/>
						</ic_liab_amt>
					</xsl:when>
					<xsl:when test="ic_outstanding_amt">
						<ic_liab_amt>
							<xsl:value-of select="ic_outstanding_amt"/>
						</ic_liab_amt>
					</xsl:when>
					<xsl:otherwise/>
				</xsl:choose>
				...
				<xsl:if test="ic_type_code">
					<ic_type_code>
						<xsl:value-of select="ic_type_code"/>
					</ic_type_code>
				</xsl:if>
			...
			<com.misys.portal.product.common.Bank role_code="08">
				...
				<xsl:if test="presenting_bank/name"><name><xsl:value-of select="presenting_bank/name"/></name></xsl:if>
				<xsl:if test="$main_bank_name and not(presenting_bank/name)"><name><xsl:value-of select="$main_bank_name"/></name></xsl:if>
				<xsl:if test="presenting_bank/address_line_1"><address_line_1><xsl:value-of select="presenting_bank/address_line_1"/></address_line_1></xsl:if>
				<xsl:if test="presenting_bank/address_line_2"><address_line_2><xsl:value-of select="presenting_bank/address_line_2"/></address_line_2></xsl:if>
				<xsl:if test="presenting_bank/dom"><dom><xsl:value-of select="presenting_bank/dom"/></dom></xsl:if>
				<xsl:if test="presenting_bank/iso_code"><iso_code><xsl:value-of select="presenting_bank/iso_code"/></iso_code></xsl:if>
				<xsl:if test="presenting_bank/reference"><reference><xsl:value-of select="presenting_bank/reference"/></reference></xsl:if>
			</com.misys.portal.product.common.Bank>
						
			<xsl:call-template name="bank">
				<xsl:with-param name="bank" select="remitting_bank"/>
			...
			<!-- Linked Licenses -->
 	 	 	 <xsl:if test="linked_licenses">
 	 	 	 	<xsl:apply-templates select="linked_licenses/license">
 	 	 	 		...
 	 	 	 		<xsl:with-param name="tnx_id" select="$tnx_id"/>
 	 	 	 		<xsl:with-param name="company_id" select="$company_id"/>
 	 	 	 		<xsl:with-param name="main_bank_name" select="//presenting_bank/abbv_name"/>
 	 	 	 	</xsl:apply-templates>
 	 	 	 </xsl:if>
		</result>
	</xsl:template>
