...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="ec_tnx_record">
		...
		<sections>
			...
			<xsl:call-template name="drawee-details"/>
		</sections>
		<sections>
			<label>
			...
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"><xsl:value-of select="localization:getGTPString($language, 'XSL_AMOUNTDETAILS_ORG_EC_AMT_LABEL')"/></xsl:with-param>
						<xsl:with-param name="value"><xsl:value-of select="lc_cur_code"/>&nbsp;<xsl:value-of select="ec_amt"/></xsl:with-param>
			</xsl:call-template>	
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"></xsl:with-param>
						<xsl:with-param name="value"></xsl:with-param>
		...
		<sections>
			...
			<xsl:call-template name="remitting-bank-details"/>
		</sections>
