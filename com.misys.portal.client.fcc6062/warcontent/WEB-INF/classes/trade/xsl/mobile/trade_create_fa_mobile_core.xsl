...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="fa_tnx_record">
		...
		<sections>
			...
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"><xsl:value-of select="localization:getGTPString($language, 'XSL_AMOUNTDETAILS_FA_AMT_LABEL')"/></xsl:with-param>
						<xsl:with-param name="value"><xsl:value-of select="fa_cur_code"/>&nbsp;<xsl:value-of select="fa_amt"/></xsl:with-param>
			</xsl:call-template>	
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"></xsl:with-param>
						<xsl:with-param name="value"></xsl:with-param>
