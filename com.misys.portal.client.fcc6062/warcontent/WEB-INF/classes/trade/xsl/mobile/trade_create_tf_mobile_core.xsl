...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="tf_tnx_record">
		...
		<sections>
			...
			<xsl:call-template name="mobile-field">
					<xsl:with-param name="label" select="localization:getGTPString($language, 'XSL_GENERALDETAILS_REQUESTED_ISSUE_DATE')"/>
					<xsl:with-param name="value" select="iss_date"/>
			</xsl:call-template>
			<xsl:call-template name="mobile-field">
					<xsl:with-param name="label" select="localization:getGTPString($language, 'XSL_GENERALDETAILS_MATURITY_DATE')"/>
		...
		<sections>
			...
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"><xsl:value-of select="localization:getGTPString($language, 'XSL_AMOUNTDETAILS_FIN_AMT_LABEL')"/></xsl:with-param>
