...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="la_tnx_record">
		...
		<sections>
			...
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"><xsl:value-of select="localization:getGTPString($language, 'XSL_TD_AMOUNT_LABEL')"/></xsl:with-param>
						<xsl:with-param name="value"><xsl:value-of select="la_cur_code"/>&nbsp;<xsl:value-of select="la_amt"/></xsl:with-param>
			</xsl:call-template>	
			<xsl:call-template name="mobile-field">
						<xsl:with-param name="label"></xsl:with-param>
						<xsl:with-param name="value"></xsl:with-param>
