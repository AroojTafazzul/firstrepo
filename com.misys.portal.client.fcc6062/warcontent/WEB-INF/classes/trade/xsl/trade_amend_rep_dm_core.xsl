...
<xsl:import href="trade_common.xsl"/>
  ...
  <xsl:param name="modules"/>
	...
	<xsl:template match="/">
		...
		<script type="text/javascript" src="/content/OLD/javascript/trade_amend_dm.js"></script>
						...
						<tr>
							...
							<td align="middle" valign="center">
								<a href="javascript:void(0)">
									<xsl:attribute name="onclick">fncShowPreview('FULL','<xsl:value-of select="product_code"/>','<xsl:value-of select="ref_id"/>','<xsl:value-of select="tnx_id"/>','DMReportingPopup');return false;	</xsl:attribute>
									<img border="0" >
	...
	<td align="center">
					...
					<table border="0" cellspacing="2" cellpadding="8">
						<tr>
							...
							<td align="middle" valign="center">
								<a href="javascript:void(0)">
									<xsl:attribute name="onclick">fncShowPreview('FULL','<xsl:value-of select="product_code"/>','<xsl:value-of select="ref_id"/>','<xsl:value-of select="tnx_id"/>');return false;</xsl:attribute>
									<img border="0" >
										<xsl:attribute name="src"><xsl:value-of select="utils:getImagePath($printerImage)"/></xsl:attribute>
									</img><br/>
									<xsl:value-of select="localization:getGTPString($language, 'XSL_ACTION_PREVIEW')"/>
    ...
    <xsl:if test="securityCheck:hasPermission(utils:getUserACL($rundata),'collaboration_client_access',utils:getUserEntities($rundata)) or securityCheck:hasPermission(utils:getUserACL($rundata),'collaboration_bank_access',utils:getUserEntities($rundata)) or securityCheck:hasPermission(utils:getUserACL($rundata),'collaboration_counterparty_access',utils:getUserEntities($rundata))">
