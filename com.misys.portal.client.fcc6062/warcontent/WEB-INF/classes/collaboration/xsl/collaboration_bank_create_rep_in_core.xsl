...
-->
	...
	<!--<script type="text/javascript" src="/content/OLD/javascript/openaccount/trade_create_in.js"></script>
	...
	<td align="left">
		...
		<table border="0" width="570" cellpadding="0" cellspacing="0">
			...
			<tr>
				<td width="40">&nbsp;</td>
				<td width="150"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_FOLDER_REF_ID')"/></td>
				<td>
					<font class="REPORTDATA"><xsl:value-of select="link_ref_id"/></font>
					<input type="hidden" name="link_ref_id"><xsl:attribute name="value"><xsl:value-of select="link_ref_id"/></xsl:attribute></input>
				</td>
			</tr>
			</xsl:if>
			<xsl:if test="bo_ref_id[.!='']">
				<tr>
					<td width="15">&nbsp;</td>
					<td width="190"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_BO_REF_ID')"/></td>
					<td>
						<font class="REPORTDATA"><xsl:value-of select="bo_ref_id"/></font>
						<input type="hidden" name="bo_ref_id"><xsl:attribute name="value"><xsl:value-of select="bo_ref_id"/></xsl:attribute></input>
					</td>
				</tr>
			</xsl:if>
			...
			<tr>
				...
				<xsl:when test="tnx_type_code[. = '03']">
					...
					<tr>
						<td width="40">&nbsp;</td>
						<td width="150"><xsl:value-of select="localization:getGTPString($language, 'XSL_GENERALDETAILS_AMD_DATE')"/></td>
						<td><font class="REPORTDATA"><xsl:value-of select="amd_date"/></font></td>
					</tr>
				</xsl:when>
			</xsl:choose>
		</table>
		...
		<table border="0" cellspacing="0" width="570">
			...
			<!--tr>
				...
				<td>
					<input type="text" size="11" maxlength="11" name="seller_bank_bic" onblur="fncRestoreInputStyle('fakeform1','seller_bank_bic');">
		...
		<xsl:if test="$section_in_line_items!='N'">
      ...
      <br/>
		...
		<!--Inco Terms Details-->	
		<xsl:if test="$section_in_inco_terms!='N'">
		...
		<xsl:if test="$section_in_routing_summary!='N'">
			
		<xsl:if test="count(/in_tnx_record/routing_summaries/routing_summary/transport_type) != 0">
		...
		<table border="0" width="570" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td>
					<font class="REPORTDATA">
						<xsl:choose>
							<xsl:when test="count(/in_tnx_record/routing_summaries/routing_summary/transport_type[. = '01']) != 0">
								<xsl:value-of select="localization:getDecode($language, 'N213', '01')"/>
							</xsl:when>
							<xsl:when test="count(/in_tnx_record/routing_summaries/routing_summary/transport_type[. = '02']) != 0">
		...
		<div id="routing_summaries_individual">
			<xsl:attribute name="style">
				<xsl:if test="count(/in_tnx_record/routing_summaries/routing_summary/transport_type[. = '01']) = 0">display:none</xsl:if>
			</xsl:attribute>
			...
			<table border="0" width="100%" cellspacing="0">
				...
				<tr>
					<td>
						...
						<!--<xsl:call-template name="routing_summary_transport_mode_air_rep">
							<xsl:with-param name="structure_name">po_routing_summary_individual_air</xsl:with-param>
							<xsl:with-param name="form_name">form_routing_summaries_individual</xsl:with-param>
						</xsl:call-template>-->
					</td>
		...
		<div id="routing_summaries_multimodal">
			<xsl:attribute name="style">
				<xsl:if test="count(/in_tnx_record/routing_summaries/routing_summary/transport_type[. = '02']) = 0">display:none</xsl:if>
			</xsl:attribute>
			...
			<tr>
				...
				<td>
					<form name="realform" 
						...
						method="POST" 
						action="/gtp/screen/InvoiceScreen" 
