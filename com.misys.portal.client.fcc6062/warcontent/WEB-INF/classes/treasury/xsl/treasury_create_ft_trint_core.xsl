  ...
  <xsl:output method="html" version="4.01" indent="no" encoding="UTF-8" omit-xml-declaration="yes" />
				...
				<xsl:with-param name="content">
					...
					<!-- <xsl:choose>
				  		<xsl:when test="$displaymode='view'">
					     	<xsl:call-template name="ft-review-fields"/>					
							Customer Payment Details and Bank Detail (Settlement Detail In View Mode) 
							<xsl:call-template name="customer_and_bank_instruction_view">
								<xsl:with-param name="prefix">review</xsl:with-param>
							</xsl:call-template>
				      	</xsl:when>
	       				Condition $option != 'FULL' added by Ravi Ande, Please discuss if any concerns with this condition
	       				 <xsl:when test="($mode = 'DRAFT' and (ft_type[.='01'] or ft_type[.='02'])) and $option != 'FULL'">
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
   				<xsl:with-param name="binding">misys.binding.treasury.create_ft_trint</xsl:with-param>
   				<xsl:with-param name="override-help-access-key">TRINT_01</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
