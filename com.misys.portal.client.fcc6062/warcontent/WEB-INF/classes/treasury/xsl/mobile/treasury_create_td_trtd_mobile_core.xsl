...
<xsl:stylesheet version="1.0" 
	...
	<xsl:include href="./common/treasury_common_mobile.xsl"/>
	...
	<xsl:template match="td_tnx_record">
		...
		<sections>
			<label>
				<xsl:value-of select="localization:getGTPString($language, 'XSL_HEADER_REQUEST_FOR_DEPOSIT_DETAILS')" />
			</label>
			<xsl:call-template name="td-trtd-details"/>	
		</sections>
