...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="ea_tnx_record/@*|ea_tnx_record/node()">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="line_items/lt_tnx_record | attachments/attachment | contacts/contact | payments/payment | incoterms/incoterm | adjustments/adjustment | taxes/tax | freight_charges/freight_charge | user_defined_informations/user_defined_information | routing_summaries/rs_tnx_record | buyer_bank/node() | issuing_bank/node() | seller_bank/node() | advising_bank/node()">
	       <xsl:copy-of select="."/>
    </xsl:template>
    ...
    <xsl:template match="mismatches/Rpt">
    	<xsl:variable name="cdata">![CDATA[</xsl:variable>
		<xsl:variable name="end">]]</xsl:variable>
		<xsl:text disable-output-escaping="yes">&lt;</xsl:text><xsl:value-of select="$cdata" /><xsl:copy-of select="."/><xsl:value-of select="$end" /><xsl:text disable-output-escaping="yes">&gt;</xsl:text>
    </xsl:template>
