	...
	<xsl:template match="ic_tnx_record">
		<ic_tnx_record>
			...
			<xsl:if test="ic_cur_code">
				<xsl:apply-templates select="ic_cur_code" mode="copy_element" />
			</xsl:if>
			<xsl:if test="ic_amt">
				<xsl:apply-templates select="ic_amt" mode="copy_element" />
			</xsl:if>
			<xsl:if test="ic_liab_amt">
				<xsl:apply-templates select="ic_liab_amt" mode="copy_element" />
			</xsl:if>
			<xsl:if test="ic_outstanding_amt">
				<xsl:apply-templates select="ic_outstanding_amt"
					mode="copy_element" />
			</xsl:if>
			<xsl:if test="ic_type_code">
				<xsl:apply-templates select="ic_type_code" mode="copy_element" />
			</xsl:if>
