	...
	<xsl:template match="si_tnx_record">
		<si_tnx_record>
			...
			<xsl:if test="product_type_code">
				<xsl:apply-templates select="product_type_code"
					mode="copy_element" />
			</xsl:if>
			<xsl:if test="product_type_details">
				<xsl:apply-templates select="product_type_details"
					mode="copy_element" />
			</xsl:if>
			...
			<xsl:if test="standby_rule_code">
				<xsl:apply-templates select="standby_rule_code"
					mode="copy_element" />
			</xsl:if>
			<xsl:if test="standby_rule_other">
				<xsl:apply-templates select="standby_rule_other"
					mode="copy_element" />
			</xsl:if>
			...
			<xsl:choose>
				...
				<xsl:when test="prod_stat_code[.='78' or .='79' or .='84'] and action_req_code">
					<action_req_code>
