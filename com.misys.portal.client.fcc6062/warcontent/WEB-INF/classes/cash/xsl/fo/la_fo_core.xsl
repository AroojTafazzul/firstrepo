...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="la_content_fo.xsl" />
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="{$pdfFont}" writing-mode="{$writingMode}">
		    ...
		    <fo:bookmark-tree>
		        ...
		        <fo:bookmark internal-destination="requestQuote">
		            <fo:bookmark-title><xsl:value-of
								select="localization:getGTPString($language, 'XSL_HEADER_REQUEST_FOR_QUOTE_DETAILS')" />
					</fo:bookmark-title>
		        </fo:bookmark>
