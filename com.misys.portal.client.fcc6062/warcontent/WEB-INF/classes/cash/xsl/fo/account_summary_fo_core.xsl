...
<xsl:stylesheet version="1.0"
	...
	<xsl:include href="account_summary_content_fo.xsl" />
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="{$pdfFont}" writing-mode="{$writingMode}">
			...
			<!-- bookmark section -->
		    <fo:bookmark-tree>
		        <fo:bookmark internal-destination="accountSummary">
		            <fo:bookmark-title><xsl:value-of
								select="localization:getGTPString($language, 'XSL_HEADER_ACCOUNT_SUMMARY')" />
					</fo:bookmark-title>
		        </fo:bookmark>
		    </fo:bookmark-tree>
			...
			<fo:page-sequence initial-page-number="1"
				...
				<fo:static-content text-align="center" flow-name="xsl-region-start">
					<fo:block font-size="8pt" text-align="center" color="#FFFFFF"
						...
						<xsl:value-of select="localization:getGTPString($language, 'XSL_HEADER_ACCOUNT_SUMMARY')" />
					</fo:block>
				</fo:static-content>
