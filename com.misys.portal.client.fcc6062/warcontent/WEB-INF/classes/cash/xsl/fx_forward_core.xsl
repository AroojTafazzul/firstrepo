...
<xsl:stylesheet 
  ...
  <xsl:template name="fx-forward-details">
   <xsl:choose>
	...
	<div id="request-details" class="widgetContainer">
		...
		<xsl:call-template name="fieldset-wrapper">
		    ...
		    <xsl:with-param name="content"> 
			    ...
			    <xsl:choose>
					<xsl:when
						test="securityCheck:hasPermission($rundata,'fx_fwd_purchase') and securityCheck:hasPermission($rundata,'fx_fwd_sale')">
						<xsl:call-template name="fx-contract-type-option" />
					</xsl:when>
					<xsl:when
						test="not(securityCheck:hasPermission($rundata,'fx_fwd_purchase')) and securityCheck:hasPermission($rundata,'fx_fwd_sale')">
						<xsl:call-template name="fx-contract-type-sale" />
					</xsl:when>
					<xsl:when
						test="securityCheck:hasPermission($rundata,'fx_fwd_purchase') and not(securityCheck:hasPermission($rundata,'fx_fwd_sale'))">
				...
				<xsl:with-param name="term-options">
					<xsl:call-template name="code-data-options">
					 <xsl:with-param name="paramId">C036</xsl:with-param>
