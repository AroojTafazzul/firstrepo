...
<xsl:stylesheet 
  ...
  <xsl:include href="../../cash/xsl/cash_create_remittance_details.xsl" />
 ...
 <xsl:template name="js-imports">
  ...
  <xsl:call-template name="common-js-imports">
   <xsl:with-param name="binding">misys.binding.cash.create_remittance</xsl:with-param>
   <xsl:with-param name="show-clearing-code-js">Y</xsl:with-param>
   <xsl:with-param name="override-action">
	<xsl:choose>
		<xsl:when test="contains($subProductCode, 'FI')"><xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/FundTransferScreen?option=REMITTANCE_FI</xsl:when>
		<xsl:otherwise><xsl:value-of select="$contextPath"/><xsl:value-of select="$servletPath"/>/screen/FundTransferScreen?option=REMITTANCE</xsl:otherwise>
	</xsl:choose>
	</xsl:with-param>
