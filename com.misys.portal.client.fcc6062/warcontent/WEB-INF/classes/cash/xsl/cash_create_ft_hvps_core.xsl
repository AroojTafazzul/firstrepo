...
<xsl:stylesheet version="1.0" 
					...
					<xsl:if test="$displaymode='edit'">
						<div id="DOM_DISCLAIMER" class="ftDisclaimer domDisclaimer">
							<xsl:call-template name="simple-disclaimer">
								<xsl:with-param name="label">XSL_FT_HVPS_DISCLAIMER</xsl:with-param>
							</xsl:call-template>
						</div>
					</xsl:if>
					<div>
						...
						<!-- Transfer sections -->						
						<xsl:call-template name="hvps-general-details"/>
						...
						</xsl:call-template><xsl:call-template name="hvps-transfer-to-details" />
						...
						<!-- FX Snippets Start -->
							...
							<!-- FX Snippets End -->  
						<xsl:call-template name="hvps-notify-beneficiary-details" />
	...
	<xsl:template name="js-imports">
		...
		<xsl:with-param name="binding">misys.binding.cash.create_ft_hvps</xsl:with-param>
			<xsl:with-param name="override-help-access-key">
			     <xsl:choose>
			       <xsl:when test="$option ='CANCEL'">FT_CANCEL</xsl:when>
			       <xsl:otherwise>FT_CNAPSD</xsl:otherwise>
			     </xsl:choose>
 		  </xsl:with-param> 
		</xsl:call-template>
		<script>
					...
					<div>
						...
						<div id="pre_approved_row" class="field">
							...
							<div id="PAB" class="content">
								...
								<xsl:value-of select="localization:getGTPString($language,'XSL_HVPS_PAB')"/>
							</div> 
						</div>
						<xsl:choose>
		...
		<xsl:call-template name="fieldset-wrapper">
			...
			<!-- some hidden fields -->
						...
						<xsl:call-template name="hidden-field">
							...
							<xsl:with-param name="value">HVPS</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="hidden-field">
	...
	<xsl:template name="hvps-general-details">
				...
				<xsl:choose>
					<xsl:when test="bulk_ref_id[.!='']">
						...
						<xsl:call-template name="user-account-field">
				          ...
				          <xsl:with-param name="product_types">FT:HVPS</xsl:with-param>
					...
					<xsl:otherwise>
					<xsl:if test="$displaymode='view' and entity[.!=''] and sub_product_code[.!='HVPS']">                                     
 	 	 			 	<div id="display_entity_row" class="field">
 	 	 	 			<span class="label"><xsl:value-of select="localization:getGTPString($language, 'ENTITY_LABEL')"/></span>
 	 	 	 			<div class="content"><xsl:value-of select="entity"/></div> 
 	 	 	 			</div>                                                  
							...
							<xsl:call-template name="input-field">
								...
								<xsl:with-param name="value"><xsl:value-of select="localization:getDecode($language, 'N047', 'HVPS')"/></xsl:with-param>
								<xsl:with-param name="override-displaymode">view</xsl:with-param>
							</xsl:call-template>
							<xsl:if test="entities[number(.) &gt; 0]">
								<xsl:call-template name="entity-field">
								    ...
								    <xsl:with-param name="override-sub-product-code">FT:HVPS</xsl:with-param>
							    </xsl:call-template>
							</xsl:if>
	...
	<xsl:template name="hvps-notify-beneficiary-details">
