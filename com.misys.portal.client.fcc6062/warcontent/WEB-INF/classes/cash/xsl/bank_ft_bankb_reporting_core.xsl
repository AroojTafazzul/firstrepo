...
<!--
##########################################################
Third Party Fund Transfer (FT-BANKB) Form.
  ...
  <xsl:include href="../../collaboration/xsl/collaboration.xsl" />
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.bank.report_ft_bankb</xsl:with-param>
			 ...
			 <xsl:call-template name="column-container">
			  <xsl:with-param name="content">
				...
				<div class="field">
    				...
    				<xsl:if test="$displaymode = 'edit' or ($displaymode ='view' and sub_product_code[.='BANKB'])">
    					<div>
    						<xsl:attribute name="class">CUR <xsl:if test="$displaymode !='edit'"> inlineBlock</xsl:if></xsl:attribute>
    						<xsl:choose>
    							<xsl:when test="$displaymode = 'edit'">
    							<div style="display:none;">
    								<div name="beneficiary_act_cur_code" id="beneficiary_act_cur_code" trim="true" hidden="true" uppercase="true" dojoType="dijit.form.ValidationTextBox" regExp="^[a-zA-Z]*$" class="xx-small inlineBlock inlineBlockNoLabel" maxLength="3">										
    						...
    						<!-- Hidden fields to store information related to Beneficary Account Validation when product type is BANKB -->
				...
				<!-- This Empty div is to show inline for BANKB product , used in JS -->				
				<div class="CUR"/>
