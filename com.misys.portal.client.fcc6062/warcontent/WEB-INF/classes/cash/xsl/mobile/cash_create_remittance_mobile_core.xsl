...
<xsl:stylesheet version="1.0" 
	...
	<xsl:include href="./common/cash_common_mobile.xsl"/>
	...
	<xsl:template match="ft_tnx_record">
		...
		<sections>
			...
			<xsl:call-template name="swift-details"/>
		</sections>
		...
		<sections>
			...
			<xsl:call-template name="transfer-to-details"/>
		</sections>
		<sections>
			<label>
				<xsl:value-of select="localization:getGTPString($language, 'XSL_HEADER_TRANSACTION_DETAILS')" />
			</label>
			<xsl:call-template name="ft-tnx-details"/>	
			<xsl:call-template name="mobile-field">
				<xsl:with-param name="label" select="localization:getGTPString($language, 'XSL_PROCESSING_DATE')"></xsl:with-param>
