...
<xsl:stylesheet 
  ...
  <xsl:include href="./common/lep_common.xsl" />
...
<!--		 	<xsl:template name="general-details-bank">-->
            ...
            <!-- Fields specific to LEP -->    		      
	      	<xsl:call-template name="recurring_content" />     	   
	      	<xsl:call-template name="lep-transfer-to-details"/>
	      	<xsl:call-template name="lep-transaction-details"/>	
...
<!--   <xsl:with-param name="binding">misys.binding.cash.BankReportingFtBinding</xsl:with-param>-->
	<xsl:with-param name="binding">misys.binding.bank.report_lep</xsl:with-param>
