				...
				<xsl:with-param name="content">
					...
					<div>
						...
						<xsl:if test="$displaymode='edit'">
							<div id="MEPS_DISCLAIMER" class="mepsDisclaimer" style="display:none;">
								<xsl:call-template name="simple-disclaimer">
									<xsl:with-param name="label">XSL_MEPS_DISCLAIMER</xsl:with-param>
								</xsl:call-template>	      				
							</div>
						</xsl:if>
						<!--  Display common menu. -->
...
<!-- 						<xsl:variable name="displayInterbankDetailsflag" select="defaultresource:getResource('MEPS_INTER_BANK_DETAILS_DISPLAY')"></xsl:variable>
...
<xsl:template name="notify-beneficiary-details">
 ...
 <xsl:template name="js-imports">
   ...
   <xsl:call-template name="common-js-imports">
	   <xsl:with-param name="binding">misys.binding.cash.create_ft_meps</xsl:with-param>
	   ...
	   <xsl:choose>
			...
			<xsl:otherwise>FT_SGD</xsl:otherwise>
			</xsl:choose>
				 ...
				 <xsl:with-param name="content">			 
				  <xsl:call-template name="column-wrapper">
				   <xsl:with-param name="content">	
						...
						<xsl:call-template name="input-field">
						   ...
						   <xsl:call-template name="input-field">
							...
							<xsl:call-template name="country-field">
				    			<xsl:with-param name="label">XSL_BENEFICIARY_COUNTRY_MEPS</xsl:with-param>
	                    ...
	                    <xsl:call-template name="input-field">
					  	...
					  	<div id="pre_approved_row" class="field">
							...
							<div id="PAB" class="content">
								...
								<xsl:value-of select="localization:getGTPString($language,'XSL_MEPS_PAB')"/>
							</div> 
						</div>
					  	
					<xsl:call-template name="hidden-field">
						<xsl:with-param name="name">beneNameRegex</xsl:with-param>
						<xsl:with-param name="value"><xsl:value-of select="defaultresource:getResource('BENEFICIARY_VALIDATION_REGEX_MEPS')"/></xsl:with-param>
					</xsl:call-template>
					<xsl:call-template name="hidden-field">
					...
					<xsl:call-template name="hidden-field">
						<xsl:with-param name="name">customerReference</xsl:with-param>
						<xsl:with-param name="value"><xsl:value-of select="defaultresource:getResource('CUSTOMER_REFERENCE_CASH_VALIDATION_REGEX_MEPS')"/></xsl:with-param>
					</xsl:call-template>
					<xsl:call-template name="hidden-field">
				   ...
				   <xsl:with-param name="content">	
					<xsl:call-template name="input-field">
			      	...
			      	<xsl:if test="$show-branch-address='Y'">
	            		...
	            		<xsl:call-template name="hidden-field">
						<xsl:with-param name="name">cpty_bank_swift_bic_code</xsl:with-param>
						<xsl:with-param name="value" select="counterparties/counterparty/cpty_bank_swift_bic_code"/>
				</xsl:call-template>
	    			</xsl:with-param>
	    		  </xsl:call-template>
	    		 </xsl:with-param>
				  ...
				  <xsl:call-template name="column-wrapper">
				   <xsl:with-param name="content">	
					<xsl:call-template name="input-field">
			      	...
			      	<xsl:call-template name="hidden-field">
						<xsl:with-param name="name">intermediary_bank_swift_bic_code</xsl:with-param>
						<xsl:with-param name="value" select="intermediary_bank_swift_bic_code"/>
				</xsl:call-template>
			  </xsl:with-param>
			  </xsl:call-template>
			</xsl:with-param>
					      		...
					      		<xsl:choose>
									<xsl:when test="$displaymode='edit'">		       		
										<xsl:call-template name="code-data-options">
		                              		...
		                              		<xsl:with-param name="subProductCode">MEPS</xsl:with-param>
	                              	...
	                              	<xsl:otherwise>
								    	...
								    	<xsl:choose>
								     		 <xsl:when test="charge_option[. = 'SHA']"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_SHA_MEPS')"/></xsl:when>
										     <xsl:when test="charge_option[. = 'OUR']"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_OUR_MEPS')"/></xsl:when>								     
										     <xsl:when test="charge_option[. = 'BEN']"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_BEN_MEPS')"/></xsl:when>
			    	...
			    	<xsl:call-template name="input-field">
			    		...
			    		<xsl:with-param name="name">cust_ref_id</xsl:with-param>
		   				...
		   				<!-- <xsl:with-param name="regular-expression">
		   							<xsl:value-of select="defaultresource:getResource('CUSTOMER_REFERENCE_CASH_VALIDATION_REGEX_MEPS')"/>
		   				</xsl:with-param> -->
		   				<xsl:with-param name="fieldsize">small</xsl:with-param>
		   				<xsl:with-param name="override-displaymode">
							<xsl:choose>
