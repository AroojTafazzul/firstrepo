...
<!--
##########################################################
HVPS Cross Border Fund Transfer (FT-HVXB) Form. 
...
<xsl:stylesheet version="1.0" 
		...
		<div>
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="content">
					...
					<xsl:if test="$displaymode='edit'">
						<div id="DOM_DISCLAIMER" class="ftDisclaimer domDisclaimer">
							<xsl:call-template name="simple-disclaimer">
								<xsl:with-param name="label">XSL_FT_HVXB_DISCLAIMER</xsl:with-param>
							</xsl:call-template>
						</div>
					</xsl:if>
					<div>
						...
						<!-- Transfer sections -->
						<xsl:call-template name="hvxb-general-details"/>
					...
					<xsl:call-template name="hvxb-transfer-to-details" />
						<xsl:if test="not(amount_access) or  (amount_access[.='true'])">
							<xsl:call-template name="hvxb-transaction-details" />
						</xsl:if>
						<!-- FX Snippets Start -->
							<xsl:if test="not(amount_access) or  (amount_access[.='true'])">
							...
							<!-- FX Snippets End -->  
						<xsl:call-template name="hvxb-notify-beneficiary-details" />
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.cash.create_ft_hvxb</xsl:with-param>
			<xsl:with-param name="override-help-access-key">
			     <xsl:choose>
			       <xsl:when test="$option ='CANCEL'">FT_CANCEL</xsl:when>
			       <xsl:otherwise>FT_CNAPSC</xsl:otherwise>
			     </xsl:choose>
 		  </xsl:with-param> 
		</xsl:call-template>
		<script>
	...
	<xsl:template name="hvxb-transfer-to-details">
				   ...
				   <xsl:call-template name="column-wrapper">
					...
					<div>
						<xsl:call-template name="input-field">
							...
							<xsl:with-param name="maxsize">60</xsl:with-param>
							<xsl:with-param name="fieldsize">beneficiary-name-field-size</xsl:with-param>
						...
						<xsl:call-template name="row-wrapper">
							...
							<xsl:with-param name="content">
								<xsl:if test="$displaymode = 'edit' or ($displaymode ='view' and sub_product_code[.='HVXB'])">
			    					<div>
			    						...
			    						<!-- Hidden fields to store information related to Beneficary Account Validation when product type is HVXB -->
			    						<xsl:call-template name="hidden-field">
						...
						<!-- This Empty div is to show inline for HVXB product , used in JS -->				
						<div class="CUR"/>
						<div id="pre_approved_row" class="field">
							...
							<div id="PAB" class="content">
								...
								<xsl:value-of select="localization:getGTPString($language,'XSL_HVXB_PAB')"/>
							</div> 
						</div>
						<xsl:choose>
						...
						<xsl:with-param name="content">
					    	<xsl:call-template name="input-field">
			                     ...
			                     <xsl:with-param name="required">Y</xsl:with-param>		
			                     <xsl:with-param name="value"><xsl:if test="sub_product_code = 'HVXB'"><xsl:value-of select="cnaps_bank_code"/></xsl:if></xsl:with-param>	
			                     <xsl:with-param name="readonly"><xsl:if test="$pre-approved_beneficiary_only or pre_approved_status ='Y'">Y</xsl:if></xsl:with-param>	     			  
							 </xsl:call-template>	
			                 ...
			                 <xsl:when test="bulk_ref_id[.!='']">
							     <xsl:if test="not($pre-approved_beneficiary_only) and $displaymode='edit'">	
									<xsl:call-template name="button-wrapper">
										<xsl:with-param name="show-image">Y</xsl:with-param>
										<xsl:with-param name="show-border">N</xsl:with-param>
										<xsl:with-param name="onclick">misys.showSearchDialog('cnaps_crossborder_bank_code', "['cnaps_bank_code','cnaps_bank_name']", '', 'AjaxScreen/action/GetStaticDataPopup', '', 'width:710px;height:350px;', '<xsl:value-of select="localization:getGTPString($language, 'LIST_TITLE_SDATA_LIST_OF_BANKS')"/>','', 'GetStaticData', false, 'GetStaticData');return false;</xsl:with-param>
	...
	<xsl:template name="hvxb-transaction-details">	 	
		<xsl:call-template name="fieldset-wrapper">
			...
			<xsl:with-param name="content">		
				...
				<xsl:with-param name="content">
					<xsl:call-template name="column-wrapper">
					<xsl:with-param name="content">
					<!-- Fix for MPG-5806, remove if amount_access -->
					<!-- xsl:if test="not(amount_access) or  (amount_access[.='true'])  "-->
						...
						<div class="urgent-transfer-details">	
						<xsl:choose>
							<xsl:when  test="$displaymode='edit'">
								<xsl:call-template name="checkbox-field">
									<xsl:with-param name="label">XSL_GENERALDETAILS_URGENT_TRANSFER</xsl:with-param>
									<xsl:with-param name="name">urgent_transfer</xsl:with-param>
									<xsl:with-param name="id">urgent_transfer</xsl:with-param>
									<xsl:with-param name="value">01</xsl:with-param>
									<xsl:with-param name="checked"><xsl:if test="urgent_transfer[.='Y']">Y</xsl:if></xsl:with-param>
								</xsl:call-template>							
							</xsl:when>
							<xsl:otherwise>
						...
						<xsl:call-template name="input-field">
							<xsl:with-param name="label">XSL_GENERALDETAILS_RELATED_TNX_DATE</xsl:with-param>
							<xsl:with-param name="name">related_transaction_date</xsl:with-param>
							<xsl:with-param name="id">related_transaction_date</xsl:with-param>
							<xsl:with-param name="type">date</xsl:with-param>
							<xsl:with-param name="fieldsize">small</xsl:with-param>
							<xsl:with-param name="value"><xsl:value-of select="related_transaction_date"/></xsl:with-param>
						</xsl:call-template>
					  	...
					  	<xsl:call-template name="select-field">
					      	...
					      	<xsl:with-param name="options">			       		
								<xsl:choose>
									<xsl:when test="$displaymode='edit'">
									    <option value="OUR"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_OUR')"/></option>
									    <option value="BEN"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_BEN')"/></option>
									    <option value="SHA"><xsl:value-of select="localization:getGTPString($language, 'XSL_CHARGE_OPTION_SHA')"/></option>
								   </xsl:when>
								   <xsl:otherwise>
								    	<xsl:choose>
				...
				<xsl:choose>
			    		...
			    		<xsl:call-template name="textarea-field">
			      			<xsl:with-param name="label">XSL_GENERALDETAILS_CROSS_BORDER_REMARK</xsl:with-param>
			      			<xsl:with-param name="name">cross_border_remark</xsl:with-param>
			      			<xsl:with-param name="messageValue"><xsl:value-of select="cross_border_remark"/></xsl:with-param>
			    		 ...
			    		 <xsl:when test="$displaymode='view' and cross_border_remark[.!='']">
				...
				<xsl:choose>
					<xsl:when test="$displaymode='edit'">
						<xsl:call-template name="select-field">
						      ...
						      <xsl:with-param name="required">Y</xsl:with-param>
						      <xsl:with-param name="value"><xsl:value-of select="business_type"/></xsl:with-param>
						      <xsl:with-param name="options">
						      		<xsl:call-template name="business_type_options"/>
						      </xsl:with-param>
						 </xsl:call-template>
						<xsl:call-template name="select-field">
						      ...
						      <xsl:with-param name="required">Y</xsl:with-param>
						      <xsl:with-param name="value"><xsl:value-of select="business_detail_type"/></xsl:with-param>
						      <xsl:with-param name="options">
						      		<xsl:call-template name="business_detail_type_options"/>
						      </xsl:with-param>
						 </xsl:call-template>				
					</xsl:when>
					<xsl:otherwise>
					
						<xsl:variable name="businessTypeDescription">CNAPS_BUSINESS_TYPE_HVXB_<xsl:value-of select="business_type"/></xsl:variable>
						<xsl:variable name="businessDetailTypeDescription">CNAPS_BUSINESS_DETAIL_TYPE_HVXB_<xsl:value-of select="business_detail_type"/></xsl:variable>
						<xsl:call-template name="input-field">
							  <xsl:with-param name="label">XSL_GENERALDETAILS_BUSINESS_TYPE</xsl:with-param>
						      <xsl:with-param name="name">business_type</xsl:with-param>
						...
						<xsl:call-template name="hidden-field">
							<xsl:with-param name="name">iss_date_unsigned</xsl:with-param>
					     	<xsl:with-param name="value" select="iss_date_unsigned"></xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="hidden-field">
						...
						<xsl:call-template name="hidden-field">
							...
							<xsl:with-param name="value"><xsl:value-of select="defaultresource:getResource('BENEFICIARY_VALIDATION_REGEX_CNAPS')"/></xsl:with-param>
						</xsl:call-template>						
						<xsl:call-template name="hidden-field">
						...
						<xsl:call-template name="hidden-field">
							...
							<xsl:with-param name="value">HVXB</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="hidden-field">
	...
	<xsl:template name="hvxb-general-details">
				...
				<xsl:choose>
					<xsl:when test="bulk_ref_id[.!='']">
						...
						<xsl:call-template name="user-account-field">
				          ...
				          <xsl:with-param name="product_types">FT:HVXB</xsl:with-param>
					...
					<xsl:otherwise>
					<xsl:if test="$displaymode='view' and entity[.!=''] and sub_product_code[.!='HVXB']">                                     
 	 	 			 	<div id="display_entity_row" class="field">
 	 	 	 			<span class="label"><xsl:value-of select="localization:getGTPString($language, 'ENTITY_LABEL')"/></span>
 	 	 	 			<div class="content"><xsl:value-of select="entity"/></div> 
 	 	 	 			</div>                                                  
							...
							<xsl:call-template name="input-field">
								...
								<xsl:with-param name="value"><xsl:value-of select="localization:getDecode($language, 'N047', 'HVXB')"/></xsl:with-param>
								<xsl:with-param name="override-displaymode">view</xsl:with-param>
							</xsl:call-template>
							<xsl:if test="entities[number(.) &gt; 0]">
								<xsl:call-template name="entity-field">
								    ...
								    <xsl:with-param name="override-sub-product-code">FT:HVXB</xsl:with-param>
							    </xsl:call-template>
							</xsl:if>
	...
	<xsl:template name="hvxb-notify-beneficiary-details">
					...
					<xsl:call-template name="fieldset-wrapper">
						...
						<xsl:with-param name="content">
						  	<div id="notify_beneficiary_row" class="field">
								...
								<div class="content" style="display: inline-block; word-break: break-all; word-wrap: normal;">
									<xsl:value-of select="notify_beneficiary_email"/>
								</div>
							</div>
						</xsl:with-param>
	...
	<xsl:template name="business_type_options">

     <xsl:for-each select="business_types/business_type">
        <option>
           <xsl:attribute name="value"><xsl:value-of select="code"/></xsl:attribute>
		<xsl:if test="count(//business_types/business_type) = 1">
			           <xsl:attribute name="selected">selected</xsl:attribute>
		</xsl:if>
			
           <xsl:value-of select="code"/> - <xsl:value-of select="localization:getGTPString($language, description)"/>
        </option>
      </xsl:for-each>
    </xsl:template>

	<xsl:template name="business_detail_type_options">

     <xsl:for-each select="business_detail_types/business_detail_type">
        <option>
		<xsl:if test="count(//business_detail_types/business_detail_type) = 1">
			           <xsl:attribute name="selected">selected</xsl:attribute>
		</xsl:if>
           <xsl:attribute name="value"><xsl:value-of select="code"/></xsl:attribute>
           <xsl:value-of select="code"/> - <xsl:value-of select="localization:getGTPString($language, description)"/>
