...
Templates for
 ...
 Request for Loan Standing Instruction (LA) Form, Customer Side.
...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="general-details">
		<xsl:call-template name="fieldset-wrapper">
	     ...
	     <xsl:if test="trade_id[.!='']">
	      <xsl:call-template name="input-field">
	       <xsl:with-param name="label">XSL_LA_TRADE_ID_LABEL</xsl:with-param>
	       ...
	       <xsl:with-param name="override-displaymode">view</xsl:with-param>
		      ...
		      <xsl:with-param name="label">XSL_LA_AMOUNT_LABEL</xsl:with-param>
		      ...
		      <xsl:with-param name="override-amt-name">la_amt</xsl:with-param>
	    	 ...
	    	 <!-- interest label -->
		  	  <xsl:call-template name="currency-field">
			      <xsl:with-param name="label">XSL_LA_INTEREST_LABEL</xsl:with-param>
			      <xsl:with-param name="product-code">la</xsl:with-param>
			      <xsl:with-param name="override-currency-displaymode">view</xsl:with-param>
			      <xsl:with-param name="override-amt-displaymode">view</xsl:with-param>
			      <xsl:with-param name="override-amt-value"><xsl:value-of select="interest"/></xsl:with-param>
			      <xsl:with-param name="override-amt-name">interest_la</xsl:with-param>
		 	  </xsl:call-template>
		 	  
		   <!-- new total amount with interest -->
		   	 <xsl:call-template name="currency-field">
		      <xsl:with-param name="label">XSL_LA_TOTAL_WITH_INTEREST_LABEL</xsl:with-param>
		      <xsl:with-param name="product-code">la</xsl:with-param>
		      <xsl:with-param name="override-currency-displaymode">view</xsl:with-param>
		      <xsl:with-param name="override-amt-displaymode">view</xsl:with-param>
	...
	<xsl:template name="standing-instructions">
		...
		<xsl:call-template name="si-bank-instructions">
			...
			<xsl:with-param name="amt">la_amt</xsl:with-param>
			<xsl:with-param name="cur_code">la_cur_code</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="la-si-hidden-fields"/>
	</xsl:template>
		...
		<script>
			dojo.ready(function(){
				...
				dojo.mixin(misys._config,  {
					la_si : <xsl:value-of select="$la_si"/>
				});
			});
		</script>
	</xsl:template>

	<xsl:template name="la-si-hidden-fields">
	  <div class="widgetContainer">
    	...
    	<xsl:call-template name="hidden-field">
	     	...
	     	<xsl:with-param name="value"><xsl:value-of select="org_previous_file/la_tnx_record/sub_tnx_type_code"/></xsl:with-param>
    	</xsl:call-template>
	 </div>
	</xsl:template>
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.cash.TradeMessageSiLaBinding</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
