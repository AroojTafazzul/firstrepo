...
<!--
##########################################################
Third Party Fund Transfer (FT-TPT) Form.
...
<xsl:stylesheet version="1.0" 
			...
			<xsl:call-template name="form-wrapper">
				...
				<xsl:with-param name="content">
					...
					<xsl:if test="$displaymode='edit'">
						<div id="TPT_DISCLAIMER" class="ftDisclaimer tptDisclaimer">
							<xsl:call-template name="simple-disclaimer">
								<xsl:with-param name="label">XSL_FT_TPT_DISCLAIMER</xsl:with-param>
							</xsl:call-template>
						</div>
					</xsl:if>
					<div>
						...
						<!-- Transfer sections -->						
						<xsl:call-template name="tpt-general-details"/>
						...
						<xsl:call-template name="tpt-transfer-to-details" />
						<xsl:call-template name="tpt-transaction-details" />
						...
						<xsl:call-template name="tpt-notify-beneficiary-details" />
	...
	<xsl:template name="js-imports">
		<xsl:call-template name="common-js-imports">
			<xsl:with-param name="binding">misys.binding.cash.create_ft_tpt</xsl:with-param>
	...
	<xsl:template name="tpt-transfer-to-details">
			 ...
			 <xsl:call-template name="column-container">
			  <xsl:with-param name="content">
				...
				<xsl:call-template name="row-wrapper">
					...
					<xsl:with-param name="content">
						<xsl:if test="$displaymode = 'edit' or ($displaymode ='view' and sub_product_code[.='TPT'])">
	    					<div>
	    						...
	    						<!-- Hidden fields to store information related to Beneficary Account Validation when product type is TPT -->
	    						<xsl:call-template name="hidden-field">
	    				...
	    				<xsl:call-template name="input-field">
	    				    ...
	    				    <xsl:with-param name="maxlength">13</xsl:with-param>
	    				    <xsl:with-param name="required">Y</xsl:with-param>	
				...
				<!-- This Empty div is to show inline for TPT product , used in JS -->				
				<div class="CUR"/>
	...
	<xsl:template name="tpt-transaction-details">	 	
		<xsl:call-template name="fieldset-wrapper">
			...
			<xsl:with-param name="content">
			  <xsl:call-template name="column-container">
				...
				<xsl:call-template name="hidden-field">
					...
					<xsl:with-param name="value">TPT</xsl:with-param>
				</xsl:call-template>
				<xsl:call-template name="hidden-field">
	...
	<xsl:template name="tpt-general-details">
			...
			<xsl:with-param name="content">
			  ...
			  <xsl:call-template name="column-wrapper">
				...
				<xsl:choose>
					<xsl:when test="bulk_ref_id[.!='']">
						...
						<!-- END- Multibank fields -->
						 <xsl:call-template name="user-account-field">
				          ...
				          <xsl:with-param name="product_types">FT:TPT</xsl:with-param>
					...
					<xsl:otherwise>
					<xsl:if test="$displaymode='view' and entity[.!=''] and sub_product_code[.!='TPT']">                                     
 	 			 	 	<div id="display_entity_row" class="field">
 	 	 			 	<span class="label"><xsl:value-of select="localization:getGTPString($language, 'ENTITY_LABEL')"/></span>
 	 	 			 	<div class="content"><xsl:value-of select="entity"/></div> 
 	 	 			 	</div>                                                  
						...
						<xsl:call-template name="input-field">
							...
							<xsl:with-param name="value"><xsl:value-of select="localization:getDecode($language, 'N047', 'TPT')"/></xsl:with-param>
							<xsl:with-param name="override-displaymode">view</xsl:with-param>
						</xsl:call-template>
						<xsl:if test="entities[number(.) &gt; 0] and $displaymode='edit'">
							<xsl:call-template name="entity-field">
							    ...
							    <xsl:with-param name="override-sub-product-code">FT:TPT</xsl:with-param>
						    </xsl:call-template>
						</xsl:if>
	...
	<xsl:template name="tpt-notify-beneficiary-details">
		<xsl:choose>
			<xsl:when test="$displaymode='edit'">
				<xsl:call-template name="fieldset-wrapper">
	...
	<!-- Added by Sapna for MPS18871 on 5/4/2013 -->
