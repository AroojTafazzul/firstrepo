...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
			...
			<xsl:variable name="bankDetailsFromBic" select="tools:retrieveBankDetailsFromBICCode(//UsrTxRef/IdIssr/BIC)" />
		    <xsl:variable name="bankAbbvName"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_abbv_name"/></xsl:variable>
		    <xsl:variable name="bankName"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_name"/></xsl:variable>
			<xsl:variable name="bankAddrLine1"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_addr_line_1"/></xsl:variable>
			<xsl:variable name="bankAddrLine2"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_addr_line_2"/></xsl:variable>
			<xsl:variable name="bankDom"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_dom"/></xsl:variable>
			<xsl:variable name="bankRef"><xsl:value-of select="$bankDetailsFromBic/bank_details/bank_ref"/></xsl:variable>
			<xsl:variable name="companyId"><xsl:value-of select="tools:getCompanyIdFromCompName($bankAbbvName)"/></xsl:variable>
			<xsl:variable name="companyName"><xsl:value-of select="$bankAbbvName"/></xsl:variable>
			...
			<tu_tnx_record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://www.neomalogic.com/gtp/interfaces/xsd/tu.xsd">
				...
				<!-- <po_ref_id><xsl:value-of select="//UsrTxRef[IdIssr/BIC = $BIC]/Id"/></po_ref_id> -->
				...
				<issuing_bank>
						...
						<iso_code><xsl:value-of select="//UsrTxRef/IdIssr/BIC"/></iso_code>
					</issuing_bank>
				<narrative_xml>
