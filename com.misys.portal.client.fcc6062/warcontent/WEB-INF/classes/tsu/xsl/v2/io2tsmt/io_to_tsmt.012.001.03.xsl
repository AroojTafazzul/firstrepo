...
<!-- This XSL will properly generate TM Resubmission XML with valid data.This XSL is in compliance with tsmt.012.001.03.xsd  -->
  ...
  <!-- Actual baseline details -->
			<Baseln>
				...
				<Buyr>
					...
					<PstlAdr>
						<xsl:if test="buyer_street_name and buyer_street_name!=''">
					...
					<!-- TODO:: correct mapping? PostCodeIdentification <PstCdId> -->
						...
						<xsl:if test="buyer_country_sub_div and buyer_country_sub_div!=''">
				...
				<Sellr>
					...
					<PstlAdr>
						<xsl:if test="seller_street_name and seller_street_name!=''">
					...
					<!-- correct mapping? PostCodeIdentification <PstCdId> -->
						...
						<xsl:if test="seller_country_sub_div and seller_country_sub_div !=''">
							...
							<xsl:choose>
								...
								<xsl:when test="fin_inst_name and fin_inst_name!=''">
									<NmAndAdr>
										...
										<Adr>
											<xsl:if test="fin_inst_street_name and fin_inst_street_name!=''">
												<StrtNm>
											...
											<xsl:if test="fin_inst_country_sub_div and and fin_inst_country_sub_div !=''">
												<CtrySubDvsn>
	...
	<LineItmDtls>
		...
		<xsl:if test="qty_tol_pstv_pct and qty_tol_pstv_pct !=''">
			<QtyTlrnce>
				...
				<xsl:if test="qty_tol_neg_pct and qty_tol_neg_pct !=''">
					<MnsPct>
...
<xsl:template match="routing_summaries/rs_tnx_record" mode="multimodal">
...
<xsl:template match="contacts/contact">
	<xsl:if test="type[.='01']">
		<BuyrCtctPrsn>
			...
			<xsl:if test="phone_number and phone_number != ''">
			<PhneNb><xsl:value-of select="phone_number"/></PhneNb>
			</xsl:if>
			<xsl:if test="fax_number and fax_number != ''">
