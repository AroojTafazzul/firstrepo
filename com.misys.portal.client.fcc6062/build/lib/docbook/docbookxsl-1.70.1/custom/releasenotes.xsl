...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:if test="$footer.rule != 0">
	...
	<xsl:template name="header.content">
			...
			<xsl:choose>
				<xsl:when test="$double.sided != 0  and $sequence != 'even'">
					...
					<xsl:choose>
						...
						<fo:block end-indent="0mm" >
							<fo:external-graphic src="url(./images/header.gif)" height="6mm" content-height="scale-to-fit"/>
							</fo:block>
							<!--<fo:block>
								...
								<xsl:value-of 
									select="/article/articleinfo/corpauthor"/>
							</fo:block>
							<fo:block>
								<xsl:value-of 
									select="/article/articleinfo/title/productname"/>
							</fo:block>-->
						</xsl:when>
					</xsl:choose>
				</xsl:when>
										...
										<fo:table-row>
											...
											<fo:table-cell>
												<fo:block text-align="left">
												    ...
												    <xsl:value-of select="/article/articleinfo/title[1]"/>&#160;
													<xsl:value-of select="/article/articleinfo/releaseinfo"/>&#160;
												</fo:block>												
											</fo:table-cell>
											<!-- Header logo -->
											<fo:table-cell>
		...
		<xsl:choose>
			<xsl:when 
				test="$pageclass = 'titlepage' and $gentext-key = 'article'
                    and $sequence='first'">
...
<xsl:template name="footer.table">
  ...
  <xsl:choose>
    <xsl:when test="$pageclass='titlepage' and $gentext-key='article'
                    and $sequence='first'">
	...
	<xsl:template match="guisubmenu">
		...
		<fo:inline>
			<xsl:attribute name="color"><xsl:value-of select="$orange"/></xsl:attribute>
			<xsl:attribute name="font-weight">bold</xsl:attribute>
			<xsl:if test="parent::entry">
				<xsl:attribute name="font-size">
					<xsl:value-of select="$body.font.master * 0.8"/>
	...
	<xsl:template name="article.titlepage.recto">
		...
		<fo:block  space-after="2mm" start-indent="25mm" font-size="30pt" font-weight="bold">
			...
			<!--print the second title for the local service manual
					...
					has been chosen-->
			 <xsl:if test="count(/article/articleinfo/title) = 2">
				<fo:block space-after="4mm" start-indent="25mm"  font-size="17pt" font-weight="bold">
					...
					<xsl:value-of select="/article/articleinfo/title[2]"/>
				</fo:block>
			</xsl:if>			
		...
		<fo:block margin-top="145mm" start-indent="25mm"  font-size="8pt" margin-right="20mm">
			...
			<!-- <xsl:value-of select="/article/articleinfo/legalnotice[1]"/> -->
		</fo:block>
		<fo:block margin-top="2mm" start-indent="25mm"  font-size="8pt" margin-right="20mm">
			<xsl:attribute name="color">
				<xsl:value-of select="$white"/>
			</xsl:attribute>
			<xsl:value-of select="/article/articleinfo/copyright/symbol"/>&#160;<xsl:value-of select="/article/articleinfo/copyright/holder"/>&#160;<xsl:value-of select="/article/articleinfo/copyright/year"/><xsl:text>.</xsl:text>&#160;<xsl:value-of select="/article/articleinfo/copyright/additionaltext"/>
		</fo:block>
	</xsl:template>
	...
	<xsl:template match="informaltable/tbody/row[1]">
...
<!--  -->
<!--  Page after the cover customization. -->
<!--  -->
<xsl:template name="article.titlepage.verso">
  <xsl:apply-templates mode="book.titlepage.verso.auto.mode" select="articleinfo/legalnotice"/>
...
<xsl:template match="para[@id = 'address']">
      <fo:block start-indent="12mm" space-before="180mm">
      <xsl:apply-templates/>
      </fo:block>
</xsl:template>

<xsl:template match="para[@id = 'info']">
      <fo:block start-indent="12mm" xsl:use-attribute-sets="normal.para.spacing">
      <xsl:apply-templates/>
      </fo:block>
</xsl:template>

<xsl:template match="para[@id = 'info']/itemizedlist/listitem" priority="2">
...
<xsl:template name="page.count.separator">
	<xsl:param name="lang"><xsl:value-of select="/article/@lang"/></xsl:param>
	<xsl:choose>
...
<xsl:template match="article">
  ...
  <fo:page-sequence hyphenate="{$hyphenate}"
                    master-reference="coversequence">
    <xsl:attribute name="language">
      <xsl:call-template name="l10n.language"/>
    </xsl:attribute>
    <xsl:attribute name="format">
      <xsl:call-template name="page.number.format">
        <xsl:with-param name="master-reference" select="coversequence"/>
      </xsl:call-template>
    </xsl:attribute>
