...
<!--
#===================================================
# OOo2sDBK : OpenOffice-Writer to Docbook
#===================================================
#
# :author: Eric Bellot
...
-->
<xsl:stylesheet 
	exclude-result-prefixes="office style text table draw fo xlink number svg chart dr3d math form script dc meta" 
	...
	<xsl:param name="oooVersionNumber">
		<xsl:value-of 
			select="substring-before(substring-after($generator,' '),' ')"/>
	</xsl:param>
	...
	<xsl:template name="measureUnit">
		<xsl:param name="firstValue" 
			select="//style:properties[1]/@style:width"/>
...
-->
	<xsl:template match="/">
		<book>
			...
			<xsl:choose>
				<xsl:when 
					test="/office:document/office:body/text:h[@text:level='1']">
...
====================
-->
	<xsl:template match="*|@*" mode="hierarchy" name="hierarchy">
		<!-- Le paramètre "level" permet de mémoriser la profondeur du niveau en cours (1, 2, etc.) -->
		...
		<xsl:choose>
			...
			<!-- Si le titre "h" est cours est d'une profondeur plus élevée que le précédent... -->
			<xsl:when test="@text:level &gt; $level">
				<!-- Execute les modèles qui précèdent le 1er titre de niveau 1-->
				<xsl:if 
					test="count(preceding-sibling::text:h[@text:level='1'])=0">
					<xsl:apply-templates select="preceding-sibling::*"/>
				</xsl:if>
				<!-- ...On construit une nouvelle section (sectn) et le contenu du tag "h" source est placé dans le tag "title" de sortie  -->
				<xsl:choose>
				...
				<xsl:apply-templates mode="scanLevel" 
					select="following-sibling::*[1]">
...
-->
	...
	<xsl:template name="bookInfo">
		<xsl:param name="bodyRootPara" 
			select="/office:document/office:body/text:p"/>
		<bookinfo>
			<xsl:apply-templates mode="bookInfo" select="$bodyRootPara"/>
			<xsl:apply-templates 
				select="/office:document/office:meta/meta:keywords"/>
			<xsl:apply-templates 
				select="/office:document/office:meta/dc:subject"/>
		</bookinfo>
	</xsl:template>
	<xsl:template match="text:p" mode="bookInfo">
		<xsl:param name="parentStyleName" 
			select="/office:document/office:automatic-styles/style:style[@style:name=(current()/@text:style-name)]/@style:parent-style-name"/>
		<xsl:param name="inAuthor" select="'false'"/>
		<xsl:call-template name="multimedia.top"/>
		<xsl:if test="@text:style-name='Title' or $parentStyleName='Title'">
			<title>
				<xsl:apply-templates/>
			</title>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Subtitle' or $parentStyleName='Subtitle'">
			<subtitle>
				<xsl:apply-templates/>
			</subtitle>
		</xsl:if>
		<xsl:if test="@text:style-name='Pubdate' or $parentStyleName='Pubdate'">
			<pubdate>
				<xsl:apply-templates/>
			</pubdate>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Releaseinfo' or $parentStyleName='Releaseinfo'">
			<releaseinfo>
				<xsl:apply-templates/>
			</releaseinfo>
		</xsl:if>
		<xsl:if test="@text:style-name='Author' or $parentStyleName='Author'">
			<author>
				<xsl:call-template name="authorContent"/>
			</author>
		</xsl:if>
		<xsl:if test="@text:style-name='Editor' or $parentStyleName='Editor'">
			<editor>
				<xsl:call-template name="authorContent"/>
			</editor>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Authorblurb' or $parentStyleName='Authorblurb'">
		...
		<xsl:if 
			test="@text:style-name='Othercredit' or $parentStyleName='Othercredit'">
			<othercredit>
				<xsl:call-template name="authorContent"/>
			</othercredit>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Corpauthor' or $parentStyleName='Corpauthor'">
			<corpauthor>
				<xsl:apply-templates/>
			</corpauthor>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Copyright' or $parentStyleName='Copyright'">
			<copyright>
				<xsl:apply-templates select="text:span"/>
			</copyright>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Bibliomisc' or $parentStyleName='Bibliomisc'">
			<bibliomisc>
				<xsl:apply-templates/>
			</bibliomisc>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Abstract' or $parentStyleName='Abstract'">
			<abstract>
				<xsl:call-template name="paraWithTitle"/>
			</abstract>
		</xsl:if>
		<xsl:if 
			test="@text:style-name='Legalnotice' or $parentStyleName='Legalnotice'">
			<legalnotice>
				<xsl:call-template name="paraWithTitle"/>
			</legalnotice>
		</xsl:if>
	...
	<xsl:template name="authorContent">
		...
		<xsl:param name="parentStyleNameOfFollowing" 
			select="/office:document/office:automatic-styles/ style:style[@style:name=current()/following-sibling::*[1]/@text:style-name]/ @style:parent-style-name"/>
		<xsl:param name="parentStyleNameOfChild" 
			select="/office:document/office:automatic-styles/ style:style[@style:name=current()/*/@text:style-name]/ @style:parent-style-name"/>
		<xsl:param name="inAuthor"/>
		<xsl:apply-templates 
			select="text:span[@text:style-name='Honorific' or  $parentStyleNameOfChild='Honorific']| text:span[@text:style-name='Firstname' or  $parentStyleNameOfChild='Firstname']| text:span[@text:style-name='Othername' or  $parentStyleNameOfChild='Othername']| text:span[@text:style-name='Surname' or  $parentStyleNameOfChild='Surname']| text:span[@text:style-name='Lineage' or  $parentStyleNameOfChild='Lineage']"/>
		<xsl:if 
			test="text:span[@text:style-name='Jobtitle' or  $parentStyleNameOfChild='Jobtitle']| text:span[@text:style-name='Orgname' or  $parentStyleNameOfChild='Orgname']">
			<affiliation>
				<xsl:apply-templates 
					select="text:span[@text:style-name='Jobtitle' or  $parentStyleNameOfChild='Jobtitle']| text:span[@text:style-name='Orgname' or  $parentStyleNameOfChild='Orgname']"/>
			</affiliation>
		</xsl:if>
		<xsl:if 
			test="following-sibling::*[1]  [@text:style-name='Authorblurb' or  $parentStyleNameOfFollowing='Authorblurb']">
...
-->
	...
	<xsl:template name="para">
		...
		<xsl:param name="parentStyleNameOfPreceding" 
			select="/office:document/office:automatic-styles/style:style[@style:name=current()/preceding-sibling::*[1]/@text:style-name]/@style:parent-style-name"/>
		<xsl:param name="parentStyleNameOfFollowing" 
			select="/office:document/office:automatic-styles/style:style[@style:name=current()/following-sibling::*[1]/@text:style-name]/@style:parent-style-name"/>
		...
		<xsl:choose>
			<xsl:when 
				test="@text:style-name='Epigraph' or $parentStyleName='Epigraph'">
				<epigraph>
					<xsl:if 
						test="following-sibling::*[1][name()='text:p' and @text:style-name='Attribution' or $parentStyleNameOfFollowing='Attribution']">
						<xsl:apply-templates mode="attribution" 
							select="following-sibling::*[1]"/>
					</xsl:if>
			...
			<xsl:when 
				test="@text:style-name='Quotations' or $parentStyleName='Quotations' or @text:style-name='Blockquote' or $parentStyleName='Blockquote'">
				<blockquote>
					<xsl:if 
						test="following-sibling::*[1][name()='text:p' and @text:style-name='Attribution' or $parentStyleNameOfFollowing='Attribution']">
						<xsl:apply-templates mode="attribution" 
			...
			<xsl:when 
				test="@text:style-name='Preformatted Text' or $parentStyleName='Preformatted Text' or @text:style-name='ProgramListing' or $parentStyleName='ProgramListing'">
				<programlisting>
					<xsl:apply-templates>
			...
			<xsl:when 
				test="@text:style-name='Literallayout' or $parentStyleName='Literallayout'">
				<literallayout>
					<xsl:apply-templates>
						<xsl:with-param name="parent" 
							select="'programListing'"/>
			...
			<xsl:when test="@text:style-name='Term' or $parentStyleName='Term'">
				<xsl:if 
					test="(preceding-sibling::*[1]/@text:style-name != 'Definition' or $parentStyleNameOfPreceding != 'Definition') and (preceding-sibling::*[1]/@text:style-name != 	'Term' or $parentStyleNameOfPreceding != 'Term')">
					<xsl:call-template name="varlistterm"/>
				</xsl:if>
				<xsl:if 
					test="$source='varlist' and (preceding-sibling::*[1]/@text:style-name='Definition' or $parentStyleNameOfPreceding='Definition')">
					<xsl:call-template name="varlist"/>
				</xsl:if>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Definition' or $parentStyleName='Definition'">
	...
	mode="varlist"/>
			...
			<xsl:when 
				test="@text:style-name='Heading' or $parentStyleName='Heading'"/>
			<xsl:when 
				test="@text:style-name='Attribution' or $parentStyleName='Attribution'"/>
			<xsl:when test="text:sequence"/>
			<!-- suppress bookInfo's styles -->
			<xsl:when 
				test="@text:style-name='Title' or $parentStyleName='Title'"/>
			<xsl:when 
				test="@text:style-name='Subtitle' or $parentStyleName='Subtitle'"/>
			<xsl:when 
				test="@text:style-name='Releaseinfo' or $parentStyleName='Releaseinfo'"/>
			<xsl:when 
				test="@text:style-name='Pubdate' or $parentStyleName='Pubdate'"/>
			<xsl:when 
				test="@text:style-name='Author' or $parentStyleName='Author'"/>
			<xsl:when 
				test="@text:style-name='Editor' or $parentStyleName='Editor'"/>
			<xsl:when 
				test="@text:style-name='Authorblurb'  or $parentStyleName='Authorblurb'"/>
			<xsl:when 
				test="@text:style-name='Othercredit' or $parentStyleName='Othercredit'"/>
	...
	<xsl:template name="paraWithBreakLine">
		<xsl:if test="text:line-break">
			<xsl:apply-templates mode="break2para" select="text:line-break"/>
		</xsl:if>
	...
	<xsl:template match="text:line-break" mode="break2para">
		...
		<xsl:variable name="content" 
			select="preceding-sibling::node()[not($br-before) or generate-id(preceding-sibling::text:line-break[1]) =generate-id($br-before)]"/>
		<xsl:if test="$content" >
			<para>
	...
	<xsl:template match="text:s">
		...
		<xsl:if test="$parent='programListing'">
			<xsl:call-template name="spaceRecursif">
				<xsl:with-param name="spaceOccurence" select="$spaceOccurence"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="precedingTitle">
		<xsl:param name="parentStyleNameOfPreceding" 
			select="/office:document/office:automatic-styles/ style:style[@style:name=current()/preceding-sibling::*[1]/ @text:style-name]/@style:parent-style-name"/>
		<xsl:if 
			test="preceding-sibling::*[1][name()='text:p'  and (@text:style-name='Heading' or $parentStyleNameOfPreceding ='Heading')]">
			<xsl:apply-templates mode="includeTitle" 
				select="current()/preceding-sibling::*[1]"/>
		</xsl:if>
	</xsl:template>
 ...
 'Term' and 'Definition' : not defaults styles of OpenOffice -->
	...
	<xsl:template name="varlist">
		...
		<xsl:param name="parentStyleNameOfFollowing2" 
			select="/office:document/office:automatic-styles/style:style[@style:name=current()/following-sibling::*[2]/@text:style-name]/@style:parent-style-name"/>
		<varlistentry>
			<term>
				<xsl:apply-templates/>
			</term>
			<listitem>
				<xsl:if 
					test="following-sibling::*[1][name()='text:p' and (@text:style-name='Definition' or $parentStyleNameOfFollowing='Definition')]">
					<xsl:apply-templates mode="varlist" 
						select="following-sibling::*[1]"/>
				</xsl:if>
			</listitem>
		</varlistentry>
...
<xsl:apply-templates select="following-sibling::*[1]"
 mode="varlist"/>
		...
		<xsl:if 
			test="following-sibling::*[2][name()='text:p' and (@text:style-name='Term' or $parentStyleNameOfFollowing2='Term')]">
			<xsl:apply-templates mode="varlist" 
...
-->
	...
	<xsl:template match="text:ordered-list" name="ordList">
		<xsl:param name="styleName">
			<xsl:if 
				test="ancestor-or-self::text:ordered-list[last()]/   @text:style-name">
				<xsl:value-of 
					select="ancestor-or-self::text:ordered-list [last()]/@text:style-name"/>
			</xsl:if>
			<xsl:if 
				test="ancestor-or-self::text:unordered-list   [last()]/@text:style-name">
				<xsl:value-of 
					select="ancestor-or-self::text:unordered-list   [last()]/@text:style-name"/>
			</xsl:if>
		</xsl:param>
		<xsl:param name="level" 
			select="count(ancestor-or-self::text:ordered-list) + count(ancestor-or-self::text:unordered-list)"/>
		<xsl:param name="numStyle" 
			select="/office:document/office:automatic-styles/ text:list-style[@style:name=$styleName]/text:list-level-style-number[@text:level=$level]/@style:num-format"/>
		<orderedlist>
...
-->
	<xsl:template match="text:span">
		<xsl:param name="fontStyle" 
			select="/office:document/office:automatic-styles/style:style[@style:name=(current()/@text:style-name)]/style:properties/@fo:font-style"/>
		<xsl:param name="fontWeight" 
			select="/office:document/office:automatic-styles/style:style[@style:name=(current()/@text:style-name)]/style:properties/@fo:font-weight"/>
		<xsl:param name="parentStyleName" 
			select="/office:document/office:automatic-styles/style:style[@style:name=(current()/@text:style-name)]/@style:parent-style-name"/>
		<xsl:choose>
			...
			<xsl:when 
				test="@text:style-name='Emphasis'  or $parentStyleName='Emphasis'">
				<emphasis>
					<xsl:apply-templates/>
				</emphasis>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Strong Emphasis'  or $parentStyleName='Strong Emphasis'">
				<emphasis>
			...
			<xsl:when 
				test="@text:style-name='Citation'  or $parentStyleName='Citation'">
				<quote>
					<xsl:apply-templates/>
				</quote>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Source Text' or @text:style-name='Example' or @text:style-name='Teletype' or @text:style-name='Literal' or $parentStyleName='Source Text'  or $parentStyleName='Example' or $parentStyleName='Teletype' or $parentStyleName='Literal'">
				<literal>
					<xsl:apply-templates/>
				</literal>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='User Entry'  or $parentStyleName='User Entry' or @text:style-name='Userentry'  or $parentStyleName='Userentry'">
				<userinput>
					<xsl:apply-templates/>
				</userinput>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Filename'  or $parentStyleName='Filename'">
				<filename>
					<xsl:apply-templates/>
				</filename>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Computeroutput' or $parentStyleName='Computeroutput'">
				<computeroutput>
					<xsl:apply-templates/>
				</computeroutput>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Command'  or $parentStyleName='Command'">
				<command>
					<xsl:apply-templates/>
				</command>
			</xsl:when>
			<!--added inline style-->
			<xsl:when 
				test="@text:style-name='Varname' or $parentStyleName='Varname'">
				<varname>
					<xsl:apply-templates/>
				</varname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Classname' or $parentStyleName='Classname'">
				<classname>
					<xsl:apply-templates/>
				</classname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Guimenuitem' or $parentStyleName='Guimenuitem'">
				<guimenuitem>
					<xsl:apply-templates/>
				</guimenuitem>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Guibutton' or $parentStyleName='Guibutton'">
				<guibutton>
					<xsl:apply-templates/>
				</guibutton>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Database' or $parentStyleName='Database'">
				<database>
					<xsl:apply-templates/>
				</database>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Wordasword' or $parentStyleName='Wordasword'">
				<wordasword>
					<xsl:apply-templates/>
				</wordasword>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Action' or $parentStyleName='Action'">
				<action>
					<xsl:apply-templates/>
				</action>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Productname' or $parentStyleName='Productname'">
				<productname>
					<xsl:apply-templates/>
				</productname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Guilabel' or $parentStyleName='Guilabel'">
				<guilabel>
					<xsl:apply-templates/>
				</guilabel>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Application' or $parentStyleName='Application'">
				<application>
					<xsl:apply-templates/>
				</application>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Keycap' or $parentStyleName='Keycap'">
				<keycap>
					<xsl:apply-templates/>
				</keycap>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Guiicon' or $parentStyleName='Guiicon'">
				<guiicon>
					<xsl:apply-templates/>
				</guiicon>
			</xsl:when>
			...
			<xsl:when 
				test="@text:style-name='Abbrev' or $parentStyleName='Abbrev'">
				<abbrev>
					<xsl:apply-templates/>
				</abbrev>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Parameter' or $parentStyleName='Parameter'">
				<parameter>
					<xsl:apply-templates/>
				</parameter>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Errorname' or $parentStyleName='Errorname'">
				<errorname>
					<xsl:apply-templates/>
				</errorname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Errorcode' or $parentStyleName='Errorcode'">
				<errorcode>
					<xsl:apply-templates/>
				</errorcode>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Sgmltag' or $parentStyleName='Sgmltag'">
				<sgmltag>
					<xsl:apply-templates/>
				</sgmltag>
			</xsl:when>
			<xsl:when test="@text:style-name='Type' or $parentStyleName='Type'">
				<type>
					<xsl:apply-templates/>
				</type>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Token' or $parentStyleName='Token'">
				<token>
					<xsl:apply-templates/>
				</token>
			</xsl:when>
			<!--End add-->
			<xsl:when 
				test="@text:style-name='Option'  or $parentStyleName='Option'">
				<option>
					<xsl:apply-templates/>
				</option>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Acronym'  or $parentStyleName='Acronym'">
				<acronym>
					<xsl:apply-templates/>
				</acronym>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Lineannotation'  or $parentStyleName='Lineannotation'">
				<lineannotation>
					<xsl:apply-templates/>
				</lineannotation>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Replaceable'  or $parentStyleName='Replaceable'">
				<replaceable>
					<xsl:apply-templates/>
				</replaceable>
			</xsl:when>
			...
			<xsl:when 
				test="@text:style-name='Honorific'  or $parentStyleName='Honorific'">
				<honorific>
					<xsl:apply-templates/>
				</honorific>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Firstname'  or $parentStyleName='Firstname'">
				<firstname>
					<xsl:apply-templates/>
				</firstname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Othername'  or $parentStyleName='Othername'">
				<othername>
					<xsl:apply-templates/>
				</othername>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Surname'  or $parentStyleName='Surname'">
				<surname>
					<xsl:apply-templates/>
				</surname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Lineage' or $parentStyleName='Lineage'">
				<lineage>
					<xsl:apply-templates/>
				</lineage>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Year'  or $parentStyleName='Year'">
				<year>
					<xsl:apply-templates/>
				</year>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Holder'  or $parentStyleName='Holder'">
				<holder>
					<xsl:apply-templates/>
				</holder>
			</xsl:when>
			<xsl:when 
				test="(@text:style-name='Email'  or $parentStyleName='Email')">
				<email>
					<xsl:apply-templates/>
				</email>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Jobtitle'  or $parentStyleName='Jobtitle'">
				<jobtitle>
					<xsl:apply-templates/>
				</jobtitle>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Orgname' or $parentStyleName='Orgname'">
				<orgname>
					<xsl:apply-templates/>
				</orgname>
			</xsl:when>
			<xsl:when 
				test="@text:style-name='Citetitle'  or $parentStyleName='Citetitle'">
				<citetitle>
					<xsl:apply-templates/>
				</citetitle>
			</xsl:when>
...
-->
	<xsl:template match="text:footnote|text:endnote">
		<footnote>
			...
			<xsl:apply-templates mode="footnote" select="text:footnote-body/*"/>
		</footnote>
	</xsl:template>
...
-->
	...
	<xsl:template 
		match="text:reference-mark|text:bookmark|  text:reference-mark-start|text:bookmark-start">
		<phrase>
			<xsl:attribute name="id">
	...
	<xsl:template 
		match="text:reference-ref|text:bookmark-ref|  text:sequence-ref">
		<link>
			<xsl:attribute name="linkend">
				<xsl:value-of select="@text:ref-name"/>
			</xsl:attribute>
...
-->
	...
	<xsl:template 
		match="draw:image[(@text:anchor-type='paragraph')   or (@text:anchor-type='char')]"/>
	<xsl:template match="draw:a[draw:image[@text:anchor-type='page']]"/>
	<xsl:template 
		match="draw:a[draw:image[(@text:anchor-type='paragraph')   or (@text:anchor-type='char')]]"/>
	<xsl:template name="multimedia.top">
		<xsl:param name="calledPos" select="'top'"/>
		<xsl:apply-templates mode="extract" 
			select=".//draw:image[(@text:anchor-type='paragraph' or text:anchor-type='char') and not(ancestor::draw:text-box)]|.//draw:text-box">
	...
	<xsl:template match="draw:text-box" mode="extract">
		<xsl:param name="parentStyleName" 
			select="/office:document/office:automatic-styles/style:style[@style:name=current()/@draw:style-name]/@style:parent-style-name"/>
		<xsl:param name="calledPos"/>
		<xsl:param name="currentPos">
			<xsl:choose>
				<xsl:when 
					test="/office:document/office:automatic-styles/ style:style[@style:name=current() /@draw:style-name]/style:properties/ @style:vertical-pos!=''">
					<xsl:value-of 
						select="/office:document/ office:automatic-styles/style:style [@style:name=current()/@draw:style-name] /style:properties/@style:vertical-pos"/>
				</xsl:when>
		...
		<xsl:if 
			test="$calledPos=$currentPos or  ($calledPos='top' and $currentPos!='bottom')">
			<xsl:if test="$parentStyleName='Note'">
				<xsl:call-template name="note"/>
			</xsl:if>
			<xsl:if 
				test="$parentStyleName='Frame'or $parentStyleName='Graphics' ">
	...
	<xsl:template name="figure">
		<figure>
			...
			<mediaobject>
				<xsl:apply-templates mode="extract" 
					select=".//draw:image[(@text:anchor-type='paragraph')  or (@text:anchor-type='char')]">
	...
	<xsl:template name="note">
		...
		<note>
			<xsl:apply-templates mode="includeTitle" 
				select="text:p[@text:style-name='Heading'   or $parentStyleName='Heading']"/>
...
-->
	<xsl:template match="draw:image" mode="extract">
		...
		<xsl:choose>
			<xsl:when 
				test="substring-before(@xlink:href,'/')='#Pictures'  or office:binary-data">
				<xsl:if 
					test="$calledPos=$currentPos or  ($calledPos='top' and $currentPos!='bottom')">
					<para>WARNING ! Incorporated graphics are not 
						supported.</para>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template 
		match="text:p[@text:style-name='Illustration']|  text:p[@text:style-name='Figure']">
		<xsl:choose>
			<xsl:when test="text:sequence">
				<xsl:apply-templates 
...
-->
	<xsl:template match="table:table" name="table">
		...
		<xsl:choose>
			<xsl:when 
				test="following-sibling::*[1]/text:sequence/@text:name='Table'">
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when 
						test="preceding-sibling::*[1]/text:sequence/@text:name='Table'">
						<xsl:call-template name="formalTable">
							<xsl:with-param name="titlePosition" 
								select="'before'"/>
							<xsl:with-param name="colsList" select="$colsList"/>
							<xsl:with-param name="colsNumber" 
								select="$colsNumber2"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template name="formalTable">
		...
		<table>
			...
			<xsl:if test="$titlePosition='before'">
				...
				<xsl:apply-templates mode="tableTitle" 
			...
			<tgroup>
				...
				<xsl:apply-templates 
					select="table:table-row|table:table-header-rows/table:table-row">
					<xsl:with-param name="colsList" select="$colsList"/>
				</xsl:apply-templates>
			</tgroup>
		</table>
...
-->
	...
	<xsl:template name="colspecAttrib">
		...
		<xsl:if test="$colsNumber &gt;= $cycle">
			...
			<xsl:call-template name="colspecAttrib">
				<xsl:with-param name="colsList" 
					select="substring-after($colsList,';')"/>
...
-->
			...
			<xsl:otherwise>
				<xsl:call-template name="ascendantcolsList">
					<xsl:with-param name="newList" 
						select="concat($newList, $littleElt,';')"/>
	...
	<xsl:template name="colsListtrunc">
		...
		<xsl:param name="littleElt"/>
		<xsl:param name="eltCompare" 
			select="substring-before($colsListTest,';')"/>
		<xsl:param name="fragmentBefore" select="''"/>
		<xsl:param name="fragmentAfter" 
			select="substring-after($colsListTest,';')"/>
		<xsl:choose>
			<xsl:when 
				test="($littleElt != $eltCompare) and ($fragmentAfter != '')">
				<xsl:call-template name="colsListtrunc">
					<xsl:with-param name="fragmentBefore">
						<xsl:value-of 
							select="concat($fragmentBefore,';',$eltCompare)"/>
					</xsl:with-param>
					<xsl:with-param name="littleElt" select="$littleElt"/>
					<xsl:with-param name="colsListTest" 
						select="$fragmentAfter"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when 
						test="starts-with(concat($fragmentBefore,';',$fragmentAfter),';')">
						<xsl:value-of 
							select="substring-after(concat($fragmentBefore,';',$fragmentAfter),';')"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of 
							select="concat($fragmentBefore,';',$fragmentAfter)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
...
-->
	<xsl:template name="fineColsList">
		...
		<xsl:param name="colsListTest" 
			select="substring-after($colsListBrut,';')"/>
		<xsl:param name="eltBaseTest" 
			select="normalize-space(substring-before($colsListBrut,';'))"/>
		<xsl:param name="eltTestresult">
			<!-- 'True' si doublon -->
		...
		<xsl:choose>
			<xsl:when test="$eltNumber = 1">
				<xsl:value-of 
					select="concat(normalize-space($eltBaseTest),';',$newList)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="fineColsList">
					<xsl:with-param name="newList">
						<xsl:choose>
							<xsl:when test="$eltTestresult = 'false'">
								<xsl:value-of 
									select="concat($eltBaseTest,';', $newList)"/>
							</xsl:when>
							<xsl:otherwise>
	...
	<xsl:template name="testDoublon">
		...
		<xsl:param name="eltBaseTest"/>
		<xsl:param name="eltListTest" 
			select="normalize-space(substring-before($colsListTest,';'))"/>
		<xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="$eltNumber &gt;= 1">
						<xsl:call-template name="testDoublon">
							<xsl:with-param name="colsListTest" 
								select="substring-after($colsListTest,';')"/>
							<xsl:with-param name="eltBaseTest" 
								select="$eltBaseTest"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
...
-->
	...
	<xsl:template name="colsList">
		<xsl:param name="stylesURI" 
			select="/office:document/office:automatic-styles/style:style"/>
...
-->
	<xsl:template name="subtableOrigine">
		...
		<xsl:param name="precedingCellNumber" 
			select="count(ancestor::table:table-cell[$cellAncestorNbr]   /preceding-sibling::table:table-cell) -count(ancestor::table:table-cell[$cellAncestorNbr]   /preceding-sibling::table:table-cell[@table:number-columns-spanned]) +sum(ancestor::table:table-cell[$cellAncestorNbr]/   preceding-sibling::table:table-cell/@table:number-columns-spanned)"/>
		<xsl:param name="columnValue">
			<xsl:choose>
				<xsl:when test="$precedingCellNumber &gt; 0">
					<xsl:apply-templates mode="cellOrigine" 
						select="ancestor::table:table-cell[$cellAncestorNbr]/parent::table:table-row/   parent::table:table/child::table:table-column[1]|  ancestor::table:table-cell[$cellAncestorNbr]/parent::table:table-row/  parent::table:sub-table/child::table:table-column[1]|  ancestor::table:table-cell[$cellAncestorNbr]/parent::table:table-row/   parent::table:table-header-rows/parent::table:table/child::table:table-column[1]">
						<xsl:with-param name="precedingCellNumber" 
							select="$precedingCellNumber"/>
						<xsl:with-param name="stylesURI" select="$stylesURI"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="$cellAncestorNbr &gt; 1">
				<xsl:call-template name="subtableOrigine">
					<xsl:with-param name="cellAncestorNbr" 
						select="$cellAncestorNbr - 1"/>
					<xsl:with-param name="cumulOrigineValue" 
						select="$cumulOrigineValue + $columnValue"/>
					<xsl:with-param name="stylesURI" select="$stylesURI"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="table:table-column" mode="cellOrigine" 
		name="cellOrigine">
		<xsl:param name="stylesURI"/>
		<xsl:param name="precedingCellNumber"/>
		...
		<xsl:param name="actualWidth">
			<xsl:value-of 
				select="substring-before($stylesURI[@style:name=$colName] /style:properties/@style:column-width,$measureUnit)"/>
		</xsl:param>
		<xsl:choose>
			<xsl:when test="$repetingCol &gt;= $precedingCellNumber">
				<xsl:value-of 
					select="$origineValue + ($actualWidth * $precedingCellNumber)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates mode="cellOrigine" 
	...
	<xsl:template name="colPosition"> <!-- externes --> <xsl:param 
		name="stylesURI"/> <xsl:param name="origineValue"/> <xsl:param 
		name="concat"/> <xsl:param name="repeatCol"/> <xsl:param 
		name="content"/> <xsl:param name="precedingColsSum"> <xsl:choose> 
		<xsl:when test="preceding-sibling::table:table-column"> 
		<xsl:call-template name="precedingColsSum"> <xsl:with-param 
		name="stylesURI" select="$stylesURI"/> </xsl:call-template> </xsl:when> 
		<xsl:otherwise>0</xsl:otherwise> </xsl:choose> </xsl:param> <xsl:param 
		name="actualColWidth"> <xsl:value-of 
		select="$stylesURI[@style:name=current()/@table:style-name]/ style:properties/@style:column-width"/> 
		</xsl:param> <xsl:param name="colWidthValue"> <xsl:value-of 
		select="substring-before($actualColWidth,$measureUnit)"/> </xsl:param> 
		<xsl:param name="colWidthRounded"> <xsl:call-template 
		name="roundValue"> <xsl:with-param name="inputValue" 
		select="$origineValue + $precedingColsSum + ($colWidthValue * $repeatCol)"/> 
		</xsl:call-template> </xsl:param> <xsl:value-of 
		select="$colWidthRounded"/>; <xsl:if test="$repeatCol &gt; 1"> 
		<xsl:call-template name="colPosition"> <xsl:with-param name="repeatCol" 
		select="$repeatCol - 1"/> <xsl:with-param name="stylesURI" 
		select="$stylesURI"/> <xsl:with-param name="origineValue" 
		select="$origineValue"/> </xsl:call-template> </xsl:if> </xsl:template>
	<xsl:template name="precedingColsSum">
		...
		<xsl:param name="precNodeNumber" 
			select="count(preceding-sibling::table:table-column)"/>
		<xsl:param name="content">
			<xsl:choose>
				<xsl:when 
					test=" $stylesURI[@style:name=current()/preceding-sibling::table:table-column[$precNodeNumber]/@table:style-name]/style:properties/@style:column-width">
					<xsl:value-of 
						select=" $stylesURI[@style:name=current()/ preceding-sibling::table:table-column[$precNodeNumber]/ @table:style-name]/ style:properties/@style:column-width"/>
				</xsl:when>
		...
		<xsl:param name="repetCols">
			<xsl:choose>
				<xsl:when 
					test="preceding-sibling::table:table-column[$precNodeNumber]/@table:number-columns-repeated">
					<xsl:value-of 
						select="number(preceding-sibling::table:table-column [$precNodeNumber]/@table:number-columns-repeated)"/>
				</xsl:when>
				<xsl:otherwise>1</xsl:otherwise>
			</xsl:choose>
		</xsl:param>
...
-->
	...
	<xsl:template match="table:table-header-rows/table:table-row">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<thead>
					<xsl:apply-templates mode="pas" select="current()">
						<xsl:with-param name="colsList" select="$colsList"/>
					</xsl:apply-templates>
				</thead>
			</xsl:otherwise>
	...
	<xsl:template 
		match="table:table/table:table-row|table:sub-table/table:table-row">
		<xsl:param name="colsList"/>
		<xsl:choose>
			<xsl:when test="count(preceding-sibling::table:table-row) != 0"/>
			<xsl:otherwise>
	...
	<xsl:template name="rowsProcess">
		...
		<xsl:param name="maxRowsInSubtables">
			<xsl:value-of 
				select="substring-before($maxRowsInSubtablesList,';')"/>
		</xsl:param>
		<xsl:param name="cycle" select="0"/>
		...
		<xsl:if test="$maxRowsInSubtables &gt; $cycle">
			<row>
				<xsl:apply-templates mode="rowProcess" 
	...
	<xsl:template match="table:table-row" mode="precedingDepth">
		...
		<xsl:param name="precedingSiblingDepth">
			<xsl:for-each select="preceding-sibling::table:table-row">
				<xsl:apply-templates mode="pas" select="table:table-cell[1]"/>
			</xsl:for-each>
		</xsl:param>
		...
		<xsl:choose>
			<xsl:when 
				test="generate-id(ancestor::table:table-cell[1]/   parent::table:table-row) != $rowID">
				<xsl:apply-templates mode="precedingDepth" 
					select="ancestor::table:table-cell[1]/parent::table:table-row">
					<xsl:with-param name="depthCumul" 
...
qui suivent la ligne courante -->
	<xsl:template match="table:table-row" mode="followingDepth">
		...
		<xsl:choose>
			<xsl:when 
				test="generate-id(ancestor::table:table-cell[1]/   parent::table:table-row) != $rowID">
				<xsl:apply-templates mode="followingDepth" 
					select="ancestor::table:table-cell[1]/parent::table:table-row">
					<xsl:with-param name="depthCumul" 
...
-->
	<xsl:template match="table:table-cell" mode="pas">
		...
		<xsl:param name="subtablesDepthRows">
			<xsl:choose>
				<xsl:when 
					test="table:sub-table/table:table-row/ table:table-cell/table:sub-table">
					<xsl:apply-templates mode="pas" 
						select="table:sub-table/table:table-row/   table:table-cell[table:sub-table][1]">
						<xsl:with-param name="depth" 
							select="concat($nucleon, '-', $depth)"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:param name="depthRows" 
			select="count(table:sub-table/table:table-row   [not(descendant::table:sub-table)])"/>
		<xsl:param name="totalDepthRows" select="$depthRows + $stDepthRowsSum"/>
		<xsl:choose>
			<xsl:when 
				test="following-sibling::table:table-cell[table:sub-table]">
				<xsl:apply-templates mode="pas" 
...
-->
	...
	<xsl:template match="table:table-cell" mode="rowProcess">
		...
		<xsl:param name="moreRowsBrut">
			<xsl:choose>
				<xsl:when 
					test="parent::table:table-row/child::table:table-cell[table:sub-table]and not(table:sub-table)">
					<xsl:apply-templates mode="pas" 
						select="parent::table:table-row/child::table:table-cell[table:sub-table][1]"/>
				</xsl:when>
				<xsl:otherwise>1</xsl:otherwise>
			</xsl:choose>
		</xsl:param>
		...
		<xsl:param name="moreRows">
			<xsl:choose>
				<xsl:when 
					test="($followingDepthValue = 0) and  $maxRowsInSubtables &gt; ($precedingDepthValue + $moreRowsValue)">
					<xsl:value-of 
						select="$maxRowsInSubtables - $precedingDepthValue "/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:param name="cellPosition">
			<xsl:choose>
				<xsl:when test="preceding-sibling::table:table-cell">
					<xsl:apply-templates mode="cellPosition" 
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="../parent::table:sub-table">
							<xsl:apply-templates mode="subtablePosition" 
	...
	<xsl:template name="cellEnd">
		...
		<xsl:param name="cellPosition">
			<xsl:apply-templates mode="cellPosition" select="current()">
				<xsl:with-param name="extremite" select="$extremite"/>
				<xsl:with-param name="colsList" select="$colsList"/>
			</xsl:apply-templates>
		</xsl:param>
...
-->
	<xsl:template match="table:table-cell" mode="cellPosition">
		...
		<xsl:param name="precedingCellsNbr" 
			select="count(preceding-sibling::table:table-cell  [not(@table:number-columns-spanned)]) + sum(preceding-sibling::table:table-cell/  @table:number-columns-spanned)"/>
		<xsl:param name="placeOfCol">
			<xsl:choose>
				<xsl:when 
					test="parent::table:table-row/parent::table:table|parent::table:table-row/parent::table:sub-table">
					<xsl:apply-templates mode="placeOfColl" 
						select="../../table:table-column[1]">
						<xsl:with-param name="placeOfCell" 
							select="$precedingCellsNbr + $currentCellSpan"/>
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="$placeOfCell &gt; $repeatOfCol">
						<xsl:apply-templates mode="placeOfColl" 
							select="following-sibling::table:table-column[1]">
							<xsl:with-param name="placeOfCell" 
								select="$placeOfCell - $repeatOfCol"/>
							<xsl:with-param name="placeOfColl" 
								select="$placeOfColl + 1"/>
						</xsl:apply-templates>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of 
							select="concat($placeOfColl,';',$placeOfCell)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
...
-->
	<xsl:template match="table:table-column" mode="colposition">
		...
		<xsl:param name="colposfine" 
			select="substring-before($colPosition,';')"/>
...
-->
			...
			<xsl:when test="substring-before($list,';') != ''">
				<xsl:call-template name="sumOfList">
					<xsl:with-param name="sum" 
						select="$sum + substring-before($list,';')"/>
					<xsl:with-param name="list" 
	...
	<xsl:template name="roundValue">
		...
		<xsl:choose>
			<xsl:when 
				test="$measureUnit = 'pt' or $measureUnit = 'pi' or $measureUnit = '%'">
	...
	<xsl:template name="entryNormal">
		...
		<xsl:param name="valign" 
			select="/office:document/office:automatic-styles/  style:style[@style:name=current()/@table:style-name]/ style:properties/@fo:vertical-align"/>
		<entry>
